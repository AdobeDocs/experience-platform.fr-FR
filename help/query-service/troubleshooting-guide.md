...
## Questions générales sur le service de requête {#general}

Cette section contient des informations sur les performances, les limites et les processus.

### Puis-je désactiver la fonction de saisie automatique dans l’éditeur du service de requête ?

+++Réponse
Non. La désactivation de la fonction de saisie semi-automatique n’est actuellement pas prise en charge par l’éditeur.
+++

### Pourquoi le requêteur ralentit-il parfois lorsque je saisis une requête ?
...
### Y a-t-il un problème ou un impact sur les performances du service de requête si plusieurs requêtes s’exécutent simultanément ?

+++Réponse
Non. Le service de requête dispose d’une fonctionnalité de mise à l’échelle automatique qui garantit que les requêtes simultanées n’ont aucun impact perceptible sur les performances du service.
+++
...
Consultez la documentation pour obtenir des conseils complets sur [comment travailler avec des structures de données imbriquées](./key-concepts/nested-data-structures.md) à l’aide du requêteur ou d’un client tiers.
+++
...
### Comment mettre en œuvre l’attribution personnalisée dans le service de requête ?
...
### Quelles fonctions d’assistant sont prises en charge par le service de requête ?

+++Réponse
Query Service fournit plusieurs fonctions d’assistance SQL intégrées pour étendre les fonctionnalités SQL. Consultez le document pour obtenir la liste complète des [fonctions SQL prises en charge par le service de requête](./sql/spark-sql-functions.md).
+++
...
Le service de requête prend en charge les fonctions SQL intégrées pour convertir une date et une heure données au format UTC et inversement à partir de ce format. Les deux méthodes `to_utc_timestamp()` et `from_utc_timestamp()` prennent deux paramètres : la date et l’heure, ainsi que le fuseau horaire.
...
La sortie de console fournie dans l’interface utilisateur du service de requête est plus lisible :
...
+++Réponse
Il existe des fonctionnalités de surveillance et d’alerte pour vérifier les activités et les statuts des requêtes. Voir les documents concernant l’[intégration du journal d’audit du service de requête](./data-governance/audit-log-guide.md) et les [journaux de requête](./ui/overview.md#log) pour plus d’informations.
+++
...
### Les connexions peuvent-elles être limitées à certains aspects du service de requête ou s’agit-il d’une solution « tout ou rien » ?

+++Réponse
Query Service est une solution « tout ou rien ». Un accès partiel ne peut pas être fourni.
+++

### Puis-je restreindre les données que le service de requête peut utiliser ou accède-t-il simplement à l’ensemble du lac de données d’Adobe Experience Platform ?

+++Réponse
Oui, vous pouvez restreindre l’interrogation des jeux de données avec un accès en lecture seule.
+++

### Quelles autres options existe-t-il pour limiter les données auxquelles le service de requête peut accéder ?
...
- Le service de requête prend en charge le contrôle d’accès basé sur les attributs. Vous pouvez restreindre l’accès aux données au niveau de la colonne, de la feuille et/ou au niveau struct. Consultez la documentation pour en savoir plus sur le contrôle d’accès basé sur les attributs.
...
### Le service de requête prend-il en charge la commande « INSERT OVERWRITE INTO » ?
...
### L’action « Créer une requête » est bloquée « Initialisation de la connexion... » lors de la tentative de connexion à Query Service. Comment résoudre le problème ?
...
### Existe-t-il un moyen d’extraire des données du service de requête après le traitement des requêtes et d’enregistrer les résultats dans un fichier CSV ? {#export-csv}

+++Réponse
Oui. Les données peuvent être extraites du service de requête et il est également possible de stocker les résultats au format CSV via une commande SQL.
...
### Puis-je connecter le service de requête à un outil tiers ?

+++Réponse
Oui, vous pouvez connecter plusieurs clients de bureau tiers à Query Service. Consultez la documentation sur les [détails complets sur les clients disponibles et comment les connecter au service de requête](./clients/overview.md).
+++

### Existe-t-il un moyen de connecter le service de requête une seule fois pour une utilisation continue avec un outil tiers ?
...
### Quels types d’éditeurs SQL tiers puis-je connecter à l’éditeur du service de requête ?

+++Réponse
Tout éditeur SQL tiers compatible avec le client PSQL ou [!DNL Postgres] peut être connecté à Query Service Editor. Consultez la documentation sur la [connexion des clients au service de requête](./clients/overview.md) pour obtenir une liste des instructions disponibles.
+++

### Puis-je connecter l’outil Power BI au service de requête ?

+++Réponse
Oui, vous pouvez connecter Power BI à Query Service. Consultez la documentation pour obtenir des [instructions sur la connexion de l’application de bureau Power BI au service de requête](./clients/power-bi.md).
+++

### Pourquoi le chargement des tableaux de bord est-il long lorsque le système est connecté au service de requête ?
...
Si vous souhaitez améliorer les temps de réponse de vos tableaux de bord, vous devez mettre en œuvre un serveur Business Intelligence (BI) en tant que couche de mise en cache entre le service de requête et les outils de BI. La plupart des outils de BI offrent une option de serveur dans leur assortiment.

L’ajout de la couche de serveur de mise en cache a pour but de mettre en cache les données du service de requête et d’utiliser les mêmes données pour les tableaux de bord, à des fins de temps de réponse réduits. Pour ce faire, les résultats des requêtes exécutées sont mis en cache sur le serveur BI quotidiennement. Le serveur de mise en cache fournit ensuite ces résultats à tout(e) utilisateur ou utilisatrice soumettant la même requête afin de réduire la latence. Pour plus d’informations sur cette mise en œuvre, consultez la documentation de l’utilitaire ou de l’outil tiers que vous utilisez.
+++

### Est-il possible d’accéder au service de requête à l’aide de l’outil de connexion pgAdmin ?

+++Réponse
Non, la connectivité pgAdmin n’est pas prise en charge. Consultez la [liste des clients tiers disponibles et les instructions pour les connecter au service de requête](./clients/overview.md) dans la documentation.
+++
...
| **42501** | Requête DROP TABLE | Déposer une table non créée par le service de requête | La table en train d’être déposée n’a pas été créée par le service de requête à l’aide de l’instruction `CREATE TABLE`. |
