...
La colonne d’horodatage se trouve dans [!DNL Experience Event] les jeux de données sont au format UTC. L’exemple utilise la méthode `from_utc_timestamp()` pour transformer l’horodatage UTC en EDT, puis utilise la variable `date_format()` pour isoler la date du reste de l’horodatage.
