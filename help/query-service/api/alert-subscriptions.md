...
| `quarantine` | Cette alerte est activée lorsqu’une exécution de requête planifiée est mise en quarantaine. Lorsque des requêtes sont inscrites dans la fonction de quarantaine, toute requête planifiée qui échoue dix exécutions consécutives est automatiquement placée dans un état [!UICONTROL Quarantined]. Elles nécessitent ensuite votre intervention avant que d’autres exécutions ne puissent avoir lieu. |
...
| --------- | ----------- |
| `orderby` | Champ spécifiant l’ordre des résultats. Les champs `created` et `updated` sont pris en charge. Ajoutez le nom de la propriété avec le `+` pour l’ordre croissant et le `-` pour l’ordre décroissant. La valeur par défaut est de `-created`. Notez que le signe plus (`+`) doit être placé dans une séquence d’échappement avec `%2B`. Par exemple, `%2Bcreated` est la valeur d’une commande créée ascendante. |
...
| `path` | Cette valeur concerne l’espace de noms dans le point d’entrée. Actuellement, la seule valeur acceptée est `/status`. |
