...
Adobe Experience Platform Query Service prend en charge les blocs anonymes. La fonction de bloc anonyme vous permet d’enchaîner une ou plusieurs instructions SQL exécutées en séquence. Les blocs anonymes permettent également de gérer les exceptions.
...
En lisant ce document, vous avez maintenant une compréhension claire des blocs anonymes et de leur structure. [Pour plus d’informations sur l’exécution des requêtes](../best-practices/writing-queries.md), veuillez lire le guide sur l’exécution de requêtes dans Query Service.
