---
title: Bloc anonyme dans le service de requête
...
# Bloc anonyme dans le service de requête

Le service de requête Adobe Experience Platform prend en charge les blocs anonymes. La fonction de bloc anonyme vous permet d’enchaîner une ou plusieurs instructions SQL exécutées de manière séquentielle. Les blocs anonymes permettent également de gérer les exceptions.
...
La requête suivante illustre un exemple de chaîne d’instructions SQL. Reportez-vous au document [Syntaxe SQL dans le service de requête](../sql/syntax.md) pour plus d’informations sur les syntaxes SQL utilisées.
...
L’ID d’instantané est stocké en tant que variable locale (`@current_sid`). Elle est ensuite utilisée dans la requête suivante pour renvoyer les résultats en fonction de l’INSTANTANÉ du même jeu de données/du même tableau. Pour plus d’[informations sur la clause d’instantané](../sql/syntax.md#SNAPSHOT-clause), consultez la documentation sur la syntaxe SQL.
...
Certains clients tiers peuvent avoir besoin d’un identifiant distinct avant et après un bloc SQL pour indiquer qu’une partie du script doit être traitée comme une seule instruction. Si vous recevez un message d’erreur lors de l’utilisation de Query Service avec un client tiers, vous devez vous reporter à la documentation du client tiers concernant l’utilisation d’un bloc SQL.

Par exemple, la mention **DbVisualizer** requiert que le délimiteur soit le seul texte de la ligne. Dans DbVisualizer, la valeur par défaut de l’identifiant de début est `--/` et celle de l’identifiant de fin est `/`. Vous trouverez ci-dessous un exemple de bloc anonyme dans DbVisualizer :
...
Pour DbVisualizer en particulier, il existe également une option « [!DNL Execute the complete buffer as one SQL statement] » dans l’interface utilisateur. Pour plus d’informations, consultez la [documentation de DbVisualizer](https://confluence.dbvis.com/display/UG120/Executing+Complex+Statements#ExecutingComplexStatements-UsingExecuteBuffer).

## Étapes suivantes

En lisant ce document, vous avez aquis une compréhension claire des blocs anonymes et de leur structure. Veuillez lire le [guide d’exécution des requêtes](../best-practices/writing-queries.md) pour plus d’informations sur l’écriture de requêtes.

Vous devez également en savoir plus sur [la manière dont les blocs anonymes sont utilisés avec le modèle de conception de chargement incrémentiel](./incremental-load.md) pour augmenter l’efficacité des requêtes.
