...
L’onglet **[!UICONTROL Accélérateurs]** de l’espace de travail Requêtes fournit un catalogue de modèles SQL paramétrés créés par Adobe pour des cas d’utilisation analytiques courants. Chaque accélérateur apparaît sous la forme d’une ligne dans une table avec un nom, un aperçu SQL et des métadonnées.

Sélectionnez un accélérateur pour l’ouvrir dans le Query Editor. Fournissez des valeurs de paramètre et exécutez la requête pour générer des résultats. Les accélérateurs sont en lecture seule et gérés par Adobe pour assurer la cohérence. Pour modifier la logique, créez une copie modifiable à l’aide de **[!UICONTROL Créer un modèle personnalisé]**. Consultez le guide [Accélérateurs de Distiller de données](./accelerators.md) pour savoir comment découvrir, exécuter, planifier et personnaliser des accélérateurs.

### Accélérateurs de Data Distiller recommandés {#recommended-accelerators}

La section **[!UICONTROL Accélérateurs de Distiller de données recommandés]** de l’onglet Aperçu permet d’accéder rapidement aux accélérateurs couramment utilisés. Ils apparaissent sous forme de cartes et prennent en charge deux workflows :
...
Sélectionnez une carte pour ouvrir l’accélérateur. Utilisez cette section pour accéder rapidement aux workflows courants ou accédez à l’onglet **[!UICONTROL Accélérateurs]** pour parcourir l’ensemble du catalogue. Pour obtenir la liste complète des accélérateurs et des instructions détaillées, consultez l’[onglet Accélérateurs](./accelerators.md#discovery-paths) ou le [guide des accélérateurs de Distiller de données](./accelerators.md).
...
| **[!UICONTROL Modifié par]** | Dernier utilisateur à avoir modifié la requête. Toute personne de votre organisation ayant accès au service de requête peut modifier les requêtes. |
...
Dans l’onglet **[!UICONTROL Admin]**, les administrateurs peuvent afficher les sessions actives dans les sandbox et mettre fin aux sessions inactives pour libérer la capacité partagée. Cette action n’interrompt pas les requêtes en cours d’exécution. Consultez le guide [Gérer les sessions Query Service](session-management.md) pour obtenir des instructions détaillées et connaître les exigences en matière d’autorisations.
