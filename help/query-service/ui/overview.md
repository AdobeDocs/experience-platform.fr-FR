...
| Colonne | Description |
| --- | --- |
| Nom | Nom de la requête créée par l’utilisateur. Vous pouvez sélectionner le nom d’ouverture de la requête dans le [!DNL Query Editor]. Vous pouvez également utiliser la barre de recherche pour effectuer une recherche sur le nom d’une requête. Les recherches sont sensibles à la casse. |
...
Maintenant que vous connaissez [!DNL Query Service] interface utilisateur de [!DNL Platform], vous pouvez accéder à [!DNL Query Editor] pour commencer à créer vos propres projets de requête à partager avec d’autres utilisateurs de votre entreprise. Pour plus d’informations sur la création et l’exécution de requêtes dans [!DNL Query Editor], reportez-vous à la section [Guide d’utilisation de Query Editor](./user-guide.md).