...
Lisez ce guide pour savoir comment exécuter, planifier et cloner des accélérateurs dans l’espace de travail [!UICONTROL Requêtes].

>[!AVAILABILITY]
>
>Les accélérateurs de Distiller de données ne sont disponibles que pour les organisations qui disposent d’un SKU Data Distiller. L’onglet [!UICONTROL Accélérateurs] et les workflows associés nécessitent le module complémentaire Distiller de données. Voir la [présentation de Data Distiller](../data-distiller/overview.md) ou contacter votre représentant Adobe pour plus d’informations.

## Conditions préalables {#prerequisites}

Avant de commencer, vérifiez que vous répondez aux exigences suivantes :

* Vous avez accès à l’espace de travail [!UICONTROL Requêtes] dans Experience Platform.
...
Pour commencer à utiliser des accélérateurs, accédez à l’espace de travail **[!UICONTROL Requêtes]** et ouvrez l’onglet **[!UICONTROL Accélérateurs]** ou l’onglet **[!UICONTROL Aperçu]**.
...
Utilisez ce chemin d’accès lorsque vous souhaitez parcourir tous les accélérateurs disponibles. Pour ouvrir le catalogue complet des accélérateurs, sélectionnez **[!UICONTROL Requêtes]** dans le volet de navigation de gauche, puis sélectionnez l’onglet **[!UICONTROL Accélérateurs]**.
...
>[!NOTE]
>
>Tous les accélérateurs sélectionnés dans l’onglet **[!UICONTROL Accélérateurs]** s’ouvrent dans le Query Editor.
...
Utilisez ce chemin d’accès lorsque vous souhaitez accéder rapidement à des accélérateurs fortement recommandés. Accédez à **[!UICONTROL Requêtes]**, puis sélectionnez l’onglet **[!UICONTROL Aperçu]**. Sélectionnez ensuite une vignette dans la section **[!UICONTROL Accélérateurs de Distiller de données recommandés]**.
...
>[!NOTE]
>
>Lorsque vous ouvrez un accélérateur dans le Query Editor, le code SQL est préchargé en lecture seule et les actions de la barre d’outils telles que [!UICONTROL Afficher les résultats], [!UICONTROL Annuler le texte] [!UICONTROL Formater le texte] sont désactivées.

Le panneau de droite affiche des métadonnées telles que **[!UICONTROL Identifiant de l’accélérateur]**, **[!UICONTROL Nom]** et les détails de modification, et donne accès à la planification via **[!UICONTROL Ajouter une planification]**.
...
Pour exécuter l’accélérateur, vous devez d’abord fournir des valeurs pour tous les paramètres requis. Les paramètres utilisent la syntaxe `${PARAMETER_NAME}` et apparaissent dans l&#39;onglet **[!UICONTROL Paramètres de requête]** sous l&#39;éditeur. Par exemple, `${START_DATE}` nécessite une valeur de date au format `YYYY-MM-DD` (par exemple, `2024-01-01`) et `${AUDIENCE_ID}` nécessite un identifiant d’audience spécifique.

Pour exécuter un accélérateur :

1. Sélectionnez **[!UICONTROL Paramètres de requête]** et saisissez une valeur pour chaque paramètre.
2. Sélectionnez l’icône de lecture (![Icône de lecture](../../images/icons/play.png)). dans la barre d’outils.

L’accélérateur s’exécute et affiche les résultats dans l’onglet **[!UICONTROL Résultats]**. Ces résultats ne sont pas conservés dans un jeu de données, sauf si vous utilisez **[!UICONTROL Exécuter en tant que CTAS]** ou planifiez l’accélérateur.
...
Pour créer un jeu de données à partir des résultats, sélectionnez **[!UICONTROL Enregistrer]** pour enregistrer l’accélérateur en tant que modèle, puis sélectionnez **[!UICONTROL Exécuter en tant que CTAS]**. La boîte de dialogue **[!UICONTROL Saisir les détails du jeu de données de sortie]** s’affiche. Saisissez un nom de jeu de données et une description facultative, puis confirmez pour créer le jeu de données. Cette action crée un jeu de données et y écrit les résultats.

![&#x200B; La boîte de dialogue [!UICONTROL Saisir les détails du jeu de données de sortie] avec un nom de jeu de données et une description renseignés.](../images/ui/accelerators/output-dataset-details-dialog.png)

## Planification d’un accélérateur {#schedule-accelerator}

Pour planifier l’exécution automatique d’un accélérateur avec des valeurs de paramètre fixes, sélectionnez **[!UICONTROL Ajouter une planification]** dans le panneau de droite.
...
Dans la boîte de dialogue de configuration du planning, vous devez fournir à nouveau une fréquence, un délai, un jeu de données de sortie et des valeurs de paramètre. Les valeurs de paramètre saisies dans le Query Editor ne sont pas transférées dans la configuration du planning. Dans la section **[!UICONTROL Détails du jeu de données]** vous pouvez choisir de **[!UICONTROL Ajouter dans un jeu de données existant]** ou **[!UICONTROL Créer et ajouter dans un nouveau jeu de données]**. Une fois le planning configuré, l’accélérateur s’exécute automatiquement en fonction de vos paramètres et écrit les résultats dans le jeu de données sélectionné.
...
Si vous devez modifier le code SQL ou réutiliser la logique sous votre propre configuration, vous pouvez créer un modèle personnalisé à partir d’un accélérateur. Tout d’abord, ouvrez un accélérateur dans le Query Editor, puis sélectionnez **[!UICONTROL Créer un modèle personnalisé]**. Modifiez le code SQL et les détails selon vos besoins, puis sélectionnez **[!UICONTROL Enregistrer]** ou **[!UICONTROL Enregistrer et fermer]** pour stocker le modèle.

Une fois enregistré, le modèle est modifiable et peut être exécuté, planifié ou utilisé avec CTAS. Le modèle est enregistré dans l’onglet **[!UICONTROL Modèles]**, où vous pouvez le gérer comme n’importe quel autre modèle. Pour plus d’informations, voir [Modèles de requête](./query-templates.md).

### Ce qui change lorsque vous créez un modèle personnalisé {#custom-template-differences}

Le modèle cloné diffère de l’accélérateur d’origine, car le code SQL est modifiable, vous pouvez enregistrer les modifications, supprimer le modèle et le planifier. Le champ **[!UICONTROL Modifié par]** affiche votre nom. Le modèle se trouve dans l’onglet **[!UICONTROL Modèles]** au lieu de **[!UICONTROL Accélérateurs]**.

## Accélérateurs liés au tableau de bord {#dashboard-accelerators}

Certains accélérateurs de l’onglet **[!UICONTROL Aperçu]** s’ouvrent sous forme de tableaux de bord au lieu de requêtes SQL. Ces accélérateurs fournissent des visualisations préconfigurées pour analyser les données d’audience et ne nécessitent pas de saisie de paramètre ni d’exécution manuelle.

Les accélérateurs suivants s’ouvrent dans l’espace de travail **[!UICONTROL Tableaux de bord]** :

**[!UICONTROL Chevauchements d’audience avancés]** analyse les intersections entre les audiences sélectionnées ou entre votre ensemble d’audiences complet afin d’identifier les modèles de chevauchement. Utilisez ces informations pour affiner la segmentation et réduire le ciblage redondant.

**[!UICONTROL Comparaison d’audiences]** compare les mesures clés entre deux audiences côte à côte, y compris la taille, la composition de l’identité et les modifications au fil du temps. Utilisez cette vue pour évaluer les différences de performance et éclairer les décisions de ciblage.

Le tableau **[!UICONTROL Tendances de l’audience]** suit l’évolution des mesures d’audience au fil du temps, y compris la taille de l’audience et le nombre d’identités. Utilisez ces tendances pour surveiller la croissance et évaluer l’impact des stratégies de segmentation.

**[!UICONTROL Chevauchements d’identités d’audience]** examine la manière dont les types d’identité se chevauchent dans les audiences sélectionnées pour comprendre les relations d’identité. Utilisez cette analyse pour améliorer la combinaison d’identités et la précision de la segmentation.
