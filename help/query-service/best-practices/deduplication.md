...
Ce document décrit comment effectuer la déduplication des données dans Query Service, ainsi que des exemples de déduplication des données. Pour plus d’informations sur les bonnes pratiques relatives à l’écriture de requêtes à l’aide de Query Service, veuillez lire la section [guide d’écriture de requêtes](./writing-queries.md).
