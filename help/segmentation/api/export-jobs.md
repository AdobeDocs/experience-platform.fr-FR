...
| -------- | ----------- |
| `destination` | Informations de destination pour les données exportées :<ul><li>`datasetId` : ID du jeu de données dans lequel les données ont été exportées.</li><li>`segmentPerBatch` : valeur booléenne qui indique si les identifiants de définition de segment sont consolidés ou non. Une valeur « false » signifie que tous les identifiants de définition de segment sont exportés dans un seul identifiant de lot. Une valeur « true » signifie qu’un identifiant de définition de segment est exporté dans un identifiant de lot. **Remarque :** la définition de la valeur sur true peut affecter les performances d’exportation par lots.</li></ul> |
...
| `filter.segments` | Définitions de segment exportées. Les champs suivants sont inclus :<ul><li>`segmentId` : identifiant de définition de segment vers lequel les profils seront exportés.</li><li>`segmentNs` : espace de noms de définition de segment pour la `segmentID` donnée.</li><li>`status` : tableau de chaînes fournissant un filtre de statut pour la `segmentID`. Par défaut, `status` aurez la valeur `["realized"]` qui représente tous les profils qui entrent dans la définition de segment à l’heure actuelle. Les valeurs possibles sont les suivantes : `realized` et `exited`. Une valeur `realized` signifie que le profil est qualifié pour la définition de segment. Une valeur `exiting` signifie que le profil quitte la définition de segment.</li></ul> |
...
| `mergePolicy` | Indique la politique de fusion pour gérer les données exportées. Incluez ce paramètre lorsqu’il existe plusieurs définitions de segment en cours d’exportation. Si elle n’est pas fournie, l’exportation utilise la même politique de fusion que la définition de segment donnée. |
| `filter` | Objet spécifiant les définitions de segment qui seront incluses dans la tâche d’exportation par identifiant, heure de qualification ou heure d’ingestion, selon les sous-propriétés répertoriées ci-dessous. Si rien n’est indiqué, toutes les données seront exportées. |
| `filter.segments` | Spécifie les définitions de segment à exporter. Si vous omettez cette valeur, toutes les données de l’ensemble des profils seront exportées. Accepte un tableau d’objets de segment, chacun contenant les champs suivants :<ul><li>`segmentId` : **(obligatoire en cas d’utilisation de `segments`)** identifiant du segment pour les profils à exporter.</li><li>`segmentNs` : *(facultatif)* espace de noms du segment pour le `segmentID` donné.</li><li>`status` : *(facultatif)* tableau de chaînes fournissant un filtre d’état pour le `segmentID`. Par défaut, `status` aurez la valeur `["realized"]` qui représente tous les profils qui entrent dans la définition de segment à l’heure actuelle. Les valeurs possibles sont les suivantes : `realized` et `exited`.  Une valeur `realized` signifie que le profil est qualifié pour la définition de segment. Une valeur `exiting` signifie que le profil quitte la définition de segment.</li></ul> |
| `filter.segmentQualificationTime` | Filtre basé sur la durée de qualification pour la définition de segment. L’heure de début et/ou l’heure de fin peuvent être fournies. |
| `filter.segmentQualificationTime.startTime` | Heure de début de qualification pour un identifiant de définition de segment pour un statut donné. Si elle n’est pas fournie, il n’y aura aucun filtre sur l’heure de début pour une qualification d’identifiant de définition de segment. La date et l’heure doivent être fournies au format [ISO 8601](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6). |
| `filter.segmentQualificationTime.endTime` | Heure de fin de qualification du segment pour un identifiant de définition de segment pour un statut donné. Si elle n’est pas fournie, il n’y aura aucun filtre sur l’heure de fin pour une qualification d’identifiant de définition de segment. La date et l’heure doivent être fournies au format [ISO 8601](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6). |
| `filter.fromIngestTimestamp` | Limite les profils exportés à inclure uniquement ceux qui ont été mis à jour après cet horodatage. La date et l’heure doivent être fournies au format [ISO 8601](https://datatracker.ietf.org/doc/html/rfc3339#section-5.6). <ul><li>`fromIngestTimestamp` pour les **profils**, le cas échéant : inclut tous les profils fusionnés dans lesquels la date et l’heure mises à jour et fusionnées sont supérieures à la date et l’heure données. Prend en charge l’opérande `greater_than`.</li><li>`fromIngestTimestamp` pour les **événements** : tous les événements ingérés après cette date et cette heure seront exportés en fonction du résultat du profil obtenu. Il ne s’agit pas de l’heure de l’événement, mais de l’heure de l’ingestion des événements.</li> |
...
| `fields` | Une liste des champs exportés, séparés par des virgules. |
| `schema.name` | Nom du schéma associé au jeu de données dans lequel les données doivent être exportées. |
| `filter.segments` | Définitions de segment exportées. Les champs suivants sont inclus :<ul><li>`segmentId` : identifiant de définition de segment pour les profils à exporter.</li><li>`segmentNs` : espace de noms de définition de segment pour la `segmentID` donnée.</li><li>`status` : tableau de chaînes fournissant un filtre de statut pour la `segmentID`. Par défaut, `status` aurez la valeur `["realized"]` qui représente tous les profils qui entrent dans la définition de segment à l’heure actuelle. Les valeurs possibles sont les suivantes : `realized` et `exited`.  Une valeur `realized` signifie que le profil est qualifié pour la définition de segment. Une valeur `exiting` signifie que le profil quitte la définition de segment.</li></ul> |
| `mergePolicy` | Informations sur la politique de fusion pour les données exportées. |
| `metrics.totalTime` | Champ indiquant le temps total d’exécution de la tâche d’exportation. |
| `metrics.profileExportTime` | Champ indiquant le temps nécessaire aux profils pour l’exportation. |
