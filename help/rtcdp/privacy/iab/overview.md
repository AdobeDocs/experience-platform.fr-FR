...
description: Découvrez comment configurer vos opérations de données et vos schémas pour transmettre les choix de consentement des utilisateurs lors de l’exportation de segments dans la plateforme de données clientes en temps réel.
...
Ce document présente une vue d&#39;ensemble de la configuration de vos opérations de données et de vos schémas de profil pour accepter les données de consentement des clients générées par votre CMP, ainsi que la manière dont [!DNL Real-time CDP] transmet les choix de consentement des utilisateurs lors de l&#39;exportation de segments.
...
>[!IMPORTANT]
>
>Si l&#39;ID de votre CMP n&#39;est pas valide, [!DNL Real-time CDP] continuera à traiter vos données en l&#39;état. Pour appliquer TCF 2.0, vous devez confirmer que votre CMP possède un ID valide qui a été enregistré auprès d&#39;IAB TCF 2.0 avant d&#39;envoyer les données à [!DNL Experience Platform].
...
Outre les services [!DNL Platform] énumérés ci-dessus, vous devez également connaître [destinations](../../destinations/overview.md) et leur utilisation dans [!DNL Real-time CDP].
...
1. À l&#39;aide de [!DNL Experience Platform Web SDK], les données de consentement générées (renvoyées par le CMP) sont envoyées à Adobe Experience Platform.
...
Outre les commandes SDK déclenchées par les crochets de modification du consentement de CMP, les données de consentement peuvent également être transférées dans [!DNL Experience Platform] par l’intermédiaire de toute donnée XDM générée par le client et directement transférées dans un jeu de données compatible [!DNL Profile].

Tout segment partagé avec [!DNL Platform] par Adobe Audience Manager (par l&#39;intermédiaire du [!DNL Audience Manager] connecteur source ou autre) peut également contenir des données de consentement, à condition que les champs appropriés aient été appliqués à ces segments par l&#39;intermédiaire de [!DNL Experience Cloud Identity Service]. Pour plus d&#39;informations sur la collecte de données de consentement dans [!DNL Audience Manager], consultez le document du module externe [Adobe Audience Manager pour IAB TCF](https://experienceleague.adobe.com/docs/audience-manager/user-guide/overview/data-privacy/consent-management/aam-iab-plugin.html).

### Application du consentement en aval

Une fois que les données de consentement IAB ont été correctement ingérées, les processus suivants ont lieu dans les services [!DNL Real-time CDP] en aval :
...
1. Lors de l’exportation de segments vers des destinations appartenant à des membres de la liste fournisseur TCF 2.0, [!DNL Real-time CDP] ne comprend que les profils si les autorisations du fournisseur pour [!DNL Real-time CDP] (565) *et* sont fournies pour chaque ID d’une grappe.

Le reste des sections de ce document explique comment configurer [!DNL Real-time CDP] et vos opérations de données pour répondre aux exigences de collecte et d&#39;application décrites ci-dessus.
...
| --- | --- |
| **Objectifs** | Les fins permettent de définir les objectifs technologiques publicitaires pour lesquels une marque peut utiliser les données d’un client. Pour que [!DNL Real-time CDP] puisse traiter les ID de client, il faut opter pour les objectifs suivants : <ul><li>**Objectif 1** : Stocker et/ou accéder aux informations sur un périphérique</li><li>**But 10** : Développer et améliorer les produits</li></ul> |
...
## Créer des jeux de données avec les champs de consentement IAB {#datasets}

Les données de consentement du client doivent être envoyées aux jeux de données dont les schémas contiennent des champs de consentement IAB. Reportez-vous au didacticiel sur la [création de jeux de données pour la capture du consentement TCF 2.0](./dataset-preparation.md) pour savoir comment créer les deux jeux de données requis avant de poursuivre avec ce guide.
...
Pour plus d&#39;informations sur la façon d&#39;utiliser les stratégies de fusion, consultez le [guide de l&#39;utilisateur des stratégies de fusion](../../../profile/ui/merge-policies.md). Lors de la configuration de vos stratégies de fusion, vous devez vous assurer que vos segments incluent tous les attributs de consentement requis fournis par le [mixin de confidentialité XDM](./dataset-preparation.md#privacy-mixin), comme indiqué dans le guide de préparation des jeux de données.

## Intégrer le [!DNL Experience Platform] SDK Web pour collecter les données de consentement des clients {#sdk}

>[!NOTE]
>
>L&#39;utilisation du [!DNL Experience Platform] SDK Web est nécessaire pour traiter directement les données de consentement dans Adobe Experience Platform. [!DNL Experience Cloud Identity Service] n’est actuellement pas prise en charge.
>
...
Pour que le SDK envoie des données à [!DNL Experience Platform], vous devez d&#39;abord créer une nouvelle configuration de bord pour [!DNL Platform] dans [!DNL Adobe Experience Platform Launch]. Des étapes spécifiques pour créer une nouvelle configuration sont fournies dans la [documentation du SDK](../../../edge/fundamentals/edge-configuration.md).
...
| --- | --- |
| [!UICONTROL Environnement de test] | Nom du [!DNL Platform] [sandbox](../../../sandboxes/home.md) qui contient la connexion de diffusion en continu et les jeux de données nécessaires pour configurer la configuration du bord. |
| [!UICONTROL Entrée de diffusion en continu] | Connexion de flux continu valide pour [!DNL Experience Platform]. Consultez le didacticiel sur [la création d&#39;une connexion de diffusion](../../../ingestion/tutorials/create-streaming-connection-ui.md) si vous n&#39;avez pas d&#39;entrée de diffusion existante. |
...
>[!NOTE]
>
>Pour une introduction à la syntaxe commune pour toutes les commandes [!DNL Platform] SDK, voir le document sur [l&#39;exécution des commandes](../../../edge/fundamentals/executing-commands.md).
...
Vous pouvez également collecter des données de consentement TCF 2.0 sur chaque événement déclenché dans [!DNL Platform] en utilisant la commande `sendEvent`.

>[!NOTE]
>
>Pour utiliser cette méthode, vous devez avoir ajouté [!DNL Experience Event Privacy mixin] à votre schéma [!DNL Profile] [!DNL XDM ExperienceEvent] activé. Consultez la section [mise à jour du schéma ExperienceEvent](./dataset-preparation.md#event-schema) dans le guide de préparation des jeux de données pour connaître les étapes de configuration de ce paramètre.
...
TCF 2.0 exige également que la source de données vérifie l&#39;autorisation du fournisseur de la destination avant d&#39;envoyer les données vers cette destination. Par conséquent, [!DNL Real-time CDP] vérifie si l’autorisation du fournisseur de la destination est activée pour tous les ID de la grappe avant d’inclure les données liées à cette destination.

>[!NOTE]
>
>Tous les segments partagés avec Adobe Audience Manager contiendront les mêmes valeurs de consentement TCF 2.0 que leurs homologues [!DNL Platform]. Puisque [!DNL Audience Manager] partage le même ID de fournisseur que [!DNL Real-time CDP] (565), les mêmes fins et les mêmes autorisations de fournisseur sont requises. Pour plus d’informations, voir le document du [module externe Adobe Audience Manager pour IAB TCF](https://experienceleague.adobe.com/docs/audience-manager/user-guide/overview/data-privacy/consent-management/aam-iab-plugin.html).
...
Ce document portait sur le processus de configuration de vos opérations de données dans [!DNL Real-time CDP] pour être conforme à TCF 2.0. Pour plus d&#39;informations sur les autres fonctionnalités de confidentialité fournies par [!DNL Real-time CDP], consultez la documentation suivante :
