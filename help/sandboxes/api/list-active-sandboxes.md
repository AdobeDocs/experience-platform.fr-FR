...
title: ' des sandbox actifs pour l’utilisateur actuel'
...
#  des sandbox actifs pour l’utilisateur actuel

>[!NOTE] Contrairement aux autres points de fin fournis dans l’API Sandbox, ce point de fin est disponible pour tous les utilisateurs, y compris ceux qui n’ont pas d’autorisations d’accès à l’administration Sandbox.

Vous pouvez  les sandbox actifs pour l’utilisateur actuel en envoyant une requête GET au point de terminaison racine (`/`).
...
Une réponse réussie renvoie un  de sandbox actifs pour l’utilisateur actuel, y compris des détails tels que `name`, `title`, `state`et `type`.
