...
| --- | --- | --- |
| Plateforme de données client | Sources | <ul><li>Pour des raisons de sécurité, les informations d’identification du compte source ne sont pas répliquées dans le sandbox cible et devront être mises à jour manuellement.</li><li>Par défaut, le flux de données source est copié dans un statut de brouillon.</li></ul> **REMARQUE :** actuellement, l’outil Sandbox ne prend en charge que les flux de données source par lots. Les flux de données source en flux continu ne sont pas pris en charge. |
| Plateforme de données client | Audiences | <ul><li>Seul le type de **[!UICONTROL Customer Audience]** **[!UICONTROL Segmentation service]** est pris en charge.</li><li>Les libellés existants pour le consentement et la gouvernance seront copiés dans la même tâche d’importation.</li><li> Le système sélectionne automatiquement la politique de fusion par défaut dans le sandbox cible avec la même classe XDM lors de la vérification des dépendances de la politique de fusion.</li><li>Si un objet existant portant le même nom est détecté lors de l’importation d’audiences, l’outil Sandbox réutilisera toujours l’objet existant, afin d’éviter la prolifération d’objets.</li></ul> |
...
| [!DNL Adobe Journey Optimizer] | Parcours | Les objets suivants utilisés dans le parcours sont copiés en tant qu’objets dépendants. Au cours du workflow d’importation, vous pouvez choisir **[!UICONTROL Create new]** ou **[!UICONTROL Use existing]** pour chacun des éléments suivants : <ul><li>Audiences</li><li>Détails de la zone de travail</li><li>Modèles de contenu</li><li>Actions personnalisées</li><li>Sources de données</li><li>Événements</li><li>Groupes de champs</li><li>Fragments</li><li>Schémas</li></ul> | Lorsque vous sélectionnez **[!UICONTROL Use existing]** au cours du processus d’importation pour copier un parcours dans un autre sandbox, les actions personnalisées existantes que vous choisissez **doivent** doivent correspondre exactement à l’action personnalisée source. S’ils ne correspondent pas, le nouveau parcours génère des erreurs impossibles à résoudre.<br>Le système copie les événements et les détails des événements utilisés dans le parcours et crée une version dans le sandbox cible. |
...
Sélectionnez **[!UICONTROL Schemas]** dans le volet de navigation de gauche, puis sélectionnez l’onglet **[!UICONTROL Browse]** qui répertorie les schémas disponibles. Sélectionnez ensuite les points de suspension (`...`) à côté du schéma sélectionné pour afficher les contrôles dans une liste déroulante. Sélectionnez **[!UICONTROL Add to package]** dans la liste déroulante.

![Liste des schémas affichant le menu déroulant mettant en surbrillance le contrôle [!UICONTROL Add to package].](../images/ui/sandbox-tooling/add-to-package.png)

Dans la boîte de dialogue **[!UICONTROL Add to package]**, sélectionnez l’option **[!UICONTROL Create new package]** . Fournissez un [!UICONTROL Name] pour votre package et une [!UICONTROL Description] facultative, puis sélectionnez **[!UICONTROL Add]**.

![Boîte de dialogue [!UICONTROL Add to package] avec [!UICONTROL Create new package] sélectionné et le [!UICONTROL Add] en surbrillance.](../images/ui/sandbox-tooling/create-new-package.png)

Vous revenez alors à l’environnement **[!UICONTROL Schemas]**. Vous pouvez maintenant ajouter des objets supplémentaires au package que vous avez créé en suivant les étapes suivantes répertoriées ci-dessous.

### Ajouter un objet à un package existant et le publier {#add-object-to-existing-package}

Pour afficher la liste des schémas disponibles, sélectionnez **[!UICONTROL Schemas]** dans le volet de navigation de gauche, puis sélectionnez l’onglet **[!UICONTROL Browse]** . Sélectionnez ensuite les points de suspension (`...`) à côté du schéma sélectionné pour afficher les options de contrôle dans un menu déroulant. Sélectionnez **[!UICONTROL Add to package]** dans la liste déroulante.

![Liste des schémas affichant le menu déroulant mettant en surbrillance le contrôle [!UICONTROL Add to package].](../images/ui/sandbox-tooling/add-to-package.png)

La boîte de dialogue **[!UICONTROL Add to package]** s’affiche. Sélectionnez l’option **[!UICONTROL Existing package]** , puis sélectionnez la liste déroulante **[!UICONTROL Package name]** et sélectionnez le package requis. Enfin, sélectionnez **[!UICONTROL Add]** pour confirmer vos choix.

![[!UICONTROL Add to package] boîte de dialogue, affichant un package sélectionné dans la liste déroulante.](../images/ui/sandbox-tooling/add-to-existing-package.png)

La liste des objets ajoutés au package est répertoriée. Pour publier le package et le rendre disponible pour l’importation dans des sandbox, sélectionnez **[!UICONTROL Publish]**.

![Liste d’objets dans le package, mettant en surbrillance l’option [!UICONTROL Publish].](../images/ui/sandbox-tooling/publish-package.png)

Sélectionnez **[!UICONTROL Publish]** pour confirmer la publication du package.

![Boîte de dialogue de confirmation de publication du package, mettant en surbrillance l’option [!UICONTROL Publish].](../images/ui/sandbox-tooling/publish-package-confirmation.png)
...
Vous revenez alors à l’onglet **[!UICONTROL Packages]** dans l’environnement de [!UICONTROL Sandboxes], où vous pouvez voir le nouveau package publié.
...
Pour importer le package dans un sandbox cible, accédez à l’onglet **[!UICONTROL Browse]** Sandbox et sélectionnez l’option plus (+) à côté du nom du sandbox.

![Onglet **[!UICONTROL Browse]** des sandbox mettant en surbrillance la sélection du package d’importation.](../images/ui/sandbox-tooling/browse-sandboxes.png)

Dans le menu déroulant, sélectionnez le **[!UICONTROL Package name]** à importer dans le sandbox ciblé. Ajoutez un **[!UICONTROL Job name]**, qui sera utilisé pour la surveillance ultérieure. Par défaut, le profil unifié est désactivé lorsque les schémas du package sont importés. Activez le bouton (bascule) **Activer les schémas pour le profil** pour l’activer, puis sélectionnez **[!UICONTROL Next]**.

![Page de détails de l’importation affichant la sélection de la liste déroulante [!UICONTROL Package name]](../images/ui/sandbox-tooling/import-package-to-sandbox.png)

La page [!UICONTROL Package object and dependencies] fournit une liste de toutes les ressources incluses dans ce package. Le système détecte automatiquement les objets dépendants requis pour réussir l&#39;importation des objets parents sélectionnés. Tous les attributs manquants s’affichent en haut de la page. Sélectionnez **[!UICONTROL View details]** pour obtenir une répartition plus détaillée.

![La page [!UICONTROL Package object and dependencies] affiche les attributs manquants.](../images/ui/sandbox-tooling/missing-attributes.png)
...
>
>L’outil Sandbox ne prend actuellement pas en charge la mise à jour ou le remplacement d’objets existants. Vous pouvez choisir de créer un nouvel objet ou de continuer à utiliser l&#39;objet existant sans modification. Si un objet existant portant le même nom est détecté, l’outil Sandbox réutilisera toujours l’objet existant, même si vous sélectionnez l’option [!UICONTROL Create new] pour éviter la prolifération d’objets.

Pour utiliser un objet existant, sélectionnez l’icône en forme de crayon en regard de l’objet dépendant.

![La page [!UICONTROL Package object and dependencies] affiche une liste des ressources incluses dans le package.](../images/ui/sandbox-tooling/package-objects-and-dependencies.png)

Les options permettant d’en créer de nouvelles ou d’utiliser des existantes s’affichent. Sélectionnez **[!UICONTROL Use existing]**.

![Page [!UICONTROL Package object and dependencies] affichant les options d’objet dépendantes [!UICONTROL Create new] et [!UICONTROL Use existing].](../images/ui/sandbox-tooling/use-existing-object.png)

La boîte de dialogue **[!UICONTROL Field group]** affiche une liste des groupes de champs disponibles pour l’objet . Sélectionnez les groupes de champs requis, puis sélectionnez **[!UICONTROL Save]**.

![Liste de champs affichée dans la boîte de dialogue de [!UICONTROL Field group], mettant en surbrillance la [!UICONTROL Save] sélection.](../images/ui/sandbox-tooling/field-group-list.png)

Vous revenez à la page [!UICONTROL Package object and dependencies]. À partir de là, sélectionnez **[!UICONTROL Finish]** pour terminer l’importation du package.

![La page [!UICONTROL Package object and dependencies] affiche une liste des ressources incluses dans le package, en mettant en surbrillance [!UICONTROL Finish].](../images/ui/sandbox-tooling/finish-object-dependencies.png)
...
Pour exporter un sandbox entier, accédez à l’onglet [!UICONTROL Sandboxes] **[!UICONTROL Packages]** et sélectionnez **[!UICONTROL Create package]**.

![Onglet [!UICONTROL Sandboxes] **[!UICONTROL Packages]** mettant en surbrillance [!UICONTROL Create package].](../images/ui/sandbox-tooling/create-sandbox-package.png)

Sélectionnez **[!UICONTROL Entire sandbox]** pour le [!UICONTROL Type of package] dans la boîte de dialogue [!UICONTROL Create package]. Fournissez un [!UICONTROL Package name] pour votre nouveau package et sélectionnez le **[!UICONTROL Sandbox]** dans la liste déroulante. Enfin, sélectionnez **[!UICONTROL Create]** pour confirmer vos entrées.

![Boîte de dialogue [!UICONTROL Create package] affichant les champs remplis et mettant en surbrillance les [!UICONTROL Create].](../images/ui/sandbox-tooling/create-package-dialog.png)

Le package a été créé avec succès. Sélectionnez **[!UICONTROL Publish]** pour publier le package.
...
Pour importer le package dans un sandbox cible, accédez à l’onglet [!UICONTROL Sandboxes] **[!UICONTROL Browse]** et sélectionnez l’option plus (+) à côté du nom du sandbox.

![Onglet **[!UICONTROL Browse]** des sandbox mettant en surbrillance la sélection du package d’importation.](../images/ui/sandbox-tooling/browse-entire-package-sandboxes.png)

Dans le menu déroulant, sélectionnez le sandbox complet à l’aide de la liste déroulante **[!UICONTROL Package name]** . Ajoutez une **[!UICONTROL Job name]**, qui sera utilisée pour la surveillance future et une **[!UICONTROL Job description]** facultative, puis sélectionnez **[!UICONTROL Next]**.

![Page de détails de l’importation affichant la sélection de la liste déroulante [!UICONTROL Package name]](../images/ui/sandbox-tooling/import-full-sandbox-package.png)
...
Vous accédez à la page [!UICONTROL Package object and dependencies] qui indique le nombre d’objets et de dépendances importés et exclus. À partir de là, sélectionnez **[!UICONTROL Import]** pour terminer l’importation du package.

![La page [!UICONTROL Package object and dependencies] affiche le message intégré des types d’objet non pris en charge, en mettant en surbrillance [!UICONTROL Import].](../images/ui/sandbox-tooling/finish-dependencies-entire-sandbox.png)

Patientez quelques instants le temps que l’importation se termine. Le délai d’exécution peut varier en fonction du nombre d’objets dans le package. Vous pouvez surveiller la tâche d’importation à partir de l’onglet [!UICONTROL Sandboxes] **[!UICONTROL Jobs]** .

### Copie express d’un objet dans un sandbox {#express-copy}

>[!IMPORTANT]
>
>La fonctionnalité de copie express est actuellement en version bêta et disponible uniquement pour certains clients. Actuellement, Express Copy (Beta) ne prend en charge que les schémas et les flux de données sources.

Vous pouvez accéder à la copie express à partir de la page d&#39;inventaire des objets. Par exemple, pour afficher la liste des schémas disponibles, sélectionnez **[!UICONTROL Schemas]** dans le volet de navigation de gauche, puis sélectionnez l’onglet **[!UICONTROL Browse]** . Sélectionnez ensuite les points de suspension (`...`) à côté du schéma sélectionné pour afficher les options de contrôle dans un menu déroulant. Sélectionnez **[!UICONTROL Add to package]** dans la liste déroulante.

![Liste des schémas affichant le menu déroulant mettant en surbrillance le contrôle [!UICONTROL Add to package].](../images/ui/sandbox-tooling/add-to-package-express.png)

La boîte de dialogue **[!UICONTROL Add to package]** s’affiche. Sélectionnez l’option **[!UICONTROL Express copy]** , puis sélectionnez le **[!UICONTROL Target sandbox]** dans la liste déroulante. Enfin, sélectionnez **[!UICONTROL Add]** pour confirmer vos choix.

![[!UICONTROL Add to package] boîte de dialogue, affichant un package sélectionné dans la liste déroulante.](../images/ui/sandbox-tooling/express-copy.png)

>[!NOTE]
>
> Express Copy regroupe automatiquement les objets sélectionnés avec leurs dépendances requises et les déploie sur le sandbox cible. Si un objet dépendant existe déjà dans le sandbox cible, il est réutilisé ; dans le cas contraire, un nouvel objet est créé.

Pour vérifier le statut de votre demande de copie express, sélectionnez **[!UICONTROL Sandboxes]** dans le volet de navigation de gauche, puis sélectionnez l’onglet **[!UICONTROL Jobs]** . Une liste de toutes les tâches et le statut actuel du traitement s’affiche.

![Onglet Tâches affichant une liste des tâches.](../images/ui/sandbox-tooling/sandboxes-jobs.png)

## Surveiller les détails de l’importation {#view-import-details}

Pour afficher les détails importés, accédez à l’onglet [!UICONTROL Sandboxes] **[!UICONTROL Jobs]** et sélectionnez le package dans la liste. Vous pouvez également utiliser la barre de recherche pour rechercher le package.

![L’onglet [!UICONTROL Jobs] des sandbox met en surbrillance la sélection du package d’importation.](../images/ui/sandbox-tooling/imports-tab.png)
...
Sélectionnez **[!UICONTROL View import summary]** dans le volet de détails droit de l’onglet **[!UICONTROL Jobs]** de l’environnement Sandbox .

![L’onglet [!UICONTROL Imports] des sandbox met en surbrillance la sélection des [!UICONTROL View import details] dans le volet de droite.](../images/ui/sandbox-tooling/view-import-details.png)

La boîte de dialogue **[!UICONTROL Import summary]** affiche une répartition des imports avec une progression en pourcentage.

>[!NOTE]
>
>Vous pouvez afficher une liste d’objets en accédant à des pages d’inventaire spécifiques.

![Boîte de dialogue [!UICONTROL Import details] affichant une répartition détaillée des imports.](../images/ui/sandbox-tooling/import-details.png)
...
Tout d’abord, mettez à jour l’objet dans votre sandbox source. Par exemple, accédez à l’espace de travail **[!UICONTROL Schemas]**, sélectionnez votre schéma et ajoutez un nouveau groupe de champs.

![L’espace de travail des schémas avec un schéma mis à jour.](../images/ui/sandbox-tooling/update-schema.png)

Une fois que vous avez mis à jour votre schéma, accédez à **[!UICONTROL Sandboxes]**, sélectionnez **[!UICONTROL Packages]**, puis localisez votre package existant.

![Interface d’outil Sandbox avec un package sélectionné](../images/ui/sandbox-tooling/select-package.png)

Utilisez l’interface des packages pour vérifier vos modifications. Sélectionnez **[!UICONTROL Check for updates]** pour afficher les modifications apportées aux artefacts de votre package. Sélectionnez ensuite **[!UICONTROL View diff]** pour recevoir un résumé détaillé de toutes les modifications apportées à vos artefacts.

![Interface du package avec le bouton Afficher la différence sélectionné.](../images/ui/sandbox-tooling/view-diff.png)

L’interface [!UICONTROL View diff] s’affiche. Reportez-vous à cet outil pour plus d&#39;informations sur vos artefacts source et cible, ainsi que sur les modifications à leur appliquer.

![Résumé des modifications.](../images/ui/sandbox-tooling/summary-of-changes.png)

Au cours de cette étape, vous pouvez également sélectionner [!UICONTROL Summarize with AI] pour obtenir un résumé détaillé de toutes les modifications.

![Résumé avec activé par l’IA.](../images/ui/sandbox-tooling/ai-summary.png)

Une fois prêt, sélectionnez **[!UICONTROL Update package]** puis **[!UICONTROL Confirm]** dans la fenêtre pop-up qui s’affiche. Une fois la tâche terminée, vous pouvez actualiser la page et sélectionner **[!UICONTROL View history]** pour vérifier la version de votre package.

![La fenêtre de confirmation.](../images/ui/sandbox-tooling/confirm-changes.png)

Pour importer vos modifications, revenez au répertoire [!UICONTROL Packages] et sélectionnez les points de suspension (`...`) à côté de votre package, puis sélectionnez **[!UICONTROL Import package]**. Experience Platform sélectionne automatiquement les [!UICONTROL Update existing objects]. Vérifiez les modifications, puis sélectionnez **[!UICONTROL Finish]**.
...
Importez ensuite votre package dans le sandbox cible qui contient les objets que vous souhaitez également mettre à jour. Au cours du processus d’importation, sélectionnez **[!UICONTROL Update existing objects]** puis utilisez le navigateur d’objets pour sélectionner manuellement les objets cibles auxquels vous souhaitez appliquer vos mises à jour.
...
Une fois que vous avez identifié les objets cibles à mettre à jour, sélectionnez **[!UICONTROL Finish]**.
