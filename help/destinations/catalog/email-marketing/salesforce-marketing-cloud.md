...
Pour envoyer des données d’audience à [!DNL Salesforce Marketing Cloud], vous devez d’abord [vous connecter à la destination](#connect-destination) dans Platform, puis [ configurer une importation de données](#import-data-into-salesforce) à partir de l’emplacement de stockage dans [!DNL Salesforce Marketing Cloud].
...
* Pour les connexions **[!UICONTROL SFTP avec mot de passe]**, vous devez fournir :
   * **[!UICONTROL Domaine]** : adresse IP ou nom de domaine de votre compte SFTP ;
...
## Configuration de l’importation des données dans [!DNL Salesforce Marketing Cloud] {#import-data-into-salesforce}
