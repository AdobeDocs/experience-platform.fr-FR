...
Lors de l’activation des audiences vers la destination [!DNL (API) Salesforce Marketing Cloud], vous devez saisir une valeur dans le champ **[!UICONTROL Mapping ID]** pour chaque audience activée, à l’étape **[Planning des audiences](#schedule-segment-export-example)**.
...
Elles s’affichent dans la fenêtre de sélection des **[!UICONTROL Target field]** lorsque vous configurez le [mapping](#mapping-considerations-example) dans le workflow pour [activer les audiences vers la destination](#activate).

>[!IMPORTANT]
>
> Dans [!DNL Salesforce Marketing Cloud], vous devez créer des attributs avec un **[!UICONTROL FIELD NAME]** qui correspond exactement à la valeur spécifiée dans **[!UICONTROL Mapping ID]** pour chaque segment Experience Platform activé. Par exemple, la capture d’écran ci-dessous montre un attribut nommé `salesforce_mc_segment_1`. Lors de l’activation d’une audience vers cette destination, ajoutez `salesforce_mc_segment_1` as **[!UICONTROL Mapping ID]** pour renseigner les audiences d’Experience Platform dans cet attribut.
...
* Le nombre de *champs personnalisés autorisés par objet* varie en fonction de votre édition Salesforce.
   ...
   * Si vous avez atteint la limite définie pour *champs personnalisés autorisés par objet* dans [!DNL Salesforce Marketing Cloud], vous devrez :
      ...
      * Mettez à jour ou supprimez toutes les audiences activées dans les destinations Experience Platform qui utilisent ces anciens noms d’attribut comme valeur fournie pour **[!UICONTROL Mapping ID]** lors de l’étape [planification des audiences](#schedule-segment-export-example).
...
| Élément | Type | Notes |
|---------|----------|---------|
| Type d’exportation | **[!UICONTROL Profile-based]** | <ul><li>Vous exportez tous les membres d’un segment, ainsi que les champs de schéma souhaités, *(par exemple : adresse e-mail, numéro de téléphone, nom)*, en fonction de votre mappage de champs.</li><li> Chaque statut de segment dans [!DNL Salesforce Marketing Cloud] est mis à jour avec le statut d’audience correspondant d’Experience Platform, en fonction de la valeur **[!UICONTROL Mapping ID]** fournie lors de l’étape [planification de l’audience](#schedule-segment-export-example).</li></ul> |
...
Dans **[!UICONTROL Destinations]** > **[!UICONTROL Catalog]**, recherchez [!DNL (API) Salesforce Marketing Cloud]. Vous pouvez également localiser cet élément dans la catégorie **[!UICONTROL Email marketing]** .

### S’authentifier auprès de la destination {#authenticate}

Pour vous authentifier auprès de la destination, renseignez les champs obligatoires ci-dessous et sélectionnez **[!UICONTROL Connect to destination]**. Reportez-vous à la section [Collecter [!DNL Salesforce Marketing Cloud] informations d’identification](#gather-credentials) pour obtenir des conseils.
...
>
> * L’intégration à l’API [!DNL Salesforce Marketing Cloud] est soumise à une limite de pagination du nombre d’attributs qu’Experience Platform peut récupérer de Salesforce. Cela signifie que pendant l’étape de **[!UICONTROL Mapping]**, le schéma du champ cible peut afficher un maximum de 2 000 attributs de votre compte Salesforce.
...
1. Dans la fenêtre de **[!UICONTROL Select target field]**, choisissez le **[!UICONTROL Select identity namespace]** et sélectionnez une identité ou choisissez **[!UICONTROL Select attributes]** catégorie et sélectionnez un attribut parmi les extensions de données affichées, le cas échéant. La destination [!DNL (API) Salesforce Marketing Cloud] utilise la [!DNL Salesforce Marketing Cloud] [!DNL Search Attribute-Set Definitions REST] [API](https://developer.salesforce.com/docs/marketing/marketing-cloud/guide/retrieveAttributeSetDefinitions.html) pour récupérer dynamiquement les extensions de données et leurs attributs liés&#39; définis dans [!DNL Salesforce Marketing Cloud]. Ils s’affichent dans la fenêtre contextuelle **[!UICONTROL Target field]** lorsque vous configurez le [mappage](#mapping-considerations-example) dans le workflow [activer les audiences](#activate).
...
| [!DNL (API) Salesforce Marketing Cloud] le nom du segment | [!DNL Salesforce Marketing Cloud] **[!UICONTROL FIELD NAME]** | [!DNL (API) Salesforce Marketing Cloud] **[!UICONTROL Mapping ID]** |
| --- | --- | --- |
...
1. Accédez au site web [[!DNL Salesforce Marketing Cloud]](https://mc.exacttarget.com/) . Accédez ensuite à la page **[!DNL Audience Builder]** > **[!DNL Contact Builder]** > **[!DNL All contacts]** > **[!DNL Email]** et vérifiez si les profils de l’audience ont été ajoutés.
