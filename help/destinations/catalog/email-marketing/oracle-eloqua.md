...
Pour envoyer des données d’audience à [!DNL Oracle Eloqua], vous devez d’abord [connecter la destination](#connect-destination) en [!DNL Adobe Experience Platform], puis [configurer une importation de données](#import-data-into-eloqua) depuis l’emplacement de stockage vers [!DNL Oracle Eloqua].
