...
>* Les attributs spécifiés dans le champ **** ne respectent aucune restriction de ce type. Vous pouvez la mapper en fonction de vos besoins. Toutefois, si le format des données n’est pas correct lors de l’envoi à [!DNL Oracle Eloqua], une erreur se produira. Par exemple, vous pouvez mapper le champ **** le `contact key` d’espace de noms d’identité, le `ABC ID`, etc. au **[!UICONTROL champ cible]** : `EloquaId` après vous être assuré que les valeurs de l’identifiant correspondent au format accepté par [!DNL Oracle Eloqua].
