...
Vous trouverez ci-dessous les identités prises en charge par [!DNL The Trade Desk] destination. Utilisez ces identités pour activer les audiences à [!DNL The Trade Desk].
...
| Élément | Type | Notes |
|---------|----------|---------|
| Type d’exportation | **[!UICONTROL Exportation de l’audience]** | Vous exportez tous les membres d’une audience vers la destination . |
...
1. **Activer la fonctionnalité de synchronisation des identifiants** : si c’est la première fois que vous configurez [!DNL The Trade Desk ID] activation et que vous n’avez pas activé la fonctionnalité de synchronisation des [identifiants](https://experienceleague.adobe.com/en/docs/id-service/using/id-service-api/methods/idsync) dans le service d’identification des visiteurs par le passé (avec Adobe Audience Manager ou d’autres applications), contactez Adobe Consulting ou l’assistance clientèle pour activer la synchronisation des identifiants.
...
Ces mappages sont grisés et en lecture seule. Dans cette étape, vous n’avez rien à configurer. Sélectionnez **[!UICONTROL Suivant]** pour continuer.
...
### Exigences d’identité par type d’activation {#identity-requirements-by-activation-type}
