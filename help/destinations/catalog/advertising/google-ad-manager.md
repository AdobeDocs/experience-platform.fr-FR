...
>[!NOTE]
>
>La liste autorisée est obligatoire avant de configurer votre première [!DNL Google Ad Manager] destination dans le CDP en temps réel. Please ensure the allow list process described below has been completed by [!DNL Google] before creating a destination.

Avant de créer la destination dans le CDP en temps réel, vous devez contacter [!DNL Google Ad Manager] [!DNL Google] pour que l’Adobe soit mis sur la liste des fournisseurs de données autorisés et que votre compte soit ajouté à la liste autorisée. Contact [!DNL Google] and provide the following information:
