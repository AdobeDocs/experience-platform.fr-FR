...
feature_v2: id: a37e4ecd-c740-426a-addf-cb1b483c5c5aid: adf04a6a-050f-44bc-a52c-db79ccb22ebfid: c132d929-fa62-4271-803e-b823be07b914id: daec7ead-f475-492a-a3b3-02ae08565d6fid: e08599ea-8888-4294-ba74-3ba0a7762a46
subfeature_v2: id: a16ec9c0-4484-4842-b9a0-5504cde38e6aid: a7150a74-b8a9-4042-b066-147c5f0ca505id: a9eb38d5-9d89-492f-af4e-b968a07f2d91id: ae2cba0e-54f2-464b-a3b3-ad371e8a886aid: b784da9a-7978-4766-bf1f-5ab2b23d894aid: ca3d6bf4-a4af-4944-936b-8de1eb09f149id: cbd4a8d8-97a6-4ac9-b8d6-b6c1f28d3342id: d175cb4c-5781-454e-a826-bf6dff786265id: d1823595-9241-4128-8a33-e4ac3bf08773id: d21bd11d-08df-4cd6-ad8f-cb59a09de5c0id: ee602049-8a18-43df-9299-a689a025a371id: f6ff4d13-7b5c-4533-8556-95e76673d4cb
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
topic_v2: id: b5ce8718-c3af-4fdb-a1a9-fca32f83a87cid: bcc5edb5-84c3-4940-9f84-ed88b6c16274id: c7d04a2c-412a-4c9d-9d7a-4456eaa5adebid: d3cdead0-685a-4489-9250-4bb709942f66id: e0eb8757-182f-49f3-94a4-1587d16f5094id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
...
# Connexion [!DNL Adobe Target] {#adobe-target-connection}

## Journal des modifications de destination {#changelog}

| Mois de publication | Type de mise à jour | Description |
|---|---|---|
| Avril 2024 | Nouvelles fonctionnalités et mise à jour de la documentation | Lors de la connexion à la destination Target et de l’utilisation d’un flux de données, vous n’avez désormais *pas besoin* d’activer nécessairement le flux de données pour la segmentation Edge. Cela signifie que la destination Target fonctionnera avec les audiences par lots et en flux continu, bien que les cas d’utilisation que vous pouvez accomplir diffèrent. Consultez la table dans la section [paramètres de connexion](#parameters) pour plus d’informations. |
| Janvier 2024 | Nouvelles fonctionnalités et mise à jour de la documentation | Vous pouvez désormais partager des audiences et des attributs de profil avec la connexion [!DNL Adobe Target] pour le sandbox de production par défaut et d’autres sandbox non par défaut. |
| Juin 2023 | Nouvelles fonctionnalités et mise à jour de la documentation | Depuis juin 2023, vous pouvez sélectionner l’espace de travail [!DNL Adobe Target] vers lequel vous souhaitez partager des audiences lors de la configuration d’une nouvelle connexion de destination [!DNL Adobe Target]. Consultez la section [Paramètres de connexion](#parameters) pour plus d’informations. En outre, consultez le tutoriel sur [la configuration des espaces de travail](https://experienceleague.adobe.com/docs/target-learn/tutorials/administration/set-up-workspaces.html?lang=fr) dans [!DNL Adobe Target] pour plus d’informations sur les espaces de travail. |
| Mai 2023 | Nouvelles fonctionnalités et mise à jour de la documentation | Depuis mai 2023, la connexion **** prend en charge la [personnalisation basée sur les attributs](../../ui/activate-edge-personalization-destinations.md#map-attributes) et est généralement disponible pour tous les clients. |

{style="table-layout:auto"}

## Vue d’ensemble {#overview}

[!DNL Adobe Target] est une application qui fournit des fonctionnalités de personnalisation et d’expérimentation en temps réel et optimisées par l’IA dans toutes les interactions des clients entrants sur les sites web, les applications mobiles, etc.

[!DNL Adobe Target] est une connexion de personnalisation dans le catalogue des destinations [!DNL Adobe Experience Platform].
...
* Le **rôle** [approbateur](https://experienceleague.adobe.com/docs/target/using/administer/manage-users/enterprise/property-channel.html#roles-and-permissions).
...
>Si vous mappez des audiences Edge qui utilisent une politique de fusion différente avec des destinations [!DNL Adobe Target], ces audiences ne seront pas évaluées pour les cas d’utilisation en temps réel et de la page suivante.
...
| Élément | Type | Notes |
|---------|----------|---------|
| Type d’exportation | **[!DNL Profile request]** | Vous demandez toutes les audiences mappées dans la destination [!DNL Adobe Target] pour un seul profil. |
...
Tenez compte des points suivants lors de l’implémentation de la connexion Adobe Target :

* **Même organisation IMS** : Adobe Experience Platform et Adobe Target doivent appartenir à la même organisation IMS.
* **Gestion des identités pour la personnalisation lors de la première connexion** : la personnalisation lors de la première connexion nécessite que l’identité principale dans la demande de personnalisation corresponde à l’identité client connue dans le graphique d’identités de Real-Time CDP. Si l’identité principale est un ECID ou une identité non regroupée, la réconciliation d’identités peut prendre plusieurs minutes avant que la personnalisation Edge ne reflète les données connues du client ou de la cliente.
* **Recherches d’espaces de noms d’identité personnalisés** : les recherches sur les espaces de noms d’identité personnalisés nécessitent un déploiement d’API Web SDK ou Edge Network, avec les identités pertinentes mappées de manière appropriée.
* **Durée de vie (TTL) des profils Edge** : les profils Edge ont une durée de vie de 14 jours. Après 14 jours d’inactivité, le profil Edge expire et doit être récupéré du hub lors de la visite de la page suivante. Cela signifie que la première visite après l’expiration peut utiliser une vue de profil historique plutôt que des données en temps réel.
...
>abstract="Cette option détermine dans quel train de données de collecte de données les audiences seront inclues. Le menu déroulant affiche uniquement les trains de données pour lesquels la configuration cible est activée. Pour utiliser la segmentation Edge, vous devez sélectionner un train de données. Si vous sélectionnez Aucun, tous les cas d&#39;utilisation qui utilisent la segmentation Edge sont désactivés."
...
[!DNL Adobe Experience Platform] se connecte automatiquement à l’instance [!DNL Adobe Target] de votre entreprise. Aucune authentification n’est requise.
...
* **Flux de données** : détermine dans quel flux de données de collecte de données les audiences seront incluses. Le menu déroulant affiche uniquement les flux de données pour lesquels les services Target et [!DNL Adobe Experience Platform] sont activés. Consultez [Configuration d’un flux de données](../../../datastreams/configure.md#aep) pour plus d’informations sur la configuration d’un flux de données pour [!DNL Adobe Experience Platform] et [!DNL Adobe Target].

  >[!IMPORTANT]
  >
  >**Unicité des flux de données dans l’ensemble de l’organisation** : la combinaison de l’identifiant du flux de données et du nom du sandbox doit être unique pour [!DNL Adobe Target] connexions de destination au sein d’une organisation IMS. Cela signifie :
  >
  >* La même combinaison d’identifiant de flux de données + de nom de sandbox ne peut pas être utilisée pour plusieurs connexions de destination [!DNL Adobe Target] dans l’ensemble de l’organisation
  >* Vous pouvez utiliser le même identifiant de flux de données pour différentes connexions de destination à condition que les connexions se trouvent sur différents sandbox
  >* Cette règle s’applique à toutes les sélections de flux de données, y compris lorsque vous sélectionnez **[!UICONTROL Aucun]**

   * **[!UICONTROL Aucune]** : sélectionnez cette option si vous devez configurer [!DNL Adobe Target] personnalisation, mais que vous ne pouvez pas implémenter le SDK Web [!DNL Adobe Experience Platform]. Lorsque vous utilisez cette option, les audiences exportées d’Experience Platform vers Target ne prennent en charge que la personnalisation de session suivante, et la segmentation Edge est désactivée. Reportez-vous au tableau de la section [cas d’utilisation pris en charge](#supported-use-cases) pour une comparaison des cas d’utilisation disponibles par type d’implémentation.

  | Implémentation d’Adobe Target *sans* Web SDK | Implémentation d’Adobe Target *avec* API Web SDK ou Edge Network | Implémentation d’Adobe Target *avec* API Web SDK ou Edge Network *et* segmentation Edge désactivée |
  |---|---|---|
  | <ul><li>Aucun flux de données n’est requis. Adobe Target peut être déployé au moyen de méthodes d’implémentation [at.js](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/overview.html?lang=fr), [côté serveur](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html#server-side-implementation) ou [hybride](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html#hybrid-implementation).</li><li>La segmentation [](../../../segmentation/methods/edge-segmentation.md) n’est pas prise en charge.</li><li>[La personnalisation de la même page et de la page suivante](../../ui/activate-edge-personalization-destinations.md) n’est pas prise en charge.</li><li>Vous pouvez partager des audiences et des attributs de profil avec la connexion Adobe Target pour les sandbox *de production par défaut* et autres.</li><li>Pour configurer la personnalisation de la session suivante sans utiliser de flux de données, utilisez [at.js](https://experienceleague.adobe.com/docs/target/using/implement-target/client-side/at-js-implementation/at-js/how-atjs-works.html).</li></ul> | <ul><li>Adobe Target peut être intégré à l’aide de l’[Web SDK](https://experienceleague.adobe.com/fr/docs/experience-platform/web-sdk/home) ou de l’API [Edge Network](https://developer.adobe.com/data-collection-apis/docs/).</li><li>Un flux de données avec Adobe Target et Experience Platform configurés en tant que services est requis.</li><li>La segmentation d’Edge fonctionne comme prévu.</li><li>[La personnalisation de la même page et de la page suivante](../../ui/activate-edge-personalization-destinations.md#use-cases) est prise en charge.</li><li>Le partage d’audiences et d’attributs de profil à partir d’autres sandbox est pris en charge.</li></ul> | <ul><li>Adobe Target peut être intégré à l’aide de l’[Web SDK](https://experienceleague.adobe.com/fr/docs/experience-platform/web-sdk/home) ou de l’API [Edge Network](https://developer.adobe.com/data-collection-apis/docs/).</li><li>Un flux de données avec Adobe Target et Experience Platform configurés en tant que services est requis.</li><li>Lors de la [configuration du flux de données](/help/destinations/ui/activate-edge-personalization-destinations.md#configure-datastream), ne cochez pas la case **Segmentation Edge**.</li><li>La [personnalisation de session suivante](../../ui/activate-edge-personalization-destinations.md#next-session) est prise en charge.</li><li>Le partage d’audiences et d’attributs de profil à partir d’autres sandbox est pris en charge.</li></ul> |

* **** : sélectionnez l’[!DNL Adobe Target] [espace de travail](https://experienceleague.adobe.com/docs/target-learn/tutorials/administration/set-up-workspaces.html?lang=fr) vers lequel les audiences seront partagées. Vous pouvez sélectionner un seul espace de travail pour chaque connexion [!DNL Adobe Target]. Lors de l’activation, les audiences sont acheminées vers l’espace de travail sélectionné tout en suivant les [libellés d’utilisation des données Experience Platform applicables](../../../data-governance/labels/overview.md).
...
><br>>Ce comportement sera modifié lors d’une prochaine mise à jour.
...
[!DNL Adobe Target] *lit* les données de profil d’[!DNL Adobe Experience Platform] Edge Network, de sorte qu’aucune donnée ne soit exportée.
