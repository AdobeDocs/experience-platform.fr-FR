...
>title="À propos des flux de données"
>abstract="Cette option détermine dans quel train de données de collecte de données les audiences seront inclues. Le menu déroulant affiche uniquement les trains de données pour lesquels la configuration cible est activée. Pour utiliser la segmentation Edge, vous devez sélectionner un flux de données. Si vous sélectionnez Aucun, tous les cas d&#39;utilisation qui utilisent la segmentation Edge sont désactivés."
...
* **Flux de données** : détermine dans quel flux de données de collecte de données les audiences seront incluses. Le menu déroulant affiche uniquement les flux de données pour lesquels les services Target et Adobe Experience Platform sont activés. Consultez [Configuration d’un flux de données](../../../datastreams/configure.md#aep) pour plus d’informations sur la configuration d’un flux de données pour Adobe Experience Platform et Adobe Target.
  ...
  >[!IMPORTANT]
  >
  >Le flux de données est unique pour chaque connexion de destination Adobe Target. Vous ne pouvez pas utiliser le même flux de données pour plusieurs connexions de destination Adobe Target.
  >Si vous devez mapper les mêmes audiences à plusieurs flux de données, vous devez [créer une connexion de destination](../../ui/connect-destination.md) pour chaque flux de données et passer par le [flux d’activation de l’audience](#activate).
