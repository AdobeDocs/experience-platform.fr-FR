...
Le tableau ci-dessous affiche les cas d’utilisation pris en charge pour la destination Adobe Target, en fonction de votre type d’implémentation, avec ou sans SDK web et avec ou sans activation de la [segmentation Edge](/help/segmentation/home.md#edge).
...
* **Flux de données** : détermine dans quel flux de données de collecte de données les audiences seront incluses. Le menu déroulant affiche uniquement les flux de données pour lesquels les services Target et Adobe Experience Platform sont activés. Consultez [Configuration d’un flux de données](../../../datastreams/configure.md#aep) pour plus d’informations sur la configuration d’un flux de données pour Adobe Experience Platform et Adobe Target.
  ...
  >[!IMPORTANT]
  >
  >**Unicité des flux de données dans l’ensemble de l’organisation** : la combinaison de l’identifiant du flux de données et du nom du sandbox doit être unique pour les connexions de destination Adobe Target au sein d’une organisation IMS. Cela signifie :
  >
  >* La même combinaison d’identifiant de flux de données + de nom de sandbox ne peut pas être utilisée pour plusieurs connexions de destination Adobe Target dans l’ensemble de l’organisation
