...
| `customerAuthenticationConfigurations.authType` | Chaîne | Les valeurs prises en charge pour les destinations de diffusion en continu sont les suivantes : <ul><li>`BASIC`</li><li>`BEARER`</li><li>`OAUTH2`</li></ul> Les valeurs prises en charge pour les destinations basées sur des fichiers sont les suivantes : <ul><li>`S3`</li><li>`AZURE_CONNECTION_STRING`</li><li>`AZURE_SERVICE_PRINCIPAL`</li><li>`SFTP_WITH_SSH_KEY`</li><li>`SFTP_WITH_PASSWORD`</li></ul> |
