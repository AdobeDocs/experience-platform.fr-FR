...
Pour comprendre les concepts et les fonctions de cette page de référence, commencez par consulter le document relatif au [format du message](message-format.md). Vous devez connaître la [structure d’un profil](message-format.md#profile-structure) dans Experience Platform avant de pouvoir utiliser des modèles de [!DNL Pebble] pour transformer les données exportées.
