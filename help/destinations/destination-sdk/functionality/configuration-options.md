...
* [Configuration des métadonnées d’audience](destination-configuration/audience-metadata-configuration.md) : découvrez comment les métadonnées de segment telles que les noms de segment ou les identifiants doivent être partagées entre Experience Platform et la destination.
...
Ce composant vous permet de configurer la manière dont les audiences ou les segments sont créés, mis à jour ou supprimés par programmation dans la destination. Pour les destinations basées sur des fichiers, il vous permet de configurer une notification chaque fois que les fichiers sont correctement diffusés vers la destination. Vous pouvez configurer cette fonctionnalité via le [point d’entrée audience-modèles](../metadata-api/create-audience-template.md).
