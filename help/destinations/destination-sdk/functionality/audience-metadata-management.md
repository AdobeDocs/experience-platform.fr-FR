...
### Cas d’utilisation 1 : vous disposez d’une API tierce et les utilisateurs et utilisatrices n’ont pas besoin de saisir les identifiants de mappage {#use-case-1}
...
### Cas d’utilisation 2 : les personnes doivent d’abord créer une audience dans votre destination et saisir manuellement l’identifiant de mappage {#use-case-2}
...
### Cas d’utilisation 3 : votre destination accepte l’identifiant d’audience Experience Platform et les personnes n’ont pas besoin de saisir manuellement l’identifiant de mappage {#use-case-3}
...
|--- |--- |
| `create` | Inclut tous les composants requis (URL, méthode HTTP, en-têtes, requête et corps de réponse) pour effectuer un appel HTTP vers votre API, pour créer par programmation des segments/audiences dans votre plateforme et resynchroniser les informations avec [!DNL Adobe Experience Platform]. |
| `update` | Inclut tous les composants requis (URL, méthode HTTP, en-têtes, requête et corps de réponse) pour effectuer un appel HTTP vers votre API, pour mettre à jour par programmation les segments/audiences de votre plateforme et resynchroniser les informations avec [!DNL Adobe Experience Platform]. |
...
| `createDestination` | Inclut tous les composants requis (URL, méthode HTTP, en-têtes, requête et corps de réponse) pour effectuer un appel HTTP vers votre API afin de créer par programmation un flux de données dans votre plateforme et de resynchroniser les informations avec [!DNL Adobe Experience Platform]. |
| `updateDestination` | Inclut tous les composants requis (URL, méthode HTTP, en-têtes, requête et corps de réponse) pour effectuer un appel HTTP vers votre API, pour mettre à jour par programmation un flux de données dans votre plateforme et resynchroniser les informations avec [!DNL Adobe Experience Platform]. |
...
| Macro | Description |
|--- |--- |
| `{{segment.alias}}` | Accède à l’alias de l’audience dans Experience Platform. |
| `{{segment.name}}` | Accède au nom de l’audience dans Experience Platform. |
| `{{segment.id}}` | Accède à l’identifiant de l’audience dans Experience Platform. |
| `{{customerData.accountId}}` | Accède au champ d’identifiant de compte que vous configurez dans la configuration de destination. |
| `{{oauth2ServiceAccessToken}}` | Génère de manière dynamique un jeton d’accès en fonction de votre configuration OAuth 2. |
