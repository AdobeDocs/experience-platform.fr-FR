...
Pour certaines destinations, Destination SDK exige que vous configuriez des métadonnées d’audience afin de créer, mettre à jour ou supprimer des audiences par programmation dans votre destination. Pour plus d’informations sur le moment et la manière d’effectuer cette configuration[&#128279;](../functionality/audience-metadata-management.md) consultez la section  Gestion des métadonnées d’audience .
...
* Si vous avez sélectionné `"authenticationRule": "CUSTOMER_AUTHENTICATION"` dans la configuration des destinations, reportez-vous aux sections suivantes pour connaître les types d’authentification pris en charge par Destination SDK pour les destinations basées sur des fichiers :
   ...
   * [Authentification SFTP avec clé SSH](../functionality/destination-configuration/customer-authentication.md#sftp-ssh-key-auth)
   * [Authentification SFTP avec mot de passe](../functionality/destination-configuration/customer-authentication.md#sftp-password-auth)
...
Une fois votre destination configurée à l’aide des points d’entrée de configuration dans les étapes précédentes, vous pouvez utiliser l’outil de test de destination [destination](../testing-api/batch-destinations/file-based-destination-testing-overview.md) pour tester l’intégration entre [!DNL Adobe Experience Platform] et votre destination.
...
* [Création d’une audience - présentation vidéo](https://experienceleague.adobe.com/docs/platform-learn/tutorials/segments/create-segments.html?lang=fr)
