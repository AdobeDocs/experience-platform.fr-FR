...
>[!NOTE]
>
Vous pouvez mettre à jour le nom d’affichage et la description facultative du schéma à partir de la barre latérale **[!UICONTROL Propriétés du schéma]**. Une fois qu’un nouveau nom est saisi, le canevas se met automatiquement à jour pour refléter le nouveau nom du schéma.

![L’éditeur de schémas avec la classe de base et le schéma en surbrillance.](../images/tutorials/create-schema/loyalty-members-schema-editor.png)
...
![L’éditeur de schémas avec le bouton Ajouter des groupes de champs surligné.](../images/tutorials/create-schema/add-field-group-button.png)
...
![La boîte de dialogue [!UICONTROL Ajouter des groupes de champs] avec l’icône d’aperçu de groupe de champs sélectionnée mise en surbrillance.](../images/tutorials/create-schema/preview-field-group-button.png)
...
![Boîte de dialogue [!UICONTROL Preview field group] avec prévisualisation du groupe de champs Détails démographiques.](../images/tutorials/create-schema/preview-field-group.png)
...
![La boîte de dialogue [!UICONTROL Ajouter des groupes de champs] avec le groupe de champs Détails démographiques sélectionné et [!UICONTROL Ajouter des groupes de champs] surligné.](../images/tutorials/create-schema/demographic-details.png)
...
![Les groupes de champs de l’éditeur de schémas avec les détails démographiques sont mis en surbrillance.](../images/tutorials/create-schema/demographic-details-structure.png)
...
>[!NOTE]
>
N’oubliez pas que les champs peuvent utiliser des types scalaires. (such (chaîne, entier, tableau ou date), ainsi que tout type de données (a groupe de champs représentant un concept commun) défini dans [!DNL Schema Registry].

Notez que le champ `name` a un type de données of &quot;[!UICONTROL Nom complet]&quot;, ce qui signifie qu’il décrit également un concept commun et contient des sous-champs liés au nom, tels que le prénom, le nom, le titre de politesse et le suffixe.
...
![La boîte de dialogue [!UICONTROL Ajouter des groupes de champs] avec deux nouveaux groupes de champs sélectionnés et [!UICONTROL Ajouter des groupes de champs] surlignée.](../images/tutorials/create-schema/more-field-groups.png)
...
Le schéma [!UICONTROL Loyalty Members] est destiné à capturer les données relatives aux membres d’un programme de fidélité. Le groupe de champs [!UICONTROL Loyalty Details] standard que vous avez ajouté au schéma fournit la plupart de ces données, y compris le type de programme, points, date de jointure, etc.
...
![ La boîte de dialogue [!UICONTROL Ajouter des groupes de champs] avec [!UICONTROL Créer un nouveau groupe de champs], [!UICONTROL Nom d’affichage] et [!UICONTROL Description] mise en surbrillance.](../images/tutorials/create-schema/create-new-field-group.png)
...
![ L’éditeur de schémas avec l’icône plus en surbrillance.](../images/tutorials/create-schema/add-field.png)
...
![L’éditeur de schémas avec un [!UICONTROL champ sans titre] et le schéma [!UICONTROL Propriétés de champ] mis en surbrillance.](../images/tutorials/create-schema/untitled-field.png)

Dans ce scénario, le schéma doit être de type objet field qui décrit en détail le niveau de fidélité actuel de la personne. À l’aide des commandes du rail de droite, commencez à créer un champ `loyaltyTier` avec le type "[!UICONTROL Object]&quot; qui sera utilisé pour contenir vos champs associés.
...
![ L’éditeur de schémas avec l’objet Loyalty Level ajouté au schéma [!UICONTROL Propriétés du champ] surligné.](../images/tutorials/create-schema/loyalty-tier-object.png)
...
* **[!UICONTROL Type] :** Type de données of le champ . Cela inclut les types scalaires de base and tout type de données defined dans le [!DNL Schema Registry]. Exemples : [!UICONTROL chaîne], [!UICONTROL entier], [!UICONTROL booléen], [!UICONTROL personne], [!UICONTROL adresse], [!UICONTROL numéro de téléphone], etc.
* **[!UICONTROL Description] :** une description facultative du champ doit être incluse avec un maximum de 200 caractères.

Le premier champ de l’objet `loyaltyTier` sera une chaîne de caractères appelée `id`, représentant l’ID du niveau actuel du membre du programme de fidélité. L’ID de niveau sera unique pour chaque membre du programme de fidélité, car cette entreprise définit différents seuils de point de niveau de fidélité pour chaque client en fonction de différents facteurs. Définir le type du nouveau champ to &quot;[!UICONTROL String]&quot; et la section **[!UICONTROL Propriétés du champ]**&quot; se remplit de plusieurs options pour appliquer des contraintes, y compris la valeur par défaut, le format et la longueur maximale. Pour en savoir plus, consultez la documentation sur les [bonnes pratiques relatives aux champs de validation des données](../schema/best-practices.md#data-validation-fields) .

![L’éditeur de schéma avec les valeurs de propriété de champ pour le nouveau champ d’identifiant surligné.](../images/tutorials/create-schema/string-constraints.png)
...
![L’éditeur de schéma avec le champ ID ajouté et surligné.](../images/tutorials/create-schema/id-field-added.png)
...
![L’éditeur de schémas avec l’objet Loyalty Level en surbrillance.](../images/tutorials/create-schema/loyalty-tier-object-fields.png)

## Ajouter un champ d’énumération au groupe de champs {#enum}

Lors de la définition de champs dans le [!DNL Schema Editor], vous pouvez appliquer d’autres options aux types de champs de base. in afin de fournir des contraintes supplémentaires sur les données que le champ peut contenir. Les cas d’utilisation de ces contraintes sont expliqués dans le tableau suivant :
...
| [!UICONTROL Tableau] | Indique que le champ contient un tableau de valeurs, chacune avec un type de données specified. Par exemple, en utilisant cette contrainte sur un champ avec un type de données of &quot;[!UICONTROL String]&quot; spécifie que le champ contiendra un tableau de chaînes. |
...
![L’éditeur de schémas avec l’objet de classe de niveau ajouté et surligné dans les [!UICONTROL propriétés de champ].](../images/tutorials/create-schema/tier-class-type.png)
...
![Les propriétés de champ enum et suggérer des valeurs ont été complétées par [!UICONTROL Apply] en surbrillance.](../images/tutorials/create-schema/tier-class-enum.png)
...
![L’éditeur de schémas avec l’objet loyaltyLevel et [!UICONTROL Convertir en nouveau type de données] mis en surbrillance.](../images/tutorials/create-schema/convert-data-type.png)
...
![ L’éditeur de schémas avec l’objet loyaltyLevel et le nouveau nom d’affichage mis en surbrillance.](../images/tutorials/create-schema/loyalty-tier-data-type.png)
...
![Certaines cases à cocher sélectionnées dans la section Groupes de champs de l’éditeur de schémas pour réduire la taille du diagramme de schéma.](../images/tutorials/create-schema/filter-by-field-group.png)
...
![Champ de recherche de l’éditeur de schémas avec les résultats pertinents surlignés sur la zone de travail.](../images/tutorials/create-schema/search.png)
...
![L’éditeur de schéma avec l’adresse électronique mise en surbrillance et la case à cocher Identité de Principal activée.](../images/tutorials/create-schema/primary-identity.png)
...
![L’éditeur de schéma avec l’adresse électronique mise en surbrillance et le champ d’identité mis en surbrillance dans la barre latérale de composition de schéma.](../images/tutorials/create-schema/identity-applied.png)
...
![Boîte de dialogue d’identité principale manquante.](../images/tutorials/create-schema/missing-primary-identity.png)
...
![ L’éditeur de schémas avec la racine de schéma et le bouton activer pour le profil sont mis en surbrillance.](../images/tutorials/create-schema/profile-toggle.png)
...
![Boîte de dialogue de confirmation Activer pour Profile.](../images/tutorials/create-schema/enable-profile.png)
...
Dans l’éditeur de schémas, vous pouvez également exécuter des actions rapides pour copier la structure JSON du schéma ou supprimer le schéma. Sélectionnez [!UICONTROL Plus] en haut de la vue pour afficher une liste déroulante avec des actions rapides.
...
Un schéma peut être supprimé dans l’interface utilisateur de l’éditeur de schémas à l’aide des actions [!UICONTROL Plus] et également des détails du schéma dans l’onglet [!UICONTROL Parcourir]. Certaines conditions empêchent la suppression d’un schéma. Un schéma ne peut pas être supprimé si :
...
>[!WARNING]
>
L’interface utilisateur [!DNL Platform] affichée dans les vidéos suivantes est obsolète. Consultez la documentation pour découvrir les dernières captures dʼécran et fonctionnalités de lʼinterface utilisateur.
