...
Pour étendre la `meta:enum` d’un champ de chaîne standard, vous pouvez créer un [descripteur de nom convivial](../api/descriptors.md#friendly-name) pour le champ en question dans un schéma particulier.
...
La requête suivante ajoute les valeurs suggérées au champ de `eventType` standard (fourni par la classe [XDM ExperienceEvent](../classes/experienceevent.md)) pour le schéma identifié sous `sourceSchema` :
...
>* [Créer un groupe de champs personnalisés](../api/field-groups.md#create)
