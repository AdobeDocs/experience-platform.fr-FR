...
Adobe Experience Platform fournit un solide ensemble de [groupes de champs](../schema/composition.md#field-group) normalisés à utiliser dans les schémas de modèle de données d’expérience (XDM). La structure et la sémantique derrière ces groupes de champs sont soigneusement conçues pour répondre à un large éventail de cas d’utilisation de la segmentation et à d’autres applications en aval dans Platform. Vous pouvez également définir vos propres groupes de champs personnalisés pour répondre à des besoins professionnels uniques.
...
Dans l’exemple suivant, le groupe de champs standard **[!UICONTROL Demographic Details]** a été ajouté à un schéma. Pour supprimer un champ unique, tel que `maritalStatus`, sélectionnez le champ dans la zone de travail, puis sélectionnez **[!UICONTROL Supprimer]** dans le rail de droite.

![ L’éditeur de schémas avec le groupe de champs, le champ État civil et la fonction Supprimer mise en surbrillance.](../images/ui/field-based-workflows/remove-single-field.png)
...
![La boîte de dialogue Gérer les champs associés avec le diagramme de groupe de champs et Confirmer mise en surbrillance.](../images/ui/field-based-workflows/select-fields.png)

Le canevas réapparaît avec uniquement les champs sélectionnés présents dans la structure du schéma.

![L’éditeur de schéma avec le nouveau groupe de champs modifié en surbrillance.](../images/ui/field-based-workflows/fields-added.png)
...
![Nom de champ en surbrillance et liste des champs standard recommandés affichés dans les propriétés de champ de l’éditeur de schémas.](../images/ui/field-based-workflows/standard-field-search.png)
...
![L’éditeur de schémas avec le groupe de champs, le champ standard et la fonction Gérer les champs associés mis en surbrillance.](../images/ui/field-based-workflows/standard-field-added.png)
...
![L’éditeur de schémas avec l’icône d’ajout et un champ de niveau racine sans titre en surbrillance.](../images/ui/field-based-workflows/root-custom-field.png)
...
À partir de là, indiquez un nom d’affichage et un type de données pour le champ. Sous **[!UICONTROL Attribuer un groupe de champs]**, vous devez sélectionner un groupe de champs pour le nouveau champ à associer. Commencez à saisir le nom du groupe de champs, et si vous avez déjà [créé des groupes de champs personnalisés](./resources/field-groups.md#create), ils apparaîtront dans la liste déroulante. Vous pouvez également saisir un nom unique dans le champ pour créer un groupe de champs à la place.

![Les paramètres de propriété Nom d’affichage, Type et Attribuer à un champ surlignés dans l’éditeur de schémas.](../images/ui/field-based-workflows/select-field-group.png)
...
Si le schéma sur lequel vous travaillez comporte un champ de type objet fourni par un groupe de champs standard, vous pouvez ajouter vos propres champs personnalisés à cet objet standard. Sélectionnez l’icône plus (**+**) en regard de la racine de l’objet.
...
![ L’éditeur de schémas avec l’icône plus en regard d’un objet standard surligné.](../images/ui/field-based-workflows/add-field-to-standard-object.png)

Pour plus d’informations sur l’ajout de champs personnalisés, consultez le [Guide de création et de modification de schémas](./resources/schemas.md#custom-fields-for-standard-groups) .
