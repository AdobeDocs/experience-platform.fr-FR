...
Dans des cas spécifiques, il peut être nécessaire de créer un schéma [!DNL Experience Data Model] (XDM) avec des champs dont l’espace de noms est utilisé uniquement par un seul jeu de données. On parle alors de schéma ad hoc. Les schémas ad hoc sont utilisés dans plusieurs workflows d’assimilation de données, notamment dans l’ingestion de fichiers CSV et dans la création de certains types de connecteurs source. Pour plus d’informations sur les schémas ad hoc, y compris sur les étapes de création d’un schéma dans l’API [!DNL Schema Registry], reportez-vous au [tutoriel sur les schémas ad hoc](../tutorials/ad-hoc.md).
