---
keywords: Experience Platform;accueil;rubriques les plus consultées;api;API;XDM;système XDM;modèle de données d’expérience;modèle de données d’expérience;modèle de données d’expérience;modèle de données;modèle de données;registre des types de données;registre des schémas;type de données;type de données;types de données;types de données;créer
solution: Experience Platform
title: Point d’entrée de l’API Data Types
description: Le point d’entrée /datatypes dans l’API Schema Registry vous permet de gérer par programmation les types de données XDM au sein de votre application d’expérience.
...
Vous pouvez répertorier tous les types de données sous le conteneur `global` ou `tenant` en effectuant une requête GET à `/global/datatypes` ou `/tenant/datatypes`, respectivement.
...
La requête suivante récupère une liste de types de données à partir du conteneur de `tenant`, à l’aide d’un paramètre de requête `orderby` pour trier les types de données en fonction de leur attribut `title`.
...
La requête ci-dessus a utilisé l’en-tête `application/vnd.adobe.xed-id+json` `Accept`. Par conséquent, la réponse inclut uniquement les attributs `title`, `$id`, `meta:altId` et `version` pour chaque type de données. L’utilisation de l’autre en-tête de `Accept` (`application/vnd.adobe.xed+json`) renvoie tous les attributs de chaque type de données. Sélectionnez l’en-tête de `Accept` approprié en fonction des informations dont vous avez besoin dans votre réponse.
...
Vous pouvez rechercher un type de données spécifique en incluant l’identifiant du type de données dans le chemin d’accès d’une requête GET.
...
| Paramètre | Description |
| --- | --- |
| `{CONTAINER_ID}` | Conteneur contenant le type de données à récupérer : `global` pour un type de données créé par Adobe ou `tenant` pour un type de données appartenant à votre organisation. |
| `{DATA_TYPE_ID}` | `$id` codée en `meta:altId` ou en URL du type de données que vous souhaitez rechercher. |
...
Une réponse réussie renvoie les détails du type de données. Les champs renvoyés dépendent de l’en-tête `Accept` envoyé dans la requête. Testez différents en-têtes `Accept` pour comparer les réponses et déterminer l’en-tête le mieux adapté à votre cas d’utilisation.
...
Vous pouvez définir un type de données personnalisé sous le conteneur `tenant` en effectuant une requête POST.
...
Contrairement aux groupes de champs, la définition d’un type de données ne nécessite pas de champs `meta:extends` ou `meta:intendedToExtend`, pas plus que les champs ne doivent être imbriqués pour éviter les collisions.

Lorsqu’il s’agit de définir la structure du champ du type de données lui-même, vous pouvez utiliser des types primitifs (comme `string` ou `object`) ou vous pouvez référencer d’autres types de données existants par le biais d’attributs `$ref`. Consultez le guide sur la [définition de champs XDM personnalisés dans l’API](../tutorials/custom-fields-api.md) pour obtenir des conseils détaillés sur le format attendu pour les différents types de champs XDM.

La requête suivante crée un type de données d’objet « Construction de propriété » avec des sous-propriétés `yearBuilt`, `propertyType` et `location` :
...
L’exécution d’une requête GET pour [répertorier tous les types de données](#list) dans le conteneur du client inclut désormais le type de données Détails de la propriété , ou vous pouvez [effectuer une requête de recherche (GET)](#lookup) à l’aide de l’URI `$id` codé par URL pour afficher directement le nouveau type de données.

## Mettre à jour un type de données {#put}

Vous pouvez remplacer un type de données entier par le biais d’une opération PUT, ce qui revient essentiellement à réécrire la ressource. Lors de la mise à jour d’un type de données par le biais d’une requête PUT, le corps doit inclure tous les champs requis lors de la [création d’un nouveau type de données](#create) dans une requête POST.

>[!NOTE]
>
>Si vous souhaitez uniquement mettre à jour une partie d’un type de données au lieu de le remplacer entièrement, consultez la section sur la [mise à jour d’une partie d’un type de données](#patch).
...
| Paramètre | Description |
| --- | --- |
| `{DATA_TYPE_ID}` | `$id` codée en `meta:altId` ou en URL du type de données que vous souhaitez réécrire. |

{style="table-layout:auto"}

**Requête**

La requête suivante réécrit un type de données existant, en ajoutant un nouveau champ de `floorSize`.
...
>[!NOTE]
>
>Si vous souhaitez remplacer une ressource entière par de nouvelles valeurs au lieu de mettre à jour des champs individuels, consultez la section sur le [remplacement d’un type de données à l’aide d’une opération PUT](#put).
...
| Paramètre | Description |
| --- | --- |
| `{DATA_TYPE_ID}` | L’URI `$id` encodé URL ou le `meta:altId` du type de données que vous souhaitez mettre à jour. |

{style="table-layout:auto"}

**Requête**

L’exemple de requête ci-dessous met à jour la `description` d’un type de données existant et ajoute un nouveau champ de `floorSize`.
...
Il peut parfois être nécessaire de supprimer un type de données du registre des schémas. Pour ce faire, il suffit d’effectuer une requête DELETE avec l’identifiant de type de données fourni dans le chemin d’accès .
...
| Paramètre | Description |
| --- | --- |
| `{DATA_TYPE_ID}` | L’URI `$id` encodé URL ou le `meta:altId` du type de données à supprimer. |
