...
[!UICONTROL Product list item] est un type de données XDM standard qui décrit un produit sélectionné par un client avec des options, un prix et un contexte d’utilisation spécifiques pour un moment donné.
...
| `SKU` | [!UICONTROL String] | Unité de stock (SKU), identifiant unique d’un produit défini par le fournisseur. |
| `_id` | [!UICONTROL String] | Identifiant d&#39;élément de ligne pour cette entrée de produit. Le produit lui-même est identifié par `product`. |
| `currencyCode` | [!UICONTROL String] | Code de devise alphabétique [ISO 4217](https://www.iso.org/iso-4217-currency-codes.html) utilisé pour la tarification du produit. |
...
| `name` | [!UICONTROL String] | Nom d’affichage du produit tel qu’il est présenté à l’utilisateur pour cette vue de produit. |
...
| `productAddMethod` | [!UICONTROL String] | Méthode utilisée par le visiteur pour ajouter un produit à la liste. |
| `productImageUrl` | [!UICONTROL String] | URL de l’image principale du produit. |
| `quantity` | [!UICONTROL Integer] | Nombre d’unités du produit que le client a indiqué. |
| `unitOfMeasureCode` | [!UICONTROL String] | Code [unité de mesure](https://ucum.org/ucum) standard du produit tel qu&#39;il est associé à la propriété `quantity`. |
