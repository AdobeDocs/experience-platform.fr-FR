...
#  Type Environmentdata
 ...
 L’environnement est un type de données XDM standard qui décrit l’environnement environnant d’un événement observé, en détaillant spécifiquement les informations transitoires telles que les versions réseau et logicielle.
...
| `colorDepth` | Entier | Nombre de bits utilisés pour chaque composant de couleur d’un seul pixel. |
...
| `operatingSystem` | Chaîne | Nom du système d’exploitation utilisé lors de l’observation. L’attribut ne doit pas contenir d’informations de version telles que `10.5.3`, mais contenir à la place des désignations &quot;d’édition&quot; telles que `Ultimate` ou `Professional`. |
...
| `type` | Chaîne | Type de l’environnement de l’application. Voir l’ [annexe](#type) pour connaître les valeurs acceptées. |
| `viewportHeight` | Entier | Taille verticale en pixels de la fenêtre dans laquelle l’expérience s’affichait. Pour un événement d’affichage Web, il s’agit de la hauteur de la fenêtre d’affichage du navigateur. |
| `viewPortWidth` | Entier | Taille horizontale en pixels de la fenêtre dans laquelle l’expérience s’affichait. Pour un événement d’affichage Web, il s’agit de la largeur de la fenêtre d’affichage du navigateur. |
