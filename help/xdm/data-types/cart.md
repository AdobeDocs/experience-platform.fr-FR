...
[!UICONTROL Panier] est un type de données standard du modèle de données d’expérience (XDM) qui fournit des propriétés liées à un panier. Utilisez ce type de données pour capturer l’identifiant unique attribué par le vendeur (`Cart ID`) et la source (`Cart Source`) dans laquelle un ou plusieurs produits ont été ajoutés au panier.

![Diagramme du type de données [!UICONTROL Panier].](../images/data-types/cart.png)

| Nom d’affichage | Propriété | Type de données | Description |
|----------------|-------------------|-----------|------------------------------------------------------------|
| [!UICONTROL ID de panier] | `cartID` | chaîne | Identifiant unique attribué au panier par le vendeur. |
| [!UICONTROL Panier Source] | `cartSource` | chaîne | Emplacement d’où un ou plusieurs produits ont été ajoutés au panier. |
