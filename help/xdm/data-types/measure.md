...
| --- | --- | --- |
| `id` | Chaîne | Identificateur unique de cette mesure. Dans les cas de collecte de données utilisant des canaux de communication avec perte, tels que les applications mobiles ou les sites Web dotés de fonctionnalités hors ligne où la transmission des mesures ne peut pas être assurée, cette propriété contient un identifiant unique généré par le client de la mesure prise. Il est recommandé de le faire suffisamment longtemps pour garantir un caractère assez aléatoire. <br><br> Si des informations telles que l’horodatage, l’ID de périphérique, l’adresse IP, l’adresse MAC ou d’autres valeurs potentiellement identifiables par l’utilisateur sont intégrées à la génération du  `id`fichier, le résultat doit être haché. Cela permet de s’assurer qu’aucune identification personnelle n’est codée dans la valeur, car l’objectif n’est pas d’identifier un utilisateur ou un périphérique, mais la mesure spécifique dans le temps. |
