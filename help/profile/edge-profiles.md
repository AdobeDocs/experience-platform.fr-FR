...
- **projection Edge** : une projection Edge est la vue de projection système du profil sur une périphérie spécifique pour représenter les données de profil avec un identifiant unique conforme à un schéma donné pour un client donné. Par exemple, une entité respectant le schéma de profil avec l’ID `CJsDEAMaEAHmCKwPCQYNvzxD9JGDHZ8`, visiteur du site web Luma, répliqué vers le centre de données VA6, contenant les champs `age = 35` et `gender = male`.

En d’autres termes, les données sont acheminées vers une périphérie par une projection, avec la **destination de projection** qui définit **à laquelle les données seront envoyées.**
...
Le service de profil express (XPS) récupère les profils sur les différentes périphéries. Ce service reçoit les demandes des solutions en aval, telles que les destinations Adobe Target ou Personalization personnalisées, recherche les profils des bases de données sur les périphéries et envoie le profil demandé à la solution qui le demande. Si le profil est introuvable, une demande d’actualisation est envoyée au hub associé.

## Étapes suivantes

Après avoir lu ce guide, vous devez avoir une compréhension de base des profils Edge, y compris des informations sur les régions et services disponibles pour les profils Edge. Pour plus d’informations sur Adobe Experience Edge, consultez la [présentation de l’Edge Network](../web-sdk/home.md#edge-network).
...
En fonction de la situation, les profils Edge peuvent atterrir dans différentes régions.
...
Pour les profils Edge, l’expiration des données détermine la durée pendant laquelle le profil restera sur Edge avant sa suppression. L’expiration des données est **roulante**, ce qui signifie que chaque fois que le profil est accessible en périphérie, le délai d’expiration des données est réinitialisé. Par défaut, l’expiration des données dure 14 jours.
