...
Pour appliquer des libellés à vos données, vous devez disposer de l’autorisation **[!UICONTROL Gérer les libellés d’utilisation]** afin de les utiliser sur le sandbox de production par défaut appelé « prod ». Pour créer un libellé personnalisé, vous devez également disposer de droits d’administration sur le profil de produit. Chaque organisation ne dispose que d’une seule liste de libellés applicables. Vous **ne pouvez pas** supprimer les libellés. Vous pouvez plutôt les supprimer des jeux de données ou des champs auxquels ils sont appliqués.
...
Pour créer un schéma, sélectionnez **[!UICONTROL Créer un schéma]** dans le coin supérieur droit de l’espace de travail **[!UICONTROL Schémas]**. Veuillez consulter le guide sur [comment créer un schéma à l’aide de l’éditeur de schémas](../../xdm/tutorials/create-schema-ui.md#create) pour obtenir des instructions complètes. Vous pouvez également [créer un schéma à l’aide de l’API Schema Registry](../../xdm/tutorials/create-schema-api.md) si nécessaire.
...
Les libellés peuvent être appliqués à l’ensemble du jeu de données à partir de l’onglet **[!UICONTROL Gouvernance des données]** de l’espace de travail **[!UICONTROL Jeux de données]**. L’espace de travail vous permet de gérer les libellés d’utilisation des données au niveau du jeu de données.

![Onglet [!UICONTROL Gouvernance des données] de l’espace de travail [!UICONTROL Jeux de données] avec la gouvernance des données en surbrillance.](../images/labels/dataset-governance.png)

Pour modifier les libellés d’utilisation des données au niveau du jeu de données, commencez par sélectionner l’icône en forme de crayon (![Icône en forme de crayon.](/help/images/icons/edit.png)) dans la ligne du nom du jeu de données.

![Onglet [!UICONTROL Gouvernance des données] de l’espace de travail [!UICONTROL Jeux de données] avec l’icône de modification en forme de crayon en surbrillance.](../images/labels/dataset-level-edit.png)

La boîte de dialogue **[!UICONTROL Modifier les libellés de gouvernance]** s’ouvre. Dans la boîte de dialogue, cochez les cases en regard des libellés que vous souhaitez appliquer au jeu de données. Souvenez-vous que ces libellés seront hérités par tous les champs du jeu de données. L’en-tête **[!UICONTROL Libellés appliqués]** se met à jour au fur et à mesure que vous cochez chaque case, affichant les libellés que vous avez choisis. Une fois les libellés sélectionnés, cliquez sur **[!UICONTROL Enregistrer les modifications]**.
...
L’espace de travail **[!UICONTROL Gouvernance des données]** réapparaît, affichant les libellés que vous avez appliqués au niveau du jeu de données dans la ligne initiale du tableau. Vous pouvez également voir les libellés, indiqués par des cartes individuelles, hérités de chacun des champs du jeu de données.

![Onglet [!UICONTROL &#x200B; Gouvernance des données] de l’espace de travail [!UICONTROL Jeux de données] avec les libellés appliqués au niveau du jeu de données et les libellés hérités du champ du jeu de données mis en surbrillance.](../images/labels/applied-dataset-labels.png)
...
Le bouton (bascule) **[!UICONTROL Afficher les libellés hérités]** est activé par défaut, ce qui vous permet de voir les libellés hérités du schéma vers ses champs. Si vous désactivez cette option, les libellés hérités du jeu de données sont masqués.
...
Vous pouvez créer vos propres libellés d’utilisation personnalisés dans l’espace de travail **[!UICONTROL Politiques]** de l’interface utilisateur de [!DNL Experience Platform]. Sélectionnez **[!UICONTROL Politiques]** dans le volet de navigation de gauche, puis sélectionnez **[!UICONTROL Libellés]** pour afficher une liste des libellés existants. À partir de là, sélectionnez **[!UICONTROL Créer un libellé]**.
...
La boîte de dialogue **[!UICONTROL Créer un libellé]** s’affiche. À partir de là, renseignez les informations suivantes pour le nouveau libellé :
