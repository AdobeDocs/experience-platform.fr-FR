...
Sur le **[!UICONTROL Créer un jeu de données]** , choisissez si vous souhaitez &quot;[!UICONTROL Création d’un jeu de données à partir d’un schéma]&quot; ou &quot;[!UICONTROL Création d’un jeu de données à partir d’un fichier CSV]&quot;.
...
Sur le **[!UICONTROL Configurer un jeu de données]** , vous devrez attribuer un nom à votre jeu de données et pourrez également fournir une description du jeu de données.
...
Une fois le chargement du fichier terminé, l’onglet **[!UICONTROL Activité du jeu de données]** n’indique plus qu’« aucun lot n’a été ajouté ». Au lieu de cela, la variable **[!UICONTROL Activité du jeu de données]** affiche désormais les mesures des jeux de données. Toutes les mesures indiqueront « 0 » à cette étape, car le lot n’a pas encore été chargé.
...
Maintenant que vous avez créé un jeu de données et que vous avez correctement ingéré des données dans [!DNL Experience Platform], vous pouvez répéter ces étapes pour créer un nouveau jeu de données ou ingérer davantage de données dans le jeu de données existant.

Pour en savoir plus sur l’ingestion par lots, consultez la section [Présentation de l’ingestion par lots](../batch-ingestion/overview.md) et complétez votre apprentissage en regardant la vidéo ci-dessous.
