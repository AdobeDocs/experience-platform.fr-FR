...
>[!AVAILABILITY]
>
>La prise en charge de l’adressage IP pour l’API d’ingestion en flux continu est en version bêta et votre organisation n’y a peut-être pas encore accès. Les fonctionnalités et la documentation sont susceptibles d’être modifiées.

Vous pouvez désormais des adresses IP pour l’API d’ingestion en flux continu. Utilisez cette fonctionnalité pour sécuriser vos points d’entrée d’ingestion en limitant l’accès aux seules adresses IP que vous spécifiez.

## Fonctionnement de l’adressage IP

La fonction de filtrage des adresses IP fonctionne comme suit :
...
## Activer l’adresse IP en cours de traitement
