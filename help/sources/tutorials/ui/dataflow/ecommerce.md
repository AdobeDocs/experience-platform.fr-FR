...
title: Configuration d’un flux de données pour une connexion source eCommerce dans l’interface utilisateur
topic-legacy: overview
type: Tutorial
description: Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un  [!DNL Platform] jeu de données. Ce tutoriel décrit les étapes de configuration d’un nouveau flux de données à l’aide de votre compte eCommerce.
...
Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un jeu de données [!DNL Platform]. Ce tutoriel décrit les étapes de configuration d’un nouveau flux de données à l’aide de votre compte **[!UICONTROL eCommerce]**.
...
En outre, ce tutoriel nécessite que vous ayez déjà créé un compte **[!UICONTROL eCommerce]**. Vous trouverez une liste des tutoriels pour créer différents **[!UICONTROL connecteurs eCommerce]** dans l’interface utilisateur dans la [présentation des connecteurs source](../../../home.md).

## Sélectionner des données

Après avoir créé votre compte **[!UICONTROL eCommerce]**, l’étape **[!UICONTROL Sélectionner les données]** s’affiche, fournissant ainsi une interface interactive vous permettant d’explorer votre hiérarchie de fichiers.
...
En suivant ce tutoriel, vous avez créé un flux de données pour importer des données **[!UICONTROL eCommerce]** et vous avez obtenu des informations sur la surveillance des jeux de données. Les données entrantes peuvent désormais être utilisées par les services [!DNL Platform] en aval tels que [!DNL Real-time Customer Profile] et [!DNL Data Science Workspace]. Pour plus d’informations, consultez les documents suivants :
