...
type: Tutorial
description: Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers une [!DNL Platform] jeu de données. Ce tutoriel décrit les étapes de configuration d’un nouveau flux de données à l’aide de votre compte eCommerce.
...
Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers une [!DNL Platform] jeu de données. Ce tutoriel décrit les étapes à suivre pour configurer un nouveau flux de données à l’aide de votre **[!UICONTROL eCommerce]** compte .
...
En outre, ce tutoriel nécessite que vous ayez déjà créé une **[!UICONTROL eCommerce]** compte . Liste des tutoriels pour créer différents **[!UICONTROL eCommerce]** Les connecteurs de l’interface utilisateur se trouvent dans la section [Présentation des connecteurs source](../../../home.md).

## Sélectionner des données

Après avoir créé votre **[!UICONTROL eCommerce]** , **[!UICONTROL Sélectionner des données]** s’affiche, fournissant ainsi une interface interactive vous permettant d’explorer votre hiérarchie de fichiers.
...
En suivant ce tutoriel, vous avez créé un flux de données à importer. **[!UICONTROL eCommerce]** données et des informations sur la surveillance des jeux de données. Les données entrantes peuvent désormais être utilisées par en aval. [!DNL Platform] des services tels que [!DNL Real-time Customer Profile] et [!DNL Data Science Workspace]. Pour plus d’informations, consultez les documents suivants :
