...
type: Tutorial
description: Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un  [!DNL Platform] jeu de données. Ce tutoriel décrit les étapes de configuration d’un nouveau flux de données à l’aide de votre compte d’automatisation marketing.
...
Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un jeu de données [!DNL Platform]. Ce tutoriel décrit les étapes de configuration d’un nouveau flux de données à l’aide de votre compte d’automatisation marketing.
...
En outre, ce tutoriel nécessite que vous ayez déjà créé un compte d’automatisation du marketing. Vous trouverez une liste des tutoriels relatifs à la création de différents connecteurs d’automatisation marketing dans l’interface utilisateur dans la [présentation des connecteurs source](../../../home.md).

## Sélectionner des données

Une fois votre compte d’automatisation du marketing créé, l’étape *Sélectionner les données* s’affiche, vous permettant ainsi d’explorer la hiérarchie des fichiers dans une interface interactive.
...
Dans l’écran **[!UICONTROL authentication]** , sélectionnez le nom de la connexion associée au flux de données que vous souhaitez désactiver.
