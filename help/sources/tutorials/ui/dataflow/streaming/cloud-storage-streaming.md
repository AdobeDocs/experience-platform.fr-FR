...
Sélectionnez le flux de données à utiliser, puis sélectionnez **[!UICONTROL Choisir le fichier]** pour charger un exemple de schéma.

>[!TIP]
>
>Si vos données sont conformes à XDM, vous pouvez ignorer le chargement d’un exemple de schéma et sélectionner **[!UICONTROL Suivant]** pour continuer.
...
La variable [!UICONTROL Jeu de données cible] mises à jour de page avec votre schéma sélectionné affiché dans le cadre du jeu de données. Pendant cette étape, vous pouvez activer votre jeu de données pour [!DNL Profile] et créer une vision globale des attributs et des comportements d’une entité. Les données de tous les jeux de données activés seront incluses dans [!DNL Profile] les modifications et sont appliquées lorsque vous enregistrez votre flux de données.
...
La variable **[!UICONTROL Sélectionner un jeu de données]** s’affiche, vous indiquant la liste des jeux de données disponibles. Sélectionnez un jeu de données dans la liste pour mettre à jour le rail droit afin d’afficher les détails spécifiques au jeu de données que vous avez sélectionné, y compris des informations sur l’activation ou non du jeu de données. [!DNL Profile].
...
Une fois que vous avez sélectionné votre jeu de données, sélectionnez la variable [!DNL Profile] bascule pour activer votre jeu de données [!DNL Profile].

![existing-profile](../../../../images/tutorials/dataflow/cloud-storage/streaming/existing-profile.png)

### Mappage des champs standard

Une fois votre jeu de données et votre schéma créés, la variable **[!UICONTROL Mappage des champs standard]** s’affiche, ce qui vous permet de configurer manuellement les champs de mappage de vos données.
...
La variable **[!UICONTROL Détails du flux de données]** s’affiche, ce qui vous permet de nommer et de décrire brièvement votre nouveau flux de données.
...
Une fois votre flux de données de stockage dans le cloud en continu créé, vous pouvez surveiller les données qui sont ingérées par celui-ci. Pour plus d’informations sur la surveillance et la suppression des flux de données en continu, consultez le tutoriel sur [surveillance des flux de données en continu](../../monitor-streaming.md).
