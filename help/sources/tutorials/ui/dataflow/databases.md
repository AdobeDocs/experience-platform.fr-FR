...
title: Créer un flux de données à l’aide d’une source de base de données dans l’interface utilisateur
type: Tutorial
...
>[!NOTE]
>
>Pour créer un flux de données, vous devez déjà disposer d’un compte authentifié avec une source de base de données. Vous trouverez une liste de tutoriels relatifs à la création de différents comptes source de base de données dans l’interface utilisateur de la section [présentation des sources](../../../home.md#database).
...
Après avoir créé votre compte source de base de données, la variable **[!UICONTROL Ajouter des données]** s’affiche, fournissant une interface permettant d’explorer la hiérarchie des tables de votre compte source de base de données.
