...
# Créer un flux de données à l’aide d’une source de protocoles dans l’interface utilisateur
...
>[!NOTE]
>
>* Pour créer un flux de données, vous devez déjà disposer d’un compte authentifié avec une source de protocoles. Vous trouverez une liste de tutoriels pour la création de différents protocoles et comptes sources dans l’interface utilisateur dans la [présentation des sources](../../../home.md#protocols).
...
Une fois votre compte source de protocoles créé, l’étape **[!UICONTROL Ajouter des données]** s’affiche, vous permettant d’explorer la hiérarchie des tables de votre compte source de protocoles.
...
Ce tutoriel vous a permis de créer un flux de données pour importer les données de la source de vos protocoles dans Experience Platform. Les données entrantes peuvent désormais être utilisées par les services [!DNL Experience Platform] en aval tels que [!DNL Real-Time Customer Profile] et [!DNL Data Science Workspace]. Consultez les documents suivants pour plus d’informations :
