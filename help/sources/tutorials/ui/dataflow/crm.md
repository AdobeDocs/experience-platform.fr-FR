...
title: Création d’un flux de données à l’aide d’un Source CRM dans l’interface utilisateur
type: Tutorial
...
# Création d’un flux de données à l’aide d’une source CRM dans l’interface utilisateur

Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un jeu de données dans Adobe Experience Platform. Ce tutoriel décrit les étapes à suivre pour créer un flux de données pour une source CRM (Customer Relationship Management) à l’aide de l’interface utilisateur de Platform.

>[!NOTE]
>
>* Pour créer un flux de données, vous devez déjà disposer d’un compte authentifié avec une source CRM. Vous trouverez une liste de tutoriels pour la création de différents comptes source CRM dans l’interface utilisateur dans la [présentation des sources](../../../home.md#crm).
...
Après la création de votre compte source CRM, l&#39;étape **[!UICONTROL Ajouter des données]** s&#39;affiche, vous permettant ainsi d&#39;explorer la hiérarchie des tables de votre compte CRM.
...
>[!NOTE]
>
>Si vous utilisez [!DNL Salesforce] dans le cadre de l’édition B2B d’Adobe Real-Time Customer Data Platform, reportez-vous au [[!DNL Salesforce]  document sur le mappage des champs](../../../connectors/adobe-applications/mapping/salesforce.md) pour obtenir des instructions sur le mappage des champs source [!DNL Salesforce] aux champs cibles XDM appropriés.
...
| Charger les données incrémentielles par | Une option avec un ensemble filtré de champs de schéma source de type, date ou heure. Ce champ est utilisé pour différencier les nouvelles données des données existantes. Les données incrémentielles seront ingérées en fonction de l’horodatage de la colonne sélectionnée. |
...
En suivant ce tutoriel, vous avez créé un flux de données pour importer les données de votre source CRM vers Platform. Les données entrantes peuvent désormais être utilisées par les services [!DNL Platform] en aval tels que [!DNL Real-Time Customer Profile] et [!DNL Data Science Workspace]. Consultez les documents suivants pour plus d’informations :
