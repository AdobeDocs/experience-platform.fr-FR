...
title: Créer un flux de données à l’aide d’un Source CRM dans l’interface utilisateur
type: Tutorial
description: Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un jeu de données Experience Platform. Ce tutoriel décrit les étapes à suivre pour créer un flux de données pour une source CRM à l’aide de l’interface utilisateur d’Experience Platform.
...
# Créer un flux de données à l’aide d’une source CRM dans l’interface utilisateur

Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un jeu de données dans Adobe Experience Platform. Ce tutoriel décrit les étapes à suivre pour créer un flux de données pour une source CRM (gestion de la relation client) à l’aide de l’interface utilisateur d’Experience Platform.

>[!NOTE]
>
>* Pour créer un flux de données, vous devez déjà disposer d’un compte authentifié avec une source CRM. Vous trouverez une liste de tutoriels sur la création de différents comptes sources CRM dans l’interface utilisateur dans la [présentation des sources](../../../home.md#crm).
...
Ce tutoriel vous a permis de créer un flux de données pour importer les données de votre source CRM dans Experience Platform. Les données entrantes peuvent désormais être utilisées par les services [!DNL Experience Platform] en aval tels que [!DNL Real-Time Customer Profile] et [!DNL Data Science Workspace]. Consultez les documents suivants pour plus d’informations :
