---
keywords: Experience Platform;accueil;rubriques populaires;analytics;mixpanel
solution: Experience Platform
title: Créer un flux de données à l’aide d’un Source Analytics dans l’interface utilisateur
description: Ce tutoriel décrit les étapes à suivre pour créer un flux de données pour une source Analytics à l’aide de l’interface utilisateur d’Experience Platform.
...
# Créer un flux de données à l’aide d’une source Analytics dans l’interface utilisateur

Un flux de données est une tâche planifiée qui récupère et ingère des données d’une source vers un jeu de données dans Adobe Experience Platform. Ce tutoriel décrit les étapes à suivre pour créer un flux de données pour une source Analytics à l’aide de l’interface utilisateur d’Experience Platform.

>[!NOTE]
>
>Pour créer un flux de données, vous devez déjà disposer d’un compte authentifié avec la source de [!DNL Mixpanel]. Pour plus d’informations, consultez le tutoriel sur la [création d’une connexion source [!DNL Mixpanel] dans l’interface utilisateur](../../ui/create/analytics/mixpanel.md).
...
>[!IMPORTANT]
>
>Vous ne pouvez pas mapper de valeurs de paire de clés dynamiques en tant qu’objet de [!DNL OneTrust] à Experience Platform et devez spécifier ces clés dans le schéma cible pour mapper vos données lors de l’ingestion.
