...
# Création d’un connecteur [!DNL Apache Hive] à la [!DNL Azure HDInsights] source dans l’interface utilisateur
...
Les connecteurs source dans l’Adobe Experience Platform permettent d’importer des données provenant de l’extérieur sur une base planifiée. Ce didacticiel décrit les étapes à suivre pour créer un connecteur [!DNL Apache Hive] on [!DNL Azure HDInsights] source à l’aide de l’ [!DNL Platform] interface utilisateur.
...
En suivant ce didacticiel, vous avez établi une connexion à votre [!DNL Hive] compte. Vous pouvez maintenant passer au didacticiel suivant et [configurer un flux de données pour importer des données dans Platform](../../dataflow/databases.md).