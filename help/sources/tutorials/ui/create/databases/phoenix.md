...
Les connecteurs source dans l’Adobe Experience Platform permettent d’importer des données provenant de l’extérieur sur une base planifiée. Ce didacticiel décrit les étapes à suivre pour créer un connecteur source Phoenix à l’aide de l’interface utilisateur de Platform.
...
En suivant ce tutoriel, vous avez établi une connexion à votre compte Phoenix. Vous pouvez maintenant passer au didacticiel suivant et [configurer un flux de données pour importer des données dans Platform](../../dataflow/databases.md).