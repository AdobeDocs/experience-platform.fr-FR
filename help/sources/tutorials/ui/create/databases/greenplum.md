...
Les connecteurs source dans Adobe Experience Platform permettent d’importer des données provenant de l’extérieur sur une base planifiée. Ce didacticiel décrit les étapes à suivre pour créer un connecteur source GreenPlum à l’aide de l’interface utilisateur de la plate-forme.
...
En suivant ce didacticiel, vous avez établi une connexion à votre compte GreenPlum. Vous pouvez maintenant passer au didacticiel suivant et [configurer un flux de données pour importer des données dans la plate-forme](../../dataflow/databases.md).