---
title: Création D’Une Connexion Source En Flux Continu Et D’Un Flux De Données Pour Shopifier Les Données À L’Aide De L’API Flow Service
...
# Création d’une connexion source en continu et d’un flux de données pour les données [!DNL Shopify] à l’aide de l’API Flow Service

>[!NOTE]
>
>La source de diffusion [!DNL Shopify] est en version bêta. Veuillez lire la [présentation des sources](../../../../home.md#terms-and-conditions) pour plus d’informations sur l’utilisation de sources étiquetées bêta.

Le tutoriel suivant décrit les étapes à suivre pour créer une connexion source en continu et un flux de données pour diffuser des données de [[!DNL Shopify]](https://www.shopify.com/) vers Adobe Experience Platform à l’aide de l’ [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Diffusion de données [!DNL Shopify] vers Platform à l’aide de l’API Flow Service

Les étapes suivantes décrivent les étapes à suivre pour créer une connexion source et un flux de données afin de diffuser vos données [!DNL Shopify] vers Platform.
...
La dernière étape pour apporter des données de [!DNL Shopify] à Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
...
| Propriété | Description |
| --- | --- |
| `name` | Nom du flux de données. Assurez-vous que le nom de votre flux de données est descriptif, car vous pouvez l’utiliser pour rechercher des informations sur votre flux de données. |
