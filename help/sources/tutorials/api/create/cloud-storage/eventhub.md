...
Ce tutoriel vous guide tout au long des étapes pour vous connecter. [!DNL Azure Event Hubs] (ci-après dénommés &quot;[!DNL Event Hubs]&quot;) à l’Experience Platform, à l’aide de la variable [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Les sections suivantes apportent des informations supplémentaires dont vous aurez besoin pour vous connecter. [!DNL Event Hubs] vers Platform à l’aide de [!DNL Flow Service] API.
...
La première étape de la création d’une connexion source consiste à authentifier votre [!DNL Event Hubs] source et générer un identifiant de connexion de base. Un identifiant de connexion de base vous permet d’explorer et de parcourir les fichiers de votre source et d’identifier les éléments spécifiques à ingérer, y compris des informations concernant leurs types et formats de données.

Pour créer un identifiant de connexion de base, envoyez une requête de POST au `/connections` point de terminaison lors de la fourniture de [!DNL Event Hubs] informations d’identification d’authentification dans le cadre des paramètres de requête.
...
| `data.format` | Le format de la variable [!DNL Event Hubs] données que vous souhaitez ingérer. Actuellement, le seul format de données pris en charge est `json`. |
