...
# Créez un flux de données pour [!DNL MailChimp Campaign] à l’aide de l’API Flow Service

Le tutoriel suivant décrit les étapes à suivre pour créer une connexion source et un flux de données afin d’importer des données [!DNL MailChimp Campaign] vers Platform à l’aide de l’[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Une fois que vous avez récupéré vos informations d’authentification [!DNL MailChimp], vous pouvez lancer le processus de création de flux de données pour importer les données [!DNL MailChimp Campaign] dans Platform. La première étape de la création d’un flux de données consiste à créer une connexion de base.
...
>[!TIP]
>
>Pour récupérer le type de format accepté pour `{SOURCE_PARAMS}`, vous devez coder l’intégralité de la chaîne `campaignId` dans base64. Par exemple, `{"campaignId": "c66a200cda"}` encodé en base64 équivaut à `eyJjYW1wYWlnbklkIjoiYzY2YTIwMGNkYSJ9`.
...
| `params.campaignId` | L’identifiant de campagne [!DNL MailChimp] identifie une campagne [!DNL MailChimp] spécifique, qui permet ensuite d’envoyer des emails à vos listes/audiences. |
...
Pour planifier une ingestion, vous devez d’abord définir la valeur de l’heure de début sur la durée en secondes. Vous devez ensuite définir la valeur de fréquence sur l’une des cinq options suivantes : `once`, `minute`, `hour`, `day` ou `week`. La valeur interval désigne la période entre deux ingestion consécutives et la création d’une ingestion unique (`once`) ne nécessite pas de définir un intervalle. Pour toutes les autres fréquences, la valeur de l’intervalle doit être égale ou supérieure à `15`.
