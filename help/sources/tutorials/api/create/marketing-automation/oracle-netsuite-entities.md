---
title: Créez une connexion source et un flux de données pour les entités NetSuite Oracle à l’aide de l’API Flow Service
description: Découvrez comment créer une connexion source et un flux de données pour importer les contacts Oracle NetSuite et les données client vers Experience Platform à l’aide de l’API Flow Service.
hide: true
hidefromtoc: true
badge: Version Beta
...
# Créer une connexion source et un flux de données pour [!DNL Oracle NetSuite Entities] utilisation de l’API Flow Service
...
Lisez le tutoriel suivant pour apprendre à importer des contacts et des données client à partir de vos [!DNL Oracle NetSuite Activities Entities] à Adobe Experience Platform à l’aide de la variable [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Les sections suivantes apportent des informations supplémentaires dont vous aurez besoin pour vous connecter. [!DNL Oracle NetSuite Entities] en utilisant la variable [!DNL Flow Service] API.
...
## Connexion [!DNL Oracle NetSuite Entities] vers Platform à l’aide de la méthode [!DNL Flow Service] API

Les étapes suivantes décrivent les étapes à suivre pour authentifier votre [!DNL Oracle NetSuite Entities] source, créer une connexion source et créer un flux de données pour importer vos données client et de contact dans Experience Platform.
...
Pour créer un identifiant de connexion de base, envoyez une requête de POST au `/connections` point de terminaison lors de la fourniture de [!DNL Oracle NetSuite Entities] informations d’identification d’authentification dans le corps de la requête.
...
| `auth.params.clientId` | La valeur de l’ID client lorsque vous créez l’enregistrement d’intégration. Le processus de création d’un enregistrement d’intégration est disponible [here](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_157771733782.html#procedure_157838925981). La valeur est une chaîne de 64 caractères semblable à `7fce.....b42f`. |
| `auth.params.clientSecret` | La valeur de l’ID client lorsque vous créez l’enregistrement d’intégration. Le processus de création d’un enregistrement d’intégration est disponible [here](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_157771733782.html#procedure_157838925981). La valeur est une chaîne de 64 caractères semblable à `5c98.....1b46`. |
| `auth.params.accessTokenUrl` | La variable [!DNL NetSuite] URL du jeton d’accès, similaire à `https://{ACCOUNT_ID}.suitetalk.api.netsuite.com/services/rest/auth/oauth2/v1/token` où vous remplacerez ACCOUNT_ID par votre [!DNL NetSuite] Identifiant de compte. |
| `auth.params.accessToken` | La valeur Jeton d’accès est générée à la fin de la variable [étape 2](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_158081952044.html#Step-Two-POST-Request-to-the-Token-Endpoint) de [Flux d’octroi du code d’autorisation OAuth 2.0](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_158074210415.html#OAuth-2.0-Authorization-Code-Grant-Flow) tutoriel . Les jetons d’accès arrivent à expiration pendant 60 minutes uniquement. la valeur est une chaîne de 1 024 caractères au format JSON Web Token (JWT) semblable à `eyJr......f4V0`. |
...
| `{SOURCE_PARAMS}` | Définit les paramètres du fichier source que vous souhaitez importer dans Platform. Pour récupérer le type de format accepté pour `{SOURCE_PARAMS}`, vous devez coder la chaîne entière en base64. <br> [!DNL Oracle NetSuite Entities] prend en charge la récupération des données client et contact. Selon le type d’objet que vous utilisez, transmettez l’un des types suivants : <ul><li>`customer` : récupérez des données client spécifiques, y compris des détails tels que les noms de client, les adresses et les identifiants clés.</li><li>`contact` : récupérez les noms de contact, les e-mails, les numéros de téléphone et tous les champs personnalisés liés aux contacts associés aux clients.</li></ul> |

>[!BEGINTABS]

>[!TAB Client]

Pour [!DNL Oracle NetSuite Entities], pour récupérer les données de contact de la valeur de `{SOURCE_PARAMS}` est transmis comme `{"object_type":"customer"}`. Lorsqu’il est codé en base64, il équivaut à `eyAib2JqZWN0X3R5cGUiOiAiY3VzdG9tZXIifQ%3D%3D` comme illustré ci-dessous.
...
Pour [!DNL Oracle NetSuite Entities], pour récupérer les données de contact de la valeur de `{SOURCE_PARAMS}` est transmis comme `{"object_type":"contact"}`. Lorsqu’il est codé en base64, il équivaut à `eyAib2JqZWN0X3R5cGUiOiAiY29udGFjdCJ9` comme illustré ci-dessous.
...
Lors de la récupération des données client, la variable `object_type` La valeur de propriété doit être `customer`.
...
Lors de la récupération des données de contact, la variable `object_type` La valeur de propriété doit être `contact`.
...
| `object_type` | [!DNL Oracle NetSuite Entities] prend en charge la récupération des clients et des contacts. Selon l’entité que vous souhaitez, transmettez l’une des entités suivantes : <ul><li>`customer` : récupérez des données client spécifiques, y compris des détails tels que les noms de client, les adresses et les identifiants clés.</li><li>`contact` : récupérez les noms de contact, les e-mails, les numéros de téléphone et tous les champs personnalisés liés aux contacts associés aux clients.</li></ul> |
...
La dernière étape pour obtenir des données de [!DNL Oracle NetSuite Entities] vers Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
