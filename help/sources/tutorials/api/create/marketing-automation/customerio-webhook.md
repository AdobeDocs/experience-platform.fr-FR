...
# Création d’une connexion source et d’un flux de données pour [!DNL Customer.io] utilisation de l’API Flow Service
...
La dernière étape pour obtenir des données de [!DNL Customer.io] à Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
...
Une réponse réussie renvoie des informations sur votre flux de données, y compris l’URL de votre point de terminaison, marquée comme `inletUrl`. Reportez-vous à la section [Configuration de Webhook](../../../ui/create/marketing-automation/customerio-webhook.md#get-streaming-endpoint-url) pour obtenir la valeur requise.
