...
Pour créer un identifiant de connexion de base, envoyez une requête de POST au point d’entrée `/connections` et indiquez vos informations d’authentification [!DNL Mixpanel] dans le corps de la requête.
...
Utilisez les appels suivants pour rechercher le chemin d’accès au fichier que vous souhaitez importer en mode Experience Platform :
...
La dernière étape pour importer des données de [!DNL Mixpanel] vers Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
