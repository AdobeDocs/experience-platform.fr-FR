...
## Connecter [!DNL Pendo] à Platform à l’aide de l’API [!DNL Flow Service] {#connect-platform-to-flow-api}

La procédure ci-dessous décrit les étapes à suivre pour créer une connexion source et un flux de données afin d’apporter vos données d’événements [!DNL Pendo] à l’Experience Platform.
...
La dernière étape pour importer des données de [!DNL Pendo] vers Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
