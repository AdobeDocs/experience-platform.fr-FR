...
# Créez une connexion source et un flux de données pour [!DNL Pendo] à l’aide de l’API Flow Service
...
Le tutoriel suivant vous guide tout au long des étapes nécessaires à la création d’une connexion source et d’un flux de données pour importer des données d’événement [[!DNL Pendo]](https://Pendo.com/) vers Adobe Experience Platform à l’aide de l’ [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connectez [!DNL Pendo] à Platform à l’aide de l’API [!DNL Flow Service] {#connect-platform-to-flow-api}

Les étapes suivantes décrivent les étapes à suivre pour créer une connexion source et un flux de données afin d’importer vos données d’événements [!DNL Pendo] dans Experience Platform.
...
La dernière étape pour apporter des données de [!DNL Pendo] à Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
...
Une réponse réussie renvoie des informations sur votre flux de données, y compris votre URL de point d’entrée, marquée comme `inletUrl`. Consultez la page [Setup Webhook](../../../ui/create/analytics/pendo-webhook.md#get-streaming-endpoint-url) pour obtenir la valeur requise.
