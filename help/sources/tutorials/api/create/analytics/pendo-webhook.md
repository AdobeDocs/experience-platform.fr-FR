---
title: Créer une connexion Source et un flux de données pour Pendo à l’aide de l’API Flow Service
...
# Créer une connexion source et un flux de données pour [!DNL Pendo] à l’aide de l’API Flow Service
...
Le tutoriel suivant vous guide tout au long des étapes nécessaires à la création d’une connexion source et d’un flux de données pour importer [[!DNL Pendo]](https://Pendo.com/) données d’événement dans Adobe Experience Platform à l’aide de l’[[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connexion de [!DNL Pendo] à Experience Platform à l’aide de l’API [!DNL Flow Service] {#connect-platform-to-flow-api}

Vous trouverez ci-dessous les étapes à suivre pour créer une connexion source et un flux de données afin d’importer vos données d’événements [!DNL Pendo] dans Experience Platform.
...
| `data.format` | Format des données [!DNL Pendo] que vous souhaitez ingérer. |
...
La dernière étape pour importer des données de [!DNL Pendo] vers Experience Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
...
Une réponse réussie renvoie des informations sur votre flux de données, y compris votre URL de point d’entrée, marquée comme `inletUrl`. Reportez-vous à la page [Configurer le Webhook](../../../ui/create/analytics/pendo-webhook.md#get-streaming-endpoint-url) pour obtenir la valeur requise.
