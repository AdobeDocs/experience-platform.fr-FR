...
# (Version bêta) Création d’une connexion source et d’un flux de données pour [!DNL SugarCRM Events] utilisation de l’API Flow Service
...
Le tutoriel suivant vous guide tout au long des étapes de création d’un [!DNL SugarCRM Events] connexion source et créer un flux de données à importer [[!DNL SugarCRM]](https://www.sugarcrm.com/) Les données d’événement vers Adobe Experience Platform à l’aide de la variable [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
## Connexion [!DNL SugarCRM Events] vers Platform à l’aide de [!DNL Flow Service] API

Les étapes suivantes décrivent les étapes à suivre pour authentifier votre [!DNL SugarCRM] source, créez une connexion source et créez un flux de données pour importer vos données d’événement dans Experience Platform.
...
Pour créer un identifiant de connexion de base, envoyez une requête de POST au `/connections` point de terminaison lors de la fourniture de [!DNL SugarCRM Events] informations d’identification d’authentification dans le corps de la requête.
...
La requête suivante permet de créer une connexion de base pour [!DNL SugarCRM Events] :
...
| `{SOURCE_PARAMS}` | Définit les paramètres du fichier source que vous souhaitez importer dans Platform. Pour récupérer le type de format accepté pour `{SOURCE_PARAMS}`, vous devez coder l’intégralité de la chaîne  en base64. <br> [!DNL SugarCRM Events] ne nécessite aucune charge utile. La valeur de `{SOURCE_PARAMS}` est transmis comme `{}`, codé en base64, il équivaut à `e30=` comme illustré ci-dessous. |
...
Une réponse réussie renvoie la structure du fichier interrogé. *Certains enregistrements ont été supprimés pour permettre une meilleure présentation.*
...
La requête suivante crée une connexion source pour [!DNL SugarCRM Events] :
...
| `baseConnectionId` | Identifiant de connexion de base de [!DNL SugarCRM Events]. Cet identifiant a été généré lors d’une étape précédente. |
| `connectionSpec.id` | Identifiant de spécification de connexion correspondant à votre source. |
| `data.format` | Format des données [!DNL SugarCRM Events] que vous souhaitez ingérer. Actuellement, le format de données `json` est le seul à être pris en charge. |
...
La requête suivante crée une connexion cible pour [!DNL SugarCRM Events] :
...
| `data.format` | Format des données [!DNL SugarCRM Events] à ingérer. |
...
La dernière étape pour obtenir des données de [!DNL SugarCRM Events] à Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
...
Pour planifier une ingestion, vous devez d’abord définir la valeur de l’heure de début en temps Unix en secondes. Vous devez ensuite définir la valeur de fréquence sur l’une des valeurs suivantes : `hour` ou `day`. La valeur interval désigne la période entre deux ingestion consécutives. La valeur d’intervalle doit être définie comme `1` ou `24` selon `scheduleParams.frequency` sélection de l’une ou l’autre `hour` ou `day`.
...
| `scheduleParams.interval` | L’intervalle désigne la période entre deux exécutions consécutives de flux. La valeur de l’intervalle doit être un nombre entier non nul. La valeur d’intervalle doit être définie comme `1` ou `24` selon `scheduleParams.frequency` sélection de l’une ou l’autre `hour` ou `day`. |
