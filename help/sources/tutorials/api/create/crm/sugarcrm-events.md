---
title: Créez une connexion source et un flux de données pour les événements SugarCRM à l’aide de l’API Flow Service
...
# (Beta) Créer une connexion source et un flux de données pour [!DNL SugarCRM Events] à l’aide de l’API Flow Service
...
Le tutoriel suivant vous guide tout au long des étapes nécessaires à la création d’une connexion source [!DNL SugarCRM Events] et d’un flux de données pour importer des données d’événements [[!DNL SugarCRM]](https://www.sugarcrm.com/) dans Adobe Experience Platform à l’aide de l’[[!DNL Flow Service] API](https://developer.adobe.com/experience-platform-apis/references/flow-service).
...
## Connexion de [!DNL SugarCRM Events] à Experience Platform à l’aide de l’API [!DNL Flow Service]

Vous trouverez ci-dessous les étapes à suivre pour authentifier la source de [!DNL SugarCRM], créer une connexion source et créer un flux de données pour importer vos données d’événements dans Experience Platform.
...
Pour créer un identifiant de connexion de base, envoyez une requête POST au point d’entrée `/connections` et indiquez vos informations d’authentification [!DNL SugarCRM Events] dans le corps de la requête.
...
À l’aide de l’identifiant de connexion de base généré à l’étape précédente, vous pouvez explorer les fichiers et répertoires en exécutant des requêtes GET.Utilisez les appels suivants pour trouver le chemin d’accès au fichier que vous souhaitez importer en [!DNL Experience Platform] :
...
| `{SOURCE_PARAMS}` | Définit les paramètres du fichier source que vous souhaitez importer dans Experience Platform. Pour récupérer le type de format accepté pour `{SOURCE_PARAMS}`, vous devez coder l’intégralité de la chaîne en base64. <br> [!DNL SugarCRM Events] ne nécessite aucune payload. La valeur de `{SOURCE_PARAMS}` est transmise en tant que `{}`, encodée en base64, elle équivaut à `e30=` comme illustré ci-dessous. |
...
La dernière étape pour importer des données de [!DNL SugarCRM Events] vers Experience Platform consiste à créer un flux de données. Vous disposez à présent des valeurs requises suivantes :
