...
>
>* Vous ne pouvez pas mettre à jour la `startTime` d’un flux de données si l’`startTime` initialement planifiée s’est déjà produite. Cette limitation s’applique aux flux de données activés et désactivés.
