---
keywords: Experience Platform;accueil;rubriques les plus consultées;système d’automatisation du marketing;collecter des données d’automatisation du marketing
solution: Experience Platform
title: Collecte de données d’automatisation du marketing à l’aide des connecteurs source et des API
...
Ce tutoriel nécessite que vous ayez accès à un système d’automatisation marketing tiers par le biais d’une connexion valide et d’informations sur le fichier que vous souhaitez importer dans Platform, y compris le chemin d’accès et la structure du fichier. Si vous ne disposez pas de ces informations, consultez le tutoriel sur [l’exploration d’un système tiers d’automatisation du marketing à l’aide de l’API Flow Service](../explore/marketing-automation.md) avant de lancer ce tutoriel.
...
| -------- | ----------- |
| `baseConnectionId` | L’identifiant de connexion unique du système d’automatisation du marketing tiers auquel vous accédez. |
| `params.path` | Le chemin d’accès au fichier source auquel vous accédez. |
| `connectionSpec.id` | Identifiant de spécification de connexion de votre système d’automatisation marketing. |
...
Une réponse réussie renvoie les détails du schéma nouvellement créé, y compris son identifiant unique (`$id`). Conservez cet identifiant car il est nécessaire lors des étapes suivantes pour créer un jeu de données cible, un mappage et un flux de données.
...
| Propriété | Description |
| -------- | ----------- |
| `schemaRef.id` | `$id` du schéma XDM cible. |
...
Pour que les données source soient ingérées dans un jeu de données cible, elles doivent d’abord être mappées au schéma cible auquel le jeu de données cible adhère. Pour ce faire, il vous suffit d’adresser une requête de POST à l’API [!DNL Conversion Service] avec des mappages de données définis dans le payload de la requête.
...
Une réponse réussie renvoie les détails du nouveau mappage, y compris son identifiant unique (`id`). Conservez cette valeur, car elle sera nécessaire à l’étape suivante pour créer un flux de données.
...
Un flux de données est chargé de collecter des données à partir de sources et de les importer dans Platform. Pour créer un flux de données, vous devez d’abord obtenir les spécifications du flux de données responsables de la collecte des données d’automatisation du marketing.
...
La dernière étape de la collecte des données d’automatisation du marketing consiste à créer un flux de données. À l’heure actuelle, les valeurs requises suivantes sont préparées :
...
| `transformations.params.deltaColum` | Colonne désignée utilisée pour différencier les données nouvelles des données existantes. Les données incrémentielles seront ingérées en fonction de l’horodatage de la colonne sélectionnée. Le format de date pris en charge pour `deltaColumn` est `yyyy-MM-dd HH:mm:ss`. |
...
En suivant ce tutoriel, vous avez créé un connecteur source pour collecter les données d’un système d’automatisation du marketing selon un calendrier précis. Les données entrantes peuvent désormais être utilisées par les services Platform en aval tels que [!DNL Real-time Customer Profile] et [!DNL Data Science Workspace]. Pour plus d’informations, consultez les documents suivants :
