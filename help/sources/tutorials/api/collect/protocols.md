...
Ce tutoriel décrit les étapes à suivre pour récupérer des données d’une source de protocoles et les importer dans Platform à l’aide de [[!DNL Flow Service] l’API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>Pour créer un flux de données, vous devez déjà disposer d’un identifiant de connexion de base valide auprès de l’une des sources de protocoles suivantes sur Platform :<ul><li>[OData générique](../create/protocols/odata.md)</li><li>[REST générique](../create/protocols/generic-rest.md)</li></ul>
...
Un flux de données est chargé de collecter des données à partir de sources et de les importer dans Platform. Pour créer un flux de données, vous devez d’abord obtenir les spécifications du flux de données responsables de la collecte de données de votre application de protocoles.
