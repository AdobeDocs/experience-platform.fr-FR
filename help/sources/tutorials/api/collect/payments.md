...
title: Collecte de données de paiement à l’aide des connecteurs source et des API
...
Ce tutoriel nécessite que vous ayez accès à une demande de paiement via une connexion valide, ainsi qu’à des informations sur le fichier que vous souhaitez importer dans Platform (y compris le chemin et la structure du fichier). Si vous ne disposez pas de ces informations, consultez le tutoriel sur [l’exploration d’une application de paiements à l’aide de l’API Flow Service](../explore/payments.md) avant de lancer ce tutoriel.
...
| -------- | ----------- |
| `baseConnectionId` | L’identifiant de connexion unique de l’application de paiements tierce à laquelle vous accédez. |
