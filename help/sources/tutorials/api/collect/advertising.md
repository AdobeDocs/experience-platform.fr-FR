...
title: Collecte de données publicitaires via des connecteurs source et des API
topic: overview
...
# Collecte de données publicitaires via des connecteurs source et des API

Ce didacticiel décrit les étapes à suivre pour récupérer les données d’un système publicitaire et les amener à la plate-forme par le biais des connecteurs source et des API.

## Prise en main

Ce didacticiel nécessite que vous disposiez d’informations sur le fichier que vous souhaitez importer dans Platform, y compris le chemin et la structure du fichier. Si vous ne disposez pas de ces informations, reportez-vous au didacticiel sur l’ [exploration d’une application publicitaire à l’aide de l’API](../../api/create/advertising/ads.md) de service de flux avant d’essayer ce didacticiel.
...
Les sections suivantes fournissent des informations supplémentaires dont vous aurez besoin pour vous connecter à un système publicitaire à l’aide de l’API de service de flux.
...
| -------- | ----------- |
| `baseConnectionId` | ID de connexion de votre application publicitaire |
| `data.schema.id` | Le  `$id` du XDM ad hoc. |
| `params.path` | Chemin d’accès du fichier source. |
| `connectionSpec.id` | ID de spécification de connexion de votre application publicitaire. |
...
| `connectionSpec.id` | ID de spécification de connexion pour votre publicité. |
...
Un flux de données est responsable de la collecte de données à partir de sources et de leur transmission dans la plateforme. Pour créer un flux de données, vous devez d’abord obtenir les spécifications du flux de données qui sont responsables de la collecte des données publicitaires.
...
Une réponse réussie renvoie les détails de la spécification de flux de données responsable de l’importation des données de votre système publicitaire dans Platform. Stocker la valeur du `id` champ comme elle est requise à l’étape suivante pour créer un nouveau flux de données.
...
La dernière étape vers la collecte des données publicitaires est de créer un flux de données. Les valeurs requises suivantes sont désormais préparées :
...
En suivant ce didacticiel, vous avez créé un connecteur source pour collecter les données d’un système publicitaire sur une base planifiée. Les données entrantes peuvent désormais être utilisées par les services Plateforme en aval, tels que les  de clients en temps réel et l’espace de travail des sciences de données. Pour plus d’informations, reportez-vous au  suivant :
