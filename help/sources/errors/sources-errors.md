...
| Code d’erreur | Titre | Message détaillé |
| --- | --- | --- |
| `1100-400` | Requête incorrecte | La requête n’a pas pu être traitée. {detailedMessage} |
...
| `1105-401` | Non autorisé | La personne utilisatrice n’est pas autorisée à effectuer cette opération. {detailedMessage} |
...
| `1304-400` | Requête incorrecte | Le champ obligatoire {field} n’est pas fourni. Indiquez le champ {field} et réessayez. |
...
| `1309-400` | Requête incorrecte | La transformation de chiffrement fournie n’est pas valide, car le paramètre {requiredParam} est manquant dans les paramètres. Indiquez le paramètre {requiredParam} et réessayez. |
...
| 1312-400 | Requête incorrecte | La valeur du paramètre fourni {paramValue} n’est pas une entrée valide pour la propriété={requestParam}. Indiquez une valeur de paramètre valide et réessayez. |
...
| `1347-400` | Requête incorrecte | La définition « dls:csvDelimiters » dans « odi.json » n’est pas prise en charge. Les csvDelimiters pris en charge sont les suivants : [ et ]. Indiquez les définitions appropriées dans « odi.json » et réessayez. |
...
| `1379-400` | Requête incorrecte | Une erreur s’est produite lors de la validation de l’entrée {value} pour le filtre à la source. Le type de données de la colonne à la source {columnDataType} et la valeur de [Chaîne] du type de données ne correspondent pas. Indiquez une valeur {value} valide et réessayez. |
