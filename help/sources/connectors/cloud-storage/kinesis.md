---
title: Présentation du connecteur Source Kinesis Amazon
...
Adobe Experience Platform fournit une connectivité native aux fournisseurs de cloud tels qu’AWS, [!DNL Google Cloud Platform] et [!DNL Azure]. Vous pouvez importer vos données de ces systèmes dans [!DNL Platform].
...
Actuellement, une source d’interface utilisateur [!DNL Kinesis] ne prend en charge que les `TRIM_HORIZON`, tandis que l’API prend en charge les modes `TRIM_HORIZON` et `LATEST` comme modes d’obtention de données. La valeur d’itérateur par défaut que Platform utilise pour la source de [!DNL Kinesis] est `TRIM_HORIZON`.
...
## Connexion de [!DNL Amazon Kinesis] à [!DNL Platform]

La documentation ci-dessous fournit des informations sur la connexion de [!DNL Amazon Kinesis] à [!DNL Platform] à l’aide d’API ou de l’interface utilisateur :
