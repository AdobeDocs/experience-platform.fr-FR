...
title: Mappage des champs pour Adobe Analytics Source Connector
description: Mappez les champs Adobe Analytics aux champs XDM à l’aide d’Analytics Source Connector.
...
| --------------- | --------- | -------- | ---------- |
| `m_evar1`<br/>`[...]`<br/>`m_evar250` | `_experience.analytics.customDimensions.`<br/>`eVars.eVar1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`eVars.eVar250` | chaîne | eVars Analytics personnalisées. Chaque organisation peut utiliser les eVars différemment. |
| `m_prop1`<br/>`[...]`<br/>`m_prop75` | `_experience.analytics.customDimensions.`<br/>`props.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`props.prop75` | Chaîne | Props Analytics personnalisées. Chaque organisation peut utiliser les props différemment. |
...
| `m_domain` | `environment.domain` | chaîne | Variable utilisée dans la dimension Domaine. Il est basé sur le fournisseur d’accès Internet (FAI) de l’utilisateur. |
...
| `mobileplaceaccuracy` | `placeContext.POIinteraction.POIDetail.`<br/>`geoInteractionDetails.deviceGeoAccuracy` | nombre | Collecté à partir de la variable a.loc.acc des données de contexte. Indique la précision du GPS en mètres au moment de la collecte. |
| `mobileplacecategory` | `placeContext.POIinteraction.POIDetail.`<br/>`category` | chaîne | Collecté à partir de la variable a.loc.category des données de contexte. Décrit la catégorie d’un lieu spécifique. |
...
| `mobileinstalls` | `application.firstLaunches` | Objet | Il est déclenché lors de la première exécution après installation ou réinstallation. | {id (string), value (number)} |
...
## Champs de fractionnement des mappages
...
Certains champs provenant d’ADC doivent être transformés, ce qui nécessite une logique au-delà d’une copie directe d’Adobe Analytics pour être générés dans XDM.

| Champ Analytics | Champ XDM | Type XDM | Description |
| --------------- | --------- | -------- | ----------- |
| `m_prop1`<br/>`[...]`<br/>`m_prop75` | `_experience.analytics.customDimensions`<br/>`.listprops.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`listprops.prop75` | Objet | Props Analytics personnalisées, configurées comme props de liste. Il contient une liste délimitée de valeurs. | {} |
...
| `m_color` | `device.colorDepth` | Entier | Identifiant de profondeur de couleur, basé sur la valeur de la colonne c_color. |
...
| `m_geo_country` | `placeContext.geo.countryCode` | Chaîne | Abréviation du pays d’où provient l’accès, basée sur l’adresse IP. |
...
| `m_ref_type` | `web.webReferrer.type` | chaîne | Identifiant numérique représentant le type de référence pour l’accès.<br/>`1` : Dans votre site<br/>`2` : autres sites web<br/>`3` : moteurs de recherche<br/>`4` : disque dur<br/>`5` : USENET<br/>`6` : Tapé/Marqué (aucun référent)<br/>`7` : courrier électronique<br/>`8` : aucun JavaScript<br/>`9` : réseaux sociaux |
...
| `hit_time_gmt` | `receivedTimestamp` | Chaîne | Horodatage de l’accès, basé sur l’heure UNIX®. |
...
Certains champs (appelés &quot;valeurs de publication&quot;) contiennent des données une fois que l’Adobe a ajusté leurs valeurs à l’aide des règles de traitement, des règles VISTA et des tables de recherche. La plupart des valeurs de publication ont une contrepartie prétraitée. Votre entreprise peut décider d’utiliser les champs prétraités, les champs post-traités, ou les deux.

Pour en savoir plus sur l’exécution de ces transformations à l’aide de Query Service, voir [Fonctions définies par Adobe](/help/query-service/sql/adobe-defined-functions.md) dans le guide d’utilisation de Query Service.

| Champ Analytics | Champ XDM | Type XDM | Description |
| --------------- | --------- | -------- | ---------- |
| `post_evar1`<br/>`[...]`<br/>`post_evar250` | `_experience.analytics.customDimensions.`<br/>`eVars.eVar1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`eVars.eVar250` | chaîne | eVars Analytics personnalisées. Chaque organisation peut utiliser les eVars différemment. |
| `post_prop1`<br/>`[...]`<br/>`post_prop75` | `_experience.analytics.customDimensions.`<br/>`props.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`props.prop75` | Chaîne | Props Analytics personnalisées. Chaque organisation peut utiliser les props différemment. |
...
| `domain` | `environment.domain` | chaîne | Variable utilisée dans la dimension Domaine. Il est basé sur le fournisseur d’accès Internet (FAI) de l’utilisateur. |
...
| `post_prop1`<br/>`[...]`<br/>`post_prop75` | `_experience.analytics.customDimensions.`<br/>`listprops.prop1`<br/>`[...]`<br/>`_experience.analytics.customDimensions.`<br/>`listprops.prop75` | Objet | Props Analytics personnalisées, configurées comme props de liste. Il contient une liste délimitée de valeurs. |
...
| `first_hit_time_gmt` | `_experience.analytics.endUser.`<br/>`firstTimestamp` | Entier | Horodatage du premier accès du visiteur à l’heure UNIX®. |
