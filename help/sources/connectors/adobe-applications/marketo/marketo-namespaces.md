...
# (Version bêta) Espaces de noms et schémas B2B

>[!IMPORTANT]
>
>Cette fonctionnalité est actuellement en version bêta. La documentation et la fonctionnalité peuvent changer.
...
- Vous pouvez télécharger la collection et l’environnement de l’utilitaire de génération automatique de l’espace de noms et du schéma à partir de ce [référentiel GitHub](https://github.com/adobe/experience-platform-postman-samples/tree/master/Postman%20Collections/CDP%20Namespaces%20and%20Schemas%20Utility).
- Pour plus d’informations sur l’utilisation des API Platform, notamment sur la manière de rassembler des valeurs pour les en-têtes requis et de lire des exemples d’appels API, consultez le guide sur la [prise en main des API Platform](../../../../landing/api-guide.md).
...
- Pour plus d’informations sur la configuration de [!DNL Postman] pour les API Platform, consultez le tutoriel sur la [configuration de Developer Console et [!DNL Postman]](../../../../landing/postman.md).

Avec une console de développement Platform et une configuration [!DNL Postman] , vous pouvez maintenant commencer à appliquer les valeurs d’environnement appropriées à votre environnement [!DNL Postman].
...
| --- | --- | --- |
| `CLIENT_SECRET` | Identifiant unique utilisé pour générer votre `{ACCESS_TOKEN}`. Consultez le tutoriel sur [l’authentification et l’accès aux API Experience Platform](../../../../landing/api-authentication.md) pour plus d’informations sur la manière de récupérer votre `{CLIENT_SECRET}`. | `{CLIENT_SECRET}` |
| `JWT_TOKEN` | Le jeton Web JSON (JWT) est un identifiant d’authentification utilisé pour générer votre {ACCESS_TOKEN}. Consultez le tutoriel sur [l’authentification et l’accès aux API Experience Platform](../../../../landing/api-authentication.md) pour plus d’informations sur la génération de votre `{JWT_TOKEN}`. | `{JWT_TOKEN}` |
| `API_KEY` | Identifiant unique utilisé pour authentifier les appels vers les API Experience Platform. Consultez le tutoriel sur [l’authentification et l’accès aux API Experience Platform](../../../../landing/api-authentication.md) pour plus d’informations sur la manière de récupérer votre `{API_KEY}`. | `c8d9a2f5c1e03789bd22e8efdd1bdc1b` |
| `ACCESS_TOKEN` | Jeton d’autorisation requis pour terminer les appels vers les API Experience Platform. Consultez le tutoriel sur [l’authentification et l’accès aux API Experience Platform](../../../../landing/api-authentication.md) pour plus d’informations sur la manière de récupérer votre `{ACCESS_TOKEN}`. | `Bearer {ACCESS_TOKEN}` |
...
| `CONTAINER_ID` | Le conteneur `global` contient toutes les classes d’Adobe standard et de partenaire Experience Platform fournies, les groupes de champs de schéma, les types de données et les schémas. En ce qui concerne [!DNL Marketo], cette valeur est fixe et toujours définie sur `global`. | `global` |
...
| `IMS` | Le système Identity Management (IMS) fournit la structure d’authentification des services Adobe. En ce qui concerne [!DNL Marketo], cette valeur est fixe et toujours définie sur : `ims-na1.adobelogin.com`. | `ims-na1.adobelogin.com` |
| `IMS_ORG` | Personne morale pouvant posséder ou accorder une licence pour des produits et des services et permettre l’accès à ses membres. Consultez le tutoriel sur la [configuration de Developer Console et [!DNL Postman]](../../../../landing/postman.md) pour obtenir des instructions sur la récupération de vos `{IMS_ORG}` informations. | `ABCEH0D9KX6A7WA7ATQE0TE@adobeOrg` |
...
Une fois votre collection et votre environnement [!DNL Postman] configurés, vous pouvez exécuter le script via l’interface [!DNL Postman].
...
Les espaces de noms d’identité sont des composants de [[!DNL Identity Service]](../../../../identity-service/home.md) qui servent à distinguer le contexte ou le type d’une identité. Une identité complète est composée d’une valeur d’identifiant et d’un espace de noms. Pour plus d’informations, consultez la [présentation des espaces de noms](../../../../identity-service/namespaces.md) .
