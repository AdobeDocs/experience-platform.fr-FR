...
| `TECHNICAL_ACCOUNT_ID` | Informations d’identification utilisées pour l’intégration à [&#128279;](https://developer.adobe.com/). Pour récupérer votre identifiant de compte technique, procédez comme suit :<ul><li>Accédez à [&#128279;](https://developer.adobe.com/) et ouvrez votre projet.</li><li>Ouvrez le **informations d’identification** qui utilise le _compte de service (JWT)_. **Remarque** : il peut être appelé « Entreprise » dans l’ancienne interface utilisateur.</li><li>Les informations d’identification doivent afficher votre ID client, votre ID de compte technique, votre ID d’organisation et d’autres métadonnées JWT.</li></ul>. | `D42AEVJZTTJC6LZADUBVPA15@techacct.adobe.com` |
...
| `PLATFORM_URL` | Point d’entrée de l’URL vers lequel vous effectuez des appels API. Cette valeur est fixe et est toujours définie sur : `platform.adobe.io/`. | `platform.adobe.io/` |
