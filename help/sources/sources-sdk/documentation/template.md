...
# Créer une connexion *YOURSOURCE* à l’aide de l’API [!DNL Flow Service]
...
Le tutoriel suivant vous guide tout au long des étapes de création d’une connexion source *YOURSOURCE* et de création d’un flux de données pour importer les données *YOURSOURCE* dans Experience Platform à l’aide de l’[[!DNL Flow Service] API](https://developer.adobe.com/experience-platform-apis/references/flow-service).
...
Pour créer un identifiant de connexion de base, envoyez une requête POST au point d’entrée `/connections` et indiquez vos informations d’authentification *YOURSOURCE* dans le corps de la requête.
...
| `{SOURCE_PARAMS}` | Définit les paramètres du fichier source que vous souhaitez importer dans Experience Platform. Pour récupérer le type de format accepté pour `{SOURCE_PARAMS}`, vous devez coder l’intégralité de la chaîne `list_id` en base64. Dans l’exemple ci-dessous, `"list_id": "10c097ca71"` codé en base64 équivaut à `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`. |
...
| `baseConnectionId` | Identifiant de connexion de base de *YOURSOURCE*. Cet identifiant a été généré lors d’une étape précédente. |
