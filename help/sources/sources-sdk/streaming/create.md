---
title: Créer une spécification de connexion pour Streaming SDK à l’aide de l’API Flow Service
...
Le document suivant décrit la procédure à suivre pour créer une spécification de connexion à l’aide de l’API [!DNL Flow Service] et intégrer une nouvelle source via des sources en libre-service (streaming SDK).
...
Pour créer une source de diffusion en continu à l’aide de sources en libre-service, vous devez d’abord vous coordonner avec Adobe, demander un référentiel Git privé et vous aligner avec Adobe sur les détails concernant le libellé, la description, la catégorie et l’icône de votre source.
...
| --- | --- | --- |
| {your_source} | Nom de votre source. Ce dossier doit contenir tous les artefacts associés à votre source, dans votre référentiel Git privé. | `medallia` |
| {your_source}-category.txt | La catégorie à laquelle votre source appartient, au format texte. **Remarque** : si vous pensez que votre source ne correspond à aucune des catégories ci-dessus, contactez votre représentant Adobe pour en discuter. | `medallia-category.txt` Dans le fichier , indiquez la catégorie de votre source, par exemple : `streaming`. |
...
| {your_source}-connectionSpec.json | Un fichier JSON contenant la spécification de connexion de votre source. Ce fichier n’est pas requis au départ, car vous allez renseigner votre spécification de connexion au fur et à mesure que vous terminez ce guide. | `medallia-connectionSpec.json` |
...
Une spécification de connexion peut être divisée en deux parties distinctes : les spécifications de la source et les spécifications d’exploration.
