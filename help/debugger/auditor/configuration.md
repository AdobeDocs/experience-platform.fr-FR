...
| Adobe Advertising - Les noms de conversion utilisent des caractères URL sécurisés | 3 | Les noms des propriétés de conversion ne doivent pas contenir d’esperluette ni de point d’interrogation. | Assurez-vous que les paramètres de propriété de transaction ne contiennent pas d’esperluette ni de point d’interrogation non codé. Ces caractères rompent le format d’URL.<br><br>Avertissement : les paramètres de propriété contenant une esperluette ou un point d&#39;interrogation non codé (par exemple : `ev_formComplete?=1` ou `ev_formComplete&Submit=1`) peuvent entraîner une perte de données. |
