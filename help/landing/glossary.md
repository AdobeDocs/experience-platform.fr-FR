...
**Action :** Dans le lancement de la plateforme d’expérience, une action est un type spécifique de composant de règle qui définit ce qui doit se produire une fois qu’un événement s’est produit et que les conditions ont été évaluées et transmises.

**Activer :** Dans la plate-forme de données client en temps réel, activer est l’action entreprise par un utilisateur pour mapper un segment ou des profils à une destination telle qu’Oracle Eloqua, Google ou Salesforce Marketing Cloud.
...
**Connecteur Amazon S3 :** Le connecteur Amazon S3 permet aux clients d’Experience Platform de se connecter et d’accéder en toute sécurité à leurs données Amazon S3.

**Ajouter une stratégie d&#39;enregistrement :** La stratégie d’ `Append` enregistrement est une option utilisée lors de la spécification de données tierces à assimiler via une connexion et de l’ajout de toute nouvelle donnée ou ligne à la fin du jeu de données. Les lignes précédemment ingérées restent intactes et seules les lignes créées depuis la dernière exécution planifiée sont assimilées à la plate-forme d’expérience. Toutes les lignes modifiées dans le système source restent inchangées sur la plateforme d’expérience.
...
**Attribution AI :** Attribution AI est un service Adobe Sensei qui offre des fonctions d’attribution algorithmique à plusieurs canaux sur l’ensemble du cycle de vie du client.
...
**Renvoi :** Dans la plate-forme de données client en temps réel, dans les connexions source planifiées, le renvoi permet l’assimilation de données historiques.

**Période de renvoi :** `Backfill period` est une option permettant de définir la durée d’importation des données historiques tierces via une connexion. La sélection d’une période de renvoi indéfiniment assimilera l’historique complet des données source à la plateforme d’expérience.
...
**Ingestion par lots :** L’assimilation de lots permet aux utilisateurs d’assimiler des pétaoctets de données et de les rendre disponibles dans les systèmes d’entreprise. Grâce aux technologies les plus récentes, les utilisateurs peuvent désormais intégrer n’importe quel schéma XDM et non XDM dans la plate-forme d’expérience.
...
**Créer :** Dans le lancement de la plate-forme d’expérience, une build est une bibliothèque déployée. La compilation est un fichier ou un ensemble de fichiers qui contient toutes les configurations et le code nécessaires pour exécuter la logique métier contenue dans cette bibliothèque.

**Outils de veille stratégique :** Les outils de Business Intelligence, également connus sous le nom d’outils &quot;BI&quot;, sont principalement intégrés au service Experience Platform Requête. Les outils BI sont des types de logiciels d&#39;application qui collectent et traitent de grandes quantités de données non structurées provenant de systèmes internes et externes.

[Retour au début](#adobe-experience-platform-glossary)

## C 

**Capping :** Dans le service de prise de décision, le plafonnement est utilisé dans les règles de prise de décision pour définir le nombre de présentations d’une offre. Il existe deux types de casquettes : le nombre de fois où une offre peut être proposée dans l&#39;audience combinée de cible, également appelée &quot;casquette globale&quot;, et le nombre de fois où une offre peut être proposée au même utilisateur final, également appelé &quot;casquette de Profil&quot;.

**Catalogue :** Dans la plate-forme de données client en temps réel, dans les sources et les destinations, un catalogue est une galerie avec des connexions disponibles aux applications Adobe et aux technologies tierces.
...
**Client :** Un client est un outil ou une application externe qui se connecte à Requête Service via le protocole postgres ou l’API HTTP.

**Collection :** Dans le service de prise de décision, les collections sont des sous-ensembles d’offres basés sur des conditions prédéfinies définies par un spécialiste du marketing, telles que la catégorie de l’offre.

**Interface de ligne de commande :** L’interface de ligne de commande est un outil de ligne de commande utilisé pour se connecter à Requête Service pour l’exécution de requêtes brutes.
...
**Console :** Dans Requête Service, la console fournit des informations sur l’état et le fonctionnement d’une Requête. La console affiche l&#39;état de la connexion à Requête Service, les opérations de requête en cours d&#39;exécution et les messages d&#39;erreur qui en résultent.
...
**Créer un tableau comme sélection :** La commande Create Table as Select est une commande SQL qui, lorsqu&#39;elle est exécutée dans le cadre d&#39;une requête SQL complète et valide, demande au service de Requête de conserver les résultats de la requête dans un jeu de données sur le lac Data. Les options incluent : Créer, Remplacer tout le précédent et Ajouter au précédent.
...
**Dictionnaire de données :** Dans le lancement de la plateforme d’expérience, un dictionnaire de données est un ensemble d’éléments de données définis dans une propriété.

**Elément de données :** Dans le lancement de la plateforme d’expérience, un élément de données est un pointeur utilisé dans les règles et extensions pour pointer vers un élément de données spécifique existant sur le périphérique client.

**Couche de données :** Dans le lancement de la plateforme d’expérience, une couche de données est une structure de données qui existe sur le périphérique client et qui contient des métadonnées sur le contexte dans lequel une page ou un écran est affiché.
...
**Gouvernance des données :** La gouvernance des données englobe les stratégies et les technologies utilisées pour s&#39;assurer que les données sont conformes aux règlements et aux politiques de l&#39;organisation en matière d&#39;utilisation des données.

**Étiquettes de gouvernance des données :** Les étiquettes de gouvernance des données permettent aux utilisateurs de classifier les données qui reflètent les considérations liées à la vie privée et les conditions contractuelles afin de se conformer aux règlements et aux politiques de l’entreprise. Les étiquettes de gouvernance des données ajoutées à un jeu de données sont héritées ou appliquées à tous les champs de ce jeu de données.  Les étiquettes de gouvernance des données peuvent également être appliquées directement aux champs.
...
**Flux de données :** Dans la plate-forme de données client en temps réel, un flux de données est un pipeline virtuel de données qui s’écoule dans la plate-forme à partir d’une source et vers des destinations.
...
**Sortie du jeu de données :** La sortie d&#39;un jeu de données fournit un mécanisme permettant de déterminer ce que l&#39;option *Créer une table comme sélection* sera utilisée pour une exécution de Requête Service particulière.
...
**Règle de décision :** Dans le service de prise de décision, une règle de décision est la logique qui définit et contrôle ce qui, quand, où et comment une offre est présentée aux utilisateurs finaux.

**Service de prise de décision :** Le service de prise de décision est un ensemble de services et d’interfaces utilisateur qui permet aux spécialistes du marketing de créer et de fournir des expériences d’offre personnalisées pour les utilisateurs finaux sur les canaux et les applications à l’aide d’une logique métier et de règles de décision.

**Colonne delta :** Dans la plate-forme de données client en temps réel, la colonne delta permet la sélection du champ de données source pour un horodatage en vue de l’assimilation incrémentielle.
...
**Catalogue de destination :** Un catalogue de destination est une liste de destinations disponibles dans la plateforme de données clientes en temps réel.
...
**Offre éligible :** Dans le Service de prise de décision, une offre éligible satisfait aux contraintes définies en amont qui peuvent être offertes de manière cohérente à un profil.

**Règles d&#39;éligibilité :** Dans le service de prise de décision, les règles d&#39;éligibilité sont appliquées à un profil lié aux contraintes de calendrier, de planification et de plafonnement.

**Code incorporé :** Dans le lancement de la plate-forme d’expérience, le code incorporé est une balise de script placée dans le code HTML sur un site ou un environnement. Le code incorporé indique au navigateur où récupérer la compilation.

**Énumération :** Un énumération est une liste de valeurs qui représente les données valides d’un champ.

**Environnement :** Dans le lancement de la plateforme d’expérience, un environnement est un ensemble d’instructions de déploiement qui spécifie la diffusion hôte et le format de fichier d’une build. Une bibliothèque doit être associée à un environnement avant de pouvoir être créée.

**Événement** dans le lancement de la plateforme d’expérience, un événement est un type spécifique de composant de règle, un déclencheur qui se produit sur un périphérique client pour commencer l’exécution d’une règle.
...
**Extensions de lancement de plateforme d’expérience :** Les extensions de lancement de plate-forme d’expérience permettent la diffusion de données de événement brutes directement vers les destinations de la plate-forme de données client en temps réel. L’installation des extensions de lancement nécessite l’accès aux propriétés de lancement.
...
**ExperienceEvent :** ExperienceEvent est un schéma standard de la plateforme d’expérience qui capture les observations, y compris le moment et l’identité du sujet concerné. Les Événements d&#39;expérience sont des enregistrements factuels de ce qui s&#39;est passé, représentant ce qui s&#39;est passé sans agrégation ni interprétation.

**Extension :** Dans le lancement de la plateforme d’expérience, une extension est un package de fonctionnalités ajouté à une propriété Launch.  Une extension est généralement axée sur une solution marketing ou d’analyse particulière et fournit les outils nécessaires pour déployer cette technologie dans un environnement client.

**Package d’extension :** Dans le lancement de la plateforme d’expérience, un package d’extension est un fichier .zip créé et téléchargé par un développeur d’extensions qui fournit tout ce qui est nécessaire pour que les utilisateurs du lancement installent l’extension dans leur propriété.  Un package d&#39;extension contient un manifeste spécifiant les informations relatives à l&#39;extension, au code HTML et au code JavaScript nécessaires pour que les utilisateurs finaux puissent configurer le comportement de l&#39;extension Launch et du code JavaScript exécutable envoyé à l&#39;environnement client, si nécessaire.

[Retour au début](#adobe-experience-platform-glossary)

## f

**Offres de secours :** Dans le service de prise de décision, une offre de secours est l’offre par défaut affichée lorsqu’un utilisateur final n’est éligible à aucune des offres de la collection utilisée.
...
**Fréquence :** La fréquence détermine la fréquence d’exécution d’une requête de service de Requête planifiée récurrente.
...
**Hôte :** Dans le lancement de la plate-forme d’expérience, un hôte spécifie l’emplacement, le domaine et les informations d’identification d’utilisateur nécessaires au lancement pour diffuser une build.
...
**Espace de nommage d&#39;identité :** Un espace de nommage d’identité est un identifiant tel que l’ID de cookie, l’ID d’appareil ou l’ID de courrier électronique pour indiquer le contexte d’où proviennent les données et est utilisé pour reconnaître et lier les identités dans Experience Cloud.

**Service d&#39;identité :** L’interface utilisateur Experience Platform Identity Service permet la création et la gestion de types d’identité afin de permettre la liaison d’identités entre les périphériques et les canaux pour une vue utilisateur complète à partir du Profil client en temps réel.
...
**Envoi :** L’importation est le processus d’ajout de données d’une source à la plateforme d’expérience. Les données peuvent être assimilées à la plate-forme d’expérience de plusieurs manières, y compris en flux continu, par lots ou ajoutées via un connecteur.

**Planification de l&#39;importation :** La planification de l’importation fournit des options temporelles lors de l’importation d’une source vers la plateforme d’expérience.
...
**Services intelligents :** Les services intelligents tels que Attribution.ai et Customer.ai sont des modèles d’apprentissage automatique basés sur l’intelligence artificielle conçus de manière ciblée qui nécessitent que la plateforme d’expérience s’exécute et fonctionne.
...
**JupyterLab :** Interface Web open-source pour Project Jupyter et étroitement intégrée à Experience Platform.

**Jupyter Notebook :** Application Web open source qui permet aux utilisateurs de créer et de partager des documents contenant du code en direct, des équations, des visualisations et du texte narratif.
...
**Bibliothèque :** Dans le lancement d’une plateforme d’expérience, une bibliothèque est un ensemble de logiques métier qui contient des instructions sur le comportement de la bibliothèque de lancement sur le périphérique client.
...
**Fusionner la stratégie :** Une stratégie de fusion est un ensemble de règles utilisées par le Profil pour déterminer comment les données seront hiérarchisées et combinées en une vue unifiée dans certaines conditions.
...
**Module :** Dans le lancement de la plateforme d’expérience, un module est un fragment de code JavaScript exécutable fourni par une extension, qui exécute des actions dans un environnement client sans que l’utilisateur de lancement ait besoin de créer une règle.
...
**Ordinateurs portables :** Les portables sont créés à l&#39;aide de *Jupyter Notebook* et contiennent une description de l&#39;analyse, des résultats et peuvent être exécutés pour effectuer l&#39;analyse des données.

[Retour au début](#adobe-experience-platform-glossary)

## o

**Offre :** Dans le service de prise de décision, une offre est un message marketing auquel des règles peuvent être associées et qui indique qui peut être autorisé à voir l’offre.

**Décision sur l&#39;Offre :** Dans le service de prise de décision, la prise de décision par offre permet à un spécialiste du marketing de gérer les règles et les modèles de Propositions d&#39;offre formés lors de l’interaction avec un utilisateur final en fonction des données collectées sur plusieurs canaux et applications.
...
**Offres personnalisées :** Dans le service de prise de décision, une offre personnalisée est un message marketing personnalisable basé sur des règles d&#39;éligibilité et des contraintes.

**Emplacements :** Dans le service de prise de décision, un emplacement correspond à l’emplacement ou au contexte dans lequel une offre s’affiche pour un utilisateur final.

**Stratégie :** Une stratégie d’utilisation des données est une règle qui spécifie les actions marketing qui sont limitées en fonction de l’application d’étiquettes d’utilisation des données sur les données dans la plate-forme d’expérience.

**Clé principale :** La clé principale est une désignation dans un schéma pour identifier de manière unique tous les enregistrements.

**Priorité :** Dans le service de prise de décision, la priorité est utilisée pour classer les offres qui répondent à toutes les contraintes, telles que l’éligibilité, le calendrier et le plafonnement.
...
**Profil du produit :** Les profils de produits permettent aux administrateurs d’accorder à l’utilisateur l’accès à l’ensemble ou à un sous-ensemble de services associés à la plate-forme d’expérience.
...
**Propriété :** Dans le lancement de la plateforme d’expérience, une propriété est un conteneur pour tout ce qui est nécessaire pour déployer un ensemble de balises.
...
**Périodicité :** Une périodicité définit si une requête Requête Service doit être exécutée une seule fois ou de manière récurrente.

**Représentation :** Dans le service de prise de décision, une représentation est une information utilisée par un canal, telle que l’emplacement ou la langue d’affichage d’une offre.

**Ressource :** Dans le lancement de la plateforme d’expérience, le terme ressource est un terme générique qui fait référence aux options que l’utilisateur de lancement peut configurer dans l’environnement client, y compris les extensions, les éléments de données et les règles.

**Contrôle d&#39;accès basé sur les rôles :** Le contrôle d&#39;accès basé sur les rôles permet aux administrateurs d’attribuer l’accès et les autorisations aux utilisateurs de la plateforme d’expérience. Les autorisations incluent la possibilité de vue et/ou d’utiliser les fonctionnalités de la plateforme d’expérience, telles que la création de sandbox, la définition de schémas et la gestion de jeux de données.

**Règle :** Dans le lancement de la plateforme d’expérience, une règle est un ensemble de composants de règle définissant un ensemble spécifique de événements, de conditions et d’actions qui doivent être regroupés logiquement.
...
**Sandbox :** Un sandbox est une forme d’isolation des données virtuelles au sein d’une organisation d’utilisateurs sur la plateforme d’expérience.
...
**Descripteur de Schéma :** Le descripteur de Schéma est une métadonnée supplémentaire liée au schéma qui décrit le comportement qui peut être utilisé par la plate-forme d’expérience pour comprendre le comportement de schéma prévu, tel que la relation entre deux schémas.

**Clé d&#39;accès secrète :** Une clé d’accès secrète est une clé Amazon S3 utilisée en association avec l’ID de clé d’accès pour signer des requêtes AWS.
...
**Créateur de segments :** Le créateur de segments est l’environnement de développement visuel utilisé pour créer des définitions de segment et sert de composant commun à toutes les applications à l’aide de la segmentation du Profil client en temps réel sur la plateforme d’expérience.
...
**Type de segment :** Il existe deux types de segments : L’un est un segment qui se met à jour dynamiquement avec les modifications des données de la plateforme d’expérience, et l’autre est un instantané d’audience qui capture toutes les règles de segment de réunion de profils, et celles-ci ne changent pas.
...
**Source :** Source est un terme général pour tout connecteur d’entrée de la plate-forme de données client en temps réel.
...
**Vue système :** La Vue système est une représentation visuelle des jeux de données source qui circulent dans le Profil client en temps réel vers les destinations.
...
**Jeton :** Un jeton est un type de sécurité d’authentification à deux facteurs qui peut être utilisé pour autoriser l’utilisation de services informatiques avec Requête Service.
...
**Schéma Union :** Le schéma d’Union est une consolidation des schémas qui ont été activés pour le Profil client en temps réel.
...
**XDM (modèle de données d’expérience) :** XDM (Experience Data Model) est le concept d’utilisation de schémas standard pour unifier les données à utiliser avec Experience Platform et les applications Adobe Experience Cloud. XDM est une spécification formelle utilisée pour représenter toutes les données d&#39;expérience client dans un seul langage ou un modèle de données standard et normalise la structure des données et accélère et simplifie le processus d&#39;obtention de connaissances à partir de quantités massives de données.
