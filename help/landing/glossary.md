...
**Contrôle d’accès :** le {#access-control}[!DNL Experience Platform] d‘ associe les utilisateurs avec des autorisations d’accès et des environnements de test grâce à des profils de produits dans Adobe Admin Console.

**Identifiant de clé d’accès :**[!DNL Amazon] l’identifiant de clé d’accès est un identifiant unique associé à une clé d’accès secrète S3. L’identifiant de clé d’accès et la clé d’accès secrète sont utilisés conjointement pour signer les requêtes AWS.
...
**Service** de Requête Adobe Experience Platform Permet l&#39;utilisation de données SQL standard pour la requête dans l&#39;Experience Platform, l&#39;association de tous les jeux de données dans Data Lake et la capture des résultats de la requête en tant que nouveau jeu de données à utiliser dans rapports, Data Science Workspace ou le Profil client en temps réel.
...
**Adobe I/O :**[!DNL Experience Platform] Adobe I/O fait partie d’Experience Platform et donne accès à tout ce dont les développeurs ont besoin pour intégrer, étendre et personnaliser Adobe , y compris aux API, aux événements, à la console de développement et aux outils utiles.

**Adobe Sensei** : Adobe Sensei est le cadre d’intelligence artificielle qui alimente [!DNL Experience Platform]. Il fournit également un ensemble de services d’IA qui permet aux marques d’améliorer leur capacité à fournir des expériences client personnalisées en temps réel.

**Compartiment  :**[!DNL Amazon S3] les compartiments S3 sont les conteneurs fondamentaux pour les données stockées dans l’écosystème Amazon. [!DNL Amazon] Les compartiments contiennent des objets, chaque objet est stocké et récupéré à l’aide d’une clé unique attribuée par le développeur.
...
**Action marketing Analytics :** Action marketing qui utilise les données à des fins d’analyse, telles que la mesure, l’analyse et le rapports de l’utilisation par les clients des sites ou applications de votre entreprise.

**Stratégie d’enregistrement d’ajout :** la stratégie d’enregistrement `Append` est une option utilisée lors de la spécification de données tierces à ingérer via une connexion et de l’ajout de nouvelles données ou lignes à la fin du jeu de données. Les lignes précédemment ingérées restent intactes et seules les lignes créées depuis la dernière exécution planifiée sont ingérées dans [!DNL Experience Platform]. Toutes les lignes modifiées dans le système source restent inchangées dans [!DNL Experience Platform].

**Gestion du cycle de vie des applications :** la gestion du cycle de vie des applications permet de créer des environnements virtuels distincts qui peuvent être utilisés pour développer et faire évoluer des applications d’expérience numérique.

**Tableau :** les tableaux sont utilisés pour les éléments triés avec le même type de données.

**Intelligence artificielle :** l’intelligence artificielle est une théorie et un développement de systèmes informatiques capables de réaliser des tâches qui nécessitent normalement l’intelligence humaine, comme la perception visuelle, la reconnaissance vocale, la prise de décision et la traduction entre les langues.

**Attributs :** les attributs sont les caractéristiques spécifiées qui représentent un profil.
...
**Période de renvoi :** la `Backfill period` est une option permettant de définir la durée d’ingestion des données historiques tierces par le biais d’une connexion. La sélection d’une période de renvoi indéfinie entraîne l’ingestion de l’historique complet des données sources vers [!DNL Experience Platform].

**Lot :** un lot est un jeu de données collectées sur une période donnée et traitées ensemble comme une unité.

**Identifiant de lot :** un identifiant de lot est l’identifiant généré par Adobe pour un lot de données.

**Ingestion par lots :** l’ingestion par lots permet aux utilisateurs d’ingérer des pétaoctets de données et de les rendre disponibles dans les systèmes d’entreprise. Grâce aux dernières technologies, les utilisateurs peuvent désormais ingérer n’importe quel schéma, XDM ou non, dans [!DNL Experience Platform].

**Segmentation par lots :** la segmentation par lots est une alternative à un processus de sélection de données en cours et déplace toutes les données de profil à la fois via des définitions de segments afin de produire les audiences correspondantes. Une fois créé, ce segment est enregistré et stocké afin de pouvoir être exporté en vue de son utilisation.
...
**Interface de ligne de commande :**[!DNL Query Service] une interface de ligne de commande est un outil de ligne de commande utilisé pour se connecter à pour l’exécution des requêtes brutes.

**Composition** : une composition est un groupe de composants qui se forment ensemble pour constituer le schéma.

**Connexion :** Une connexion est un pipeline virtuel qui permet aux données d&#39;entrer et de sortir [!DNL Experience Platform]. Les connexions sont désormais remplacées par des sources.

**Connecteur :** Les connecteurs Adobe Experience Platform Source permettent aux utilisateurs d’assimiler facilement des données provenant de plusieurs sources, ce qui leur permet de structurer, d’étiqueter et d’améliorer les données à l’aide de [!DNL Experience Platform Services]. Les données peuvent être ingérées à partir de différentes sources, comme le stockage dans le cloud, les logiciels tiers et les systèmes de gestion de la relation client.

**Condition :** dans Experience Platform Launch, une condition est un composant de règle qui évalue une instruction logique qui doit renvoyer `true` ou `false`. Toutes les conditions doivent être évaluées sur `true` et toutes les conditions d’exception doivent être évaluées sur `false` avant l’exécution des actions de la règle.

**Console :** Dans [!DNL Query Service], la console fournit des informations sur l’état et le fonctionnement d’une Requête. The console displays the connection status to [!DNL Query Service], query operations being executed, and any error messages that result from those queries.

**Étiquettes de données Contrat « C » :** les étiquettes Contrat `C` permettent de classer les données qui ont des obligations contractuelles ou qui sont liées aux politiques de gouvernance des données d’un client.

**Étiquette Contrat C1 :** l’étiquette de gouvernance des données Contrat `C1` indique que les données peuvent uniquement être exportées à partir d’Adobe Experience Cloud dans un formulaire agrégé sans inclure d’identifiants d’appareils ou d’individus. Par exemple, les données provenant des réseaux sociaux.

**Étiquette Contrat C2 :** l’étiquette de gouvernance des données Contrat `C2` spécifie les données qui ne peuvent pas être exportées vers un tiers. Certains fournisseurs de données ont des clauses dans leurs contrats qui interdisent l’exportation de données à partir de l’endroit où elles ont été collectées à l’origine.  Par exemple, les contrats des réseaux sociaux limitent souvent le transfert des données que vous recevez d’eux. L’étiquette C2 est plus restrictive que la C1, qui ne nécessite que l’agrégation et des données anonymes.

**Étiquette Contrat C3 :** l’étiquette de gouvernance des données Contrat `C3` spécifie les données qui ne peuvent être combinées ou utilisées d’une autre manière avec des informations directement identifiables. Certains fournisseurs de données ont des clauses dans leurs contrats qui interdisent la combinaison ou l’utilisation de ces données avec des informations directement identifiables.  Par exemple, les contrats pour les données provenant des réseaux publicitaires, des serveurs de publicités et des fournisseurs de données tiers incluent souvent des interdictions contractuelles spécifiques sur l’utilisation des données directement identifiables.

**Étiquette Contrat C4 :** l’étiquette de gouvernance des données Contrat `C4` indique que les données ne peuvent pas être utilisées pour cibler des publicités ou du contenu, que ce soit sur site ou entre sites. L’étiquette C4 est l’étiquette la plus restrictive puisqu’elle englobe les étiquettes C5, C6 et C7.

**Étiquette Contrat C5 :** l’étiquette de gouvernance des données Contrat `C5` indique que les données ne peuvent pas être utilisées pour cibler des publicités ou du contenu, en fonction des centres d’intérêt et entre sites. Le ciblage en fonction des centres d’intérêt, ou personnalisation, se produit si les trois conditions suivantes sont remplies : les données collectées sur un site sont utilisées pour établir des inférences sur les centres d’intérêt d’un utilisateur, elles sont utilisées dans un autre contexte, comme sur un autre site ou une autre application, et elles sont utilisées pour sélectionner le contenu ou les publicités qui sont diffusés en fonction de ces inférences.

**Étiquette Contrat C6 :** l’étiquette de gouvernance des données Contrat `C6` indique que les données ne peuvent pas être utilisées pour le ciblage des publicités sur site. Les données ne peuvent ni être utilisées pour le ciblage des publicités sur site, y compris la sélection et la diffusion de publicités sur les sites web ou les applications de votre organisation, ni pour évaluer la diffusion et l’efficacité de ces publicités.  Cela inclut l’utilisation de données collectées précédemment sur le site sur les centres d’intérêt des utilisateurs pour sélectionner des publicités, pour traiter les données sur les publicités affichées, le moment et l’endroit où elles ont été affichées, et pour savoir si les utilisateurs ont pris des mesures en rapport avec la publicité, comme cliquer sur une publicité ou faire un achat.

**Étiquette Contrat C7 :** l’étiquette de gouvernance des données Contrat `C7` indique que les données ne peuvent pas être utilisées pour le ciblage de contenu sur site.  Les données ne peuvent ni être utilisées pour le ciblage de contenu sur site, y compris la sélection et la diffusion de contenu sur les sites web ou les applications de votre organisation, ni pour évaluer la diffusion et l’efficacité de ce contenu.  Cela inclut les informations précédemment collectées sur l’intérêt des utilisateurs à sélectionner le contenu, traiter les données sur le contenu affiché, la fréquence ou la durée d’affichage, le moment et l’emplacement d’affichage, et si les utilisateurs ont pris des mesures relatives au contenu, notamment en cliquant sur le celui-ci.

**Étiquette Contrat C8 :** l’étiquette de gouvernance des données Contrat `C8` indique que les données ne peuvent pas être utilisées pour évaluer les sites web ou les applications de votre organisation. Les données ne peuvent pas être utilisées pour mesurer, comprendre et générer des rapports sur l’utilisation des sites ou applications de votre organisation par les utilisateurs. Cela ne comprend pas le ciblage basé sur l’intérêt, qui est la collecte d’informations sur votre utilisation de ce service pour personnaliser par la suite le contenu et/ou la publicité dans d’autres contextes.

**Étiquette Contrat C9 :** l’étiquette de gouvernance des données Contrat `C9` indique que les données ne peuvent pas être utilisées dans les workflows de science des données. Certains contrats prévoient des interdictions explicites sur les données utilisées pour la science des données.  Parfois, cela fait partie de conditions qui interdisent l’utilisation de données pour l’intelligence artificielle (IA), l’apprentissage automatique ou la modélisation.

**Étiquette de contrat C10 :** `C10` l&#39;étiquette de gouvernance des données de contrat spécifie que les données ne peuvent pas être utilisées pour l&#39;activation d&#39;identité assemblé. Certaines stratégies d’utilisation des données limitent l’utilisation de données d’identité assemblées pour la personnalisation. Le libellé C10 est automatiquement appliqué aux segments si leurs stratégies de fusion utilisent l’option &quot;graphique privé&quot;.

**Colonne Date de création :** la sélection d’une colonne `Created Date` est une option lors de la spécification de données tierces via une connexion. Lorsque la stratégie d’enregistrement d’ajout est sélectionnée et que le jeu de données contient plusieurs schémas de date, l’utilisateur doit choisir dans le schéma date/heure disponible pour spécifier une colonne `Created Date` clé. L’option `Created Date` n’est pas disponible lorsque la stratégie d’enregistrement de remplacement est sélectionnée.

**Create Table as Select :**[!DNL Query Service] Create Table as Select est une commande SQL qui, lorsqu’elle est exécutée dans le cadre d’une requête SQL complète et valide, demande à de conserver les résultats de la requête dans un jeu de données du lac de données. Les options incluent : Créer, Remplacer précédents et Ajouter au précédent.
...
**Ciblage sur plusieurs sites Action** marketing Utilise les données pour le ciblage des publicités sur plusieurs sites. La combinaison de données provenant de plusieurs sites, y compris une combinaison de données sur site et de données hors site ou une combinaison de données provenant de plusieurs sources hors site, est appelée données intersites. Les données intersites sont généralement collectées et traitées afin d’en déduire les intérêts des utilisateurs.

**Espaces de noms d’identité personnalisés :** les espaces de noms d’identité personnalisés sont des identifiants créés par le client qui représentent les identités d’une organisation ou d’une entreprise spécifique.

**Libellés personnalisés :** Les étiquettes de gouvernance des données personnalisées permettent aux utilisateurs de créer et d’appliquer des étiquettes spécifiques aux champs de données qui répondent à des besoins spécifiques de l’entreprise.

**Customer AI :** Customer AI est un service Adobe Sensei qui enrichit les profils client par des propensions basées sur l’IA et renforce les efforts de segmentation et de ciblage des clients.
...
**Mappage de données :** le mappage des données est le processus de mappage des champs de données sources aux champs cibles liés à la destination.
...
**Partenaires d’intégration des données :**[!DNL Experience Platform] les partenaires d’intégration des données simplifient et automatisent le chargement et la transformation de volumes très importants de données de plus de 200 sources vers sans devoir coder.
...
** :**[!DNL Data Science Workspace] Data Science Workspace dans permet aux clients de créer des modèles d’apprentissage automatique à l’aide de données dans et les applications Adobe, afin de générer des insights et des prédictions intelligents pour créer des expériences numériques attrayantes pour l’utilisateur final.[!DNL Experience Platform][!DNL Experience Platform]

**Source de données :** une source de données est un utilisateur désigné comme étant à l’origine de données. Une source de données peut être une application mobile, des événements d’expérience ou de profil, des événements de profil de sites web ou un système de gestion de la relation client.
...
**Flux de données :** un flux de données est un jeu ou une collection de messages qui partagent le même schéma et sont envoyés par la même source.

**Type de données :** le type de données est un objet réutilisable avec des propriétés dans une représentation hiérarchique.

**Libellés d’utilisation des données :** les libellés d’utilisation des données permettent aux utilisateurs de classer les données en fonction des considérations liées à la confidentialité et des conditions contractuelles afin qu’elles soient conformes aux réglementations et aux politiques d’entreprise.
...
**Flux de données :** L&#39;exécution de flux de données est un flux de données qui parvient dans l&#39;Experience Platform en fonction d&#39;un calendrier défini par l&#39;utilisateur.

**Jeu de données :** un jeu de données est une structure de stockage et de gestion utilisée pour la collecte de données (généralement sous la forme d’un tableau) qui contient un schéma (des colonnes) et des champs (des lignes).

**Identifiant de jeu de données :** identifiant généré par Adobe pour un jeu de données ingéré.

**Sortie d’un jeu de données :** la sortie d’un jeu de données fournit un mécanisme permettant de déterminer que la commande *Create Table as Select*[!DNL Query Service] (Créer un tableau en tant que sélection) sera utilisée pour une exécution spécifique.
...
**Code incorporé :** Dans [!DNL Experience Platform Launch]le cas présent, le code incorporé est une balise de script placée dans le code HTML sur un site ou un environnement. Le code incorporé indique au navigateur où récupérer la version.
...
**Expérience :** une expérience est un processus de création d’un modèle formé en entraînant l’instance avec une portion d’échantillon des données de production en direct.

**Expériences :** les expériences consistent à appliquer un modèle formé à une petite partie des données de production en direct afin de valider ses performances. Ceci est différent d’un modèle formé testé par rapport à un jeu de données de test d’exclusion. Ceci est également différent du concept d’expérience dans certains frameworks d’apprentissage automatique où cela signifie en fait un exemple de projet de modélisation.
...
**Champ :** un champ est l’élément de niveau le plus bas d’un jeu de données. Chaque champ a un nom de référence et un type pour identifier le type de données qu’il contient. Les types de champ peuvent être les suivants : entier, nombre, chaîne, valeur booléenne et schéma.

**Libellés de champ :** les libellés de champ sont des étiquettes de gouvernance des données qui sont soit héritées d’un jeu de données, soit appliquées directement à un champ.

**Nom du champ :** le nom du champ est un nom utilisé pour référencer le champ dans les requêtes et services.
...
**Géo-barrière :** une géo-barrière est une limite géographique virtuelle, définie par les technologies GPS ou RFID qui permet à un logiciel de déclencher une réponse lorsqu’un appareil mobile entre ou quitte une zone particulière.

**RGPD** : le règlement général sur la protection des données (RGPD) est un cadre juridique qui définit des lignes directrices pour la collecte et le traitement des informations personnelles des individus au sein de l’Union européenne (UE). Le RGPD énonce les principes de la gestion des données et les droits des individus et couvre toutes les entreprises qui traitent les données des citoyens de l’UE.

**Étiquette de données RGPD :** l’étiquette de gouvernance du RGPD sert à définir les champs qui peuvent contenir des identifiants personnels à utiliser dans les requêtes d’accès ou de suppression en vertu du RGPD.
...
**Identité :** l’identité est un identifiant, tel qu’un identifiant de cookie, un identifiant d’appareil ou un identifiant de courrier électronique, qui représente de manière unique un client final.

**Étiquettes de données Identité « I » :** les étiquettes `Identity I` permettent de classer les données pouvant identifier ou contacter une personne spécifique.

**Graphique d’identités :** le graphique d’identités est un mappage des relations entre les identités associées et liées, qui se met à jour en temps quasi réel avec l’activité des clients.

**Espaces de noms d’identité :** un espace de noms d’identité est un identifiant, tel qu’un identifiant de cookie, un identifiant d’appareil ou un identifiant de courrier électronique, pour indiquer le contexte d’origine des données et est utilisé pour reconnaître et lier les identités dans [!DNL Experience Cloud].
...
**Combinaison d’identités :** la combinaison d’identités est un processus d’identification de fragments de données, puis leur combinaison, pour former un enregistrement complet de profil.
...
**Étiquette de données I2 :** l’étiquette de données `I2` sert à classer les données identifiables indirectement, qui peuvent être utilisées en combinaison avec toute autre donnée pour identifier ou contacter une personne spécifique.

**Ingérer :** l’ingestion est le processus d’ajout de données d’une source à [!DNL Experience Platform]. Data can be ingested to [!DNL Experience Platform] in a number of ways including streamed, batched, or added via connector.

**Planification de l’ingestion :** la planification de l’ingestion fournit des options temporelles lors de l’ingestion d’une source vers [!DNL Experience Platform].

**Fonctionnalité d’entrée :** la fonctionnalité d’entrée est spécifiée dans le mappage des fonctionnalités et est utilisée par un modèle d’apprentissage automatique pour faire des prédictions.
...
**Ciblage en fonction de l’intérêt ou personnalisation :** le ciblage en fonction de l’intérêt, également appelé personnalisation, se produit si les trois conditions suivantes sont remplies : les données collectées sur un site sont utilisées pour établir des inférences sur l’intérêt d’un utilisateur ; les données sont utilisées dans un autre contexte, par exemple sur un autre site ou une autre application (hors site) ; et si les données sont utilisées pour sélectionner le contenu ou les publicités qui sont diffusés en fonction de ces inférences.
...
**Action marketing :** Une action marketing, également appelée cas d’utilisation marketing, dans le cadre de la gouvernance des données, est une action entreprise par un utilisateur de [!DNL Experience Platform] données, pour laquelle il est nécessaire de vérifier les violations des stratégies d’utilisation des données.

**Méthode de fusion :** une `merge method` est une option de stratégie de fusion qui permet de hiérarchiser la fusion des fragments de données. Les options de méthode de fusion sont fusionnées par ordre de priorité du jeu de données ou par horodatage du jeu de données.

**Stratégie de fusion :**[!DNL Profile] une stratégie de fusion est un ensemble de règles utilisé par pour déterminer comment les données seront hiérarchisées et combinées dans un affichage unifié, sous certaines conditions.
...
**Colonne Date de modification :** la sélection d’une colonne `Modified Date` est une option lors de la spécification de données tierces via une connexion. Lorsque la stratégie d’enregistrement `Delta` est sélectionnée et que le jeu de données contient plusieurs schémas de date, l’utilisateur doit choisir dans le type de schéma date/heure disponible pour spécifier la colonne Date de modification clé. L’option `Modified Date` n’est pas disponible lorsque la stratégie d’enregistrement `Overwrite` est sélectionnée.
...
**Ciblage sur site Action** marketing Action marketing qui utilise des données pour les publicités sur site, y compris la sélection et la diffusion des publicités sur les sites Web ou les applications de votre organisation, ou pour mesurer la diffusion et l&#39;efficacité de ces publicités.

**Organisation :** une organisation est le nom utilisé pour identifier une société ou un groupe spécifique au sein d’une société dans les produits Adobe. L’administrateur peut configurer et gérer l’accès et les autorisations relatifs aux fonctionnalités pour les utilisateurs d’une organisation.

**Stratégie d’enregistrement Overwrite (Remplacer) :** la stratégie d’enregistrement `Overwrite` est une option permettant d’ingérer des données tierces via une connexion, où l’utilisateur indique si les données ingérées seront remplacées selon un calendrier spécifié. [!DNL Experience Platform] ingère le jeu de données spécifié à partir de la source tierce et remplace le jeu de données sur [!DNL Experience Platform].

[Haut de page](#adobe-experience-platform-glossary)

## P

**Ingestion partielle :** l’ingestion partielle permet l’ingestion d’enregistrements valides de données de lot avec un seuil d’erreur spécifié. Les diagnostics d&#39;erreur pour les enregistrements en échec peuvent être téléchargés ou l&#39;accès dans l&#39;aperçu de l&#39;exécution du flux de données de surveillance ou de sources.
...
**Politique :** une politique d’utilisation des données est une règle qui spécifie les actions marketing qui sont restreintes en fonction de l’application de libellés d’utilisation des données sur les données dans [!DNL Experience Platform].

**Application des** stratégies Permet d’appliquer des stratégies d’utilisation des données à l’aide d’actions marketing appliquées afin d’empêcher les opérations de données qui constituent des violations des stratégies au sein d’une organisation.
...
**Profil de produits :** les profils de produits permettent aux administrateurs d’accorder à l’utilisateur l’accès à l’ensemble ou à un sous-ensemble de services associés à [!DNL Experience Platform].

**Environnement de test de production :** environnement de test de production d’isolation des données virtuelles sur Platform, qui ne peuvent ni être réinitialisées ni supprimées.
...
**Requête :** une requête est une demande de données provenant de tables de base de données.

**Éditeur de requêtes :** l’éditeur de requêtes est un outil permettant le codage, la validation et l’envoi d’instructions SQL dans [!DNL Query Service].

**Requête Service pour Adobe Experience Platform :** *[!DNL Experience Platform Query Service]* permet aux analystes de données d’effectuer des requêtes [!DNL ExperienceEvents] et des fichiers XDM pour les utiliser dans les analyses et l’apprentissage automatique. With [!DNL Query Service], data scientists and analysts will be able to pull all of their datasets stored in [!DNL Experience Platform] – including behavioral data as well as point-of-sale (POS), customer relationship management (CRM), and more – and query those datasets to answer specific questions about the data.

[Haut de page](#adobe-experience-platform-glossary)

## R

**Plate-forme de données client en temps réel :** L&#39;Adobe [!DNL Real-time Customer Data Platform] rassemble des données clients connues et inconnues afin de créer des profils clients de confiance avec une intégration simplifiée, une segmentation intelligente et une activation en temps réel tout au long du parcours client numérique.

**Profil client en temps réel :** [!DNL Real-time Customer Profile] est un profil centralisé pour la gestion d’expérience ciblée et personnalisée et fournit un profil de consommation unifié en temps réel basé sur des données agrégées provenant de sources multiples.
...
**Enregistrement :** il s’agit de données qui persistent sous forme de lignes dans un jeu de données.
...
**Contrôle d&#39;accès basé sur les rôles :** Le contrôle d&#39;accès basé sur les rôles permet aux administrateurs d’attribuer l’accès et les autorisations aux utilisateurs de [!DNL Experience Platform]. Permissions include the ability to view and/or use [!DNL Experience Platform] features, such as creating sandboxes, defining schemas, and managing datasets.
...
**Runtime :** Runtime spécifie un environnement d’exécution pour une recette d’apprentissage automatique. [!DNL Python], R, [!DNL Spark], PySpark et Tensorflow permettent d&#39;entrer une URL vers une image de docker pour une source de recettes.

[Haut de page](#adobe-experience-platform-glossary)

## S

**Données d’exemple :** les données d’exemple constituent un aperçu d’un fichier de données, généralement les 100 premières lignes, destiné à fournir à un spécialiste des données ou à un ingénieur une idée des schémas ou des données qu’il contient.

**Environnement de test :** un environnement de test est une forme d’isolation des données virtuelles au sein d’une organisation d’utilisateurs sur [!DNL Experience Platform].

**Réinitialisation d’un environnement de test :** la réinitialisation d’un environnement de test supprime toutes les données, y compris les données, les profils et les segments de cet environnement de test. La réinitialisation d’un environnement de test peut affecter les données connectées à des destinations internes ou externes.

**Commutateur sandbox :** Le contrôle sandbox dans [!DNL Experience Platform] permet aux utilisateurs de naviguer entre les sandbox auxquels ils ont accès. Le fait de changer d’environnement de test modifie tout le contenu et peut modifier l’accès aux fonctionnalités en fonction des autorisations.

**Planification :** la planification est une spécification définie par l’utilisateur sur la fréquence ou la cadence d’ingestion des données d’une source de données tierce vers Adobe [!DNL Experience Platform].

**Notation :** la notation est le processus de génération d’informations à partir de données en utilisant un modèle formé.

**Schéma :** un schéma est constitué d’une classe et d’un mixin facultatif. Il est utilisé pour créer des jeux de données et des flux de données. Un schéma inclut des attributs comportementaux, un horodatage, une identité, des définitions d’attributs et des relations.

**Descripteur de schéma :**[!DNL Experience Platform] le descripteur de schéma est une métadonnée liée au schéma supplémentaire qui décrit le comportement qui peut être utilisé par pour comprendre le comportement de schéma prévu, tel que la relation entre deux schémas.

**Clé d’accès secrète :**[!DNL Amazon] une clé d’accès secrète est une clé S3 utilisée conjointement avec l’identifiant de clé d’accès pour signer des demandes AWS.

**Segment :** un segment est un ensemble de règles qui incluent des attributs et des données d’événement qui permettent à un certain nombre de profils devenir une audience.
...
**Définition de segment :** la définition de segment est l’ensemble des règles utilisées pour décrire les caractéristiques ou les comportements clés d’une audience cible. Une fois conceptualisées, les règles décrites dans une définition de segment sont utilisées pour déterminer les membres de l’audience admissibles à un segment.
...
**Adhésion au segment :** l’adhésion au segment affiche le segment auquel fait actuellement partie un profil.

**Règles de segment :** les règles de segment indiquent où et comment l’utilisateur définit les conditions que le segment exige des profils.
...
**Services :** cadre puissant permettant de rendre opérationnels les services d’intelligence artificielle et d’apprentissage automatique, grâce à l’utilisation d’Adobe Intelligent Services. Services offre des expériences client personnalisées en temps réel ou rendent opérationnels des services intelligents personnalisés.

**Personnalisation d’identité unique Action** marketing Action marketing qui utilise les données pour la personnalisation de contenu sur site. La personnalisation sur site est toute donnée utilisée pour faire des inférences sur les intérêts des utilisateurs et sert à sélectionner le contenu ou les publicités qui sont diffusés en fonction de ces inférences.

**Étiquette de données S1 :** l’étiquette de données `S1` sert à classer les données spécifiant la latitude et la longitude qui peuvent être utilisées pour déterminer l’emplacement précis d’un appareil.

**Étiquette de données S2 :** l’étiquette de données `S2` sert à classer les données qui peuvent être utilisées pour déterminer une zone de géo-barrière définie dans l’ensemble.
...
**Espace de noms d’identité standard :** les espaces de noms d’identité standard sont des identifiants prédéfinis Adobe, incluant les solutions Adobe et standard de l’industrie utilisées pour identifier les utilisateurs.

**Schéma standard :** les schémas standard sont composés de classes et de mixins, et sont destinés à être réutilisés.

**URL de point de terminaison en continu :** une URL de point de terminaison en continu est un point de terminaison unique fourni par Adobe et lié à l’organisation IMS d’un client pour diffuser des données dans [!DNL Experience Platform].

**Ingestion par flux :**[!DNL Experience Platform] l’ingestion par flux fournit aux utilisateurs une méthode pour envoyer en temps réel des données de périphériques côté client et côté serveur vers 

**Segmentation par flux :** la segmentation par flux est un processus continu de sélection des données qui met à jour les segments en réponse aux activités des utilisateurs. Une fois qu’un segment a été créé et enregistré, la définition de segment est appliquée aux données d’entrée [!DNL Real-time Customer Profile]. Les ajouts et les suppressions de segments sont traités régulièrement, ce qui vous permet de vous assurer que votre ciblage d’audience reste pertinent.

**Symbole :** un symbole est l’abréviation d’un espace de noms d’identité, qui peut être utilisé comme référence dans les API.

**Vue du système :**[!DNL Real-time Customer Profile] la vue du système est une représentation visuelle des jeux de données sources qui transitent par vers les destinations.

[Haut de page](#adobe-experience-platform-glossary)

## T

**Fonctionnalités cible :** la fonctionnalité cible est spécifiée dans le mappage des fonctionnalités. C’est la fonction prédite par un modèle.

**Modèle formé :** un modèle formé représente la sortie exécutable d’un processus d’entraînement de modèle, dans lequel un jeu de données d’apprentissage a été appliqué à l’instance de modèle. Un modèle formé conserve une référence au service web intelligent qui est créé à partir de celui-ci. Le modèle formé est adapté à la notation et à la création d’un service web intelligent. Les modifications apportées à un modèle formé peuvent être suivies comme une nouvelle version.
...
**Type :** le type est la classe de problème d’apprentissage automatique pour laquelle une recette est conçue, utilisée après l’entraînement pour aider à personnaliser l’évaluation de la session d’entraînement.

[Haut de page](#adobe-experience-platform-glossary)

## U

**Schéma d’union :** le schéma d’union est une consolidation des schémas qui ont été activés pour [!DNL Real-time Customer Profile].
...
**Modèle de données d’expérience (XDM) :**[!DNL Experience Platform] XDM est le concept d’utilisation de schémas standard pour unifier les données en vue de les utiliser avec et les applications Adobe Experience Cloud. XDM est une spécification officielle utilisée pour représenter toutes les données d’expérience client dans un langage unique ou un modèle de données standard. Elle normalise la structure des données et accélère et simplifie le processus d’obtention d’insights à partir de très grandes quantités de données.

**Événement de décision XDM :** un événement de décision sert à recueillir des observations sur le résultat et le contexte d’une activité de décision ; y compris des renseignements sur la façon dont la décision a été prise, le moment où elle a eu lieu, les options proposées (choisies) et l’état contextuel qui a influé sur la décision ou qui a pu être observé pendant le processus de décision. Les événements de décision récoltent également l’identifiant de proposition, un identifiant unique global qui peut être utilisé pour corréler la décision à d’autres événements. Les événements de décision sont non seulement liés aux événements d’expérience qui ont influé sur une décision, mais aussi aux événements d’expérience qui sont une réponse directe à une proposition. On s’attend à ce que les applications référencent l’identifiant de proposition dans chaque événement d’expérience qui a été influencé par les propositions. L’historique proposition-réponse d’un profil individuel est conservé à l’aide des identifiants de proposition.

**XDM ExperienceEvent :** un ExperienceEvent est un enregistrement factuel de ce qui s’est passé, y compris le moment et l’identité de la personne impliquée. Les événements d’expérience peuvent être explicites (actions humaines directement observables) ou implicites (obtenus sans action humaine directe), et sont enregistrés sans agrégation ni interprétation. Ils sont essentiels à l’analyse du domaine temporel, car ils permettent l’observation et l’analyse des changements qui surviennent dans une période donnée, et la comparaison entre plusieurs périodes pour suivre les tendances.

**XDM  :**[!DNL Individual Profile] un schéma XDM Individual Profile forme une représentation singulière des attributs et des intérêts des individus identifiés et partiellement identifiés. Les profils moins identifiés peuvent contenir uniquement des signaux comportementaux anonymes, tels que des cookies de navigateur, tandis que les profils hautement identifiés peuvent contenir des informations personnelles détaillées, telles que le nom, la date de naissance, l’emplacement et l’adresse électronique. À mesure qu’un profil se développe, il devient un solide référentiel d’informations personnelles, d’informations d’identification, de coordonnées et de préférences de communication pour une personne.

**Système XDM :**[!DNL Experience Platform] le système XDM est l’infrastructure, la sémantique des données et le workflow dans , optimisé par des schémas standard.
