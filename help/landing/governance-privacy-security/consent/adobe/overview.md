...
* [Real-time Customer Profile](../../../../profile/home.md) : Utilise  [!DNL Identity Service] des fonctionnalités pour créer des profils client détaillés à partir de vos jeux de données en temps réel. Real-time Customer Profile extrait des données du lac de données et conserve les profils clients dans sa propre banque de données distincte.
...
* [Service de segmentation Adobe Experience Platform](../../../../segmentation/home.md) : Permet de diviser les données de Real-time Customer Profile en groupes d’individus partageant des caractéristiques similaires et réagissant de la même manière aux stratégies marketing.
...
1. Au chargement de chaque page (ou lorsque votre CMP détecte une modification des préférences de consentement), un script personnalisé sur votre site associe les préférences actuelles à un objet XDM standard. Cet objet est ensuite transmis à la commande SDK Web de Platform `setConsent`.
...
Dans la version actuelle de la prise en charge du traitement du consentement dans Platform, seule l’autorisation de collecte de données (`collect.val`) est automatiquement appliquée par le SDK Web de Platform. Bien que des préférences et des consentements plus granulaires puissent être collectés et conservés dans les profils client, ces signaux supplémentaires doivent être imposés manuellement dans vos propres processus en aval.
...
## Ajout de champs de consentement normalisés à un jeu de données compatible [!DNL Profile] {#dataset}
...
Consultez la documentation du SDK Mobile pour [configurer l’extension mobile de consentement](https://aep-sdks.gitbook.io/docs/v/AEP-Edge-Docs/using-mobile-extensions/adobe-edge-consent) et [à l’aide de l’API de consentement](https://aep-sdks.gitbook.io/docs/v/AEP-Edge-Docs/using-mobile-extensions/adobe-edge-consent/edge-consent-api-reference). Pour plus d’informations sur la manière de gérer les problèmes de confidentialité à l’aide du SDK Mobile, reportez-vous à la section [Confidentialité et RGPD](https://aep-sdks.gitbook.io/docs/v/AEP-Edge-Docs/resources/privacy-and-gdpr).
...
Suivez le tutoriel [mappage d’un fichier CSV à XDM](../../../../ingestion/tutorials/map-a-csv-file.md) pour savoir comment convertir vos champs de données en XDM et les ingérer dans Platform. Lorsque vous sélectionnez la [!UICONTROL destination] du mappage, veillez à sélectionner l’option **[!UICONTROL Utiliser le jeu de données existant]** et à choisir le jeu de données de consentement [!DNL Profile] activé que vous avez créé précédemment.

## Tester votre mise en oeuvre {#test-implementation}

Après avoir ingéré des données de consentement du client dans votre jeu de données activé [!DNL Profile], vous pouvez vérifier vos profils mis à jour pour voir s’ils contiennent des attributs de consentement.
...
Consultez la section [navigation des profils par identité](../../../../profile/ui/user-guide.md#browse) dans le guide de l’interface utilisateur [!DNL Profile] pour obtenir des instructions spécifiques sur la manière de rechercher les détails d’un profil.
