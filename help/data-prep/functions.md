...
| split | Divise une chaîne d’entrée sur un séparateur. Le séparateur suivant **doit être placé en séquence d’échappement** avec `\` : `\`. Si vous incluez plusieurs délimiteurs, la chaîne est fractionnée sur **n** des délimiteurs présents dans la chaîne. | <ul><li>ENTRÉE : **Obligatoire** Chaîne d’entrée qui va être fractionnée.</li><li>SÉPARATEUR : **Obligatoire** Chaîne utilisée pour fractionner l’entrée.</li></ul> | split(INPUT, SEPARATOR) | split(&quot;Hello world&quot;, &quot; &quot;) | `["Hello", "world"]` |
