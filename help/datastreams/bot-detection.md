...
feature_v2: id: b3f03848-ae12-48b2-8aab-cad18567eb32id: e08599ea-8888-4294-ba74-3ba0a7762a46id: fd307ce7-56f5-4ee3-af68-a7833ff6e85e
subfeature_v2: id: ca3d6bf4-a4af-4944-936b-8de1eb09f149id: f1f1a2d4-0976-4881-b091-c2bb8de7ffac
...
Utilisez la détection de robots pour identifier les événements générés par l’[Web SDK](/help/collection/js/js-overview.md), [Mobile SDK](https://developer.adobe.com/client-sdks/home/) et [l’API Edge Network](https://developer.adobe.com/data-collection-apis/docs/api/) comme étant générés par des araignées et des robots connus.
...
Lorsqu’une requête au [!DNL Edge Network] correspond à l’une des règles de détection de robots, le schéma XDM est mis à jour avec un score de robots (toujours défini sur 1) :
...
* Les règles de détection de robots peuvent prendre jusqu’à 15 minutes pour se propager à travers le [!DNL Edge Network] après leur création.

## Conditions préalables {#prerequisites}

Pour que la détection de robots fonctionne sur votre flux de données, vous devez ajouter le groupe de champs **[Informations sur la détection de robots](/help/xdm/field-groups/event/bot-detection-information.md)** à votre schéma. Consultez la documentation [Schéma XDM](/help/xdm/ui/resources/schemas.md#add-field-groups) pour savoir comment ajouter des groupes de champs à un schéma.

## Configuration de la détection des robots pour les flux de données {#configure}

Vous pouvez configurer la détection des robots après avoir créé une configuration de train de données. Consultez la documentation sur la [création et configuration d’un flux de données](/help/datastreams/configure.md), puis suivez les instructions ci-dessous pour ajouter des fonctionnalités de détection de robots à votre flux de données.
...
Pour marquer toutes les requêtes provenant d’une adresse IP spécifique comme du trafic de robots, créez une règle de détection de robots qui évalue une adresse IP unique.

![Règle de détection de robots configurée pour évaluer une seule adresse IP.](assets/bot-detection/bot-detection-one-ip.png)

### Détection de robots basée sur deux adresses IP {#two-ip}

Pour marquer toutes les requêtes provenant de l’une des deux adresses IP spécifiques comme du trafic de robots, créez une règle de détection de robots qui évalue deux adresses IP.

![Règle de détection de robots configurée pour évaluer deux adresses IP spécifiques.](assets/bot-detection/bot-detection-two-ips.png)

### Détection de robots basée sur une plage d&#39;adresses IP {#range}

Pour marquer toutes les requêtes provenant de n’importe quelle adresse IP dans une plage spécifique comme du trafic de robots, créez une règle de détection de robots qui évalue une plage entière d’adresses IP.

![Règle de détection de robots configurée pour évaluer une plage d’adresses IP.](assets/bot-detection/bot-detection-range.png)

### Détection de robots basée sur une adresse IP et un en-tête de requête {#ip-header}

Pour marquer toutes les requêtes provenant d’une adresse IP spécifique et contenant un en-tête de requête spécifique comme trafic de robots, créez une règle de détection de robots. Cette règle vérifie si la requête provient d’une adresse IP spécifique et si l’en-tête de la requête `referer` commence par `www.adobe.com`.

![Règle de détection de robots configurée pour évaluer une adresse IP et l’en-tête de requête du référent.](assets/bot-detection/bot-detection-header-ip.png)
...
* **Plusieurs conditions différentes** : différentes conditions sont évaluées en tant qu’opération de [!DNL AND] logique, ce qui signifie que toutes les conditions doivent être remplies simultanément pour que le système puisse identifier la requête comme étant du trafic de robots.
* **Conditions multiples du même type** : les conditions du même type sont évaluées comme une opération de [!DNL OR] logique, ce qui signifie que si une condition est remplie, la requête est identifiée comme trafic de robots.

La règle suivante identifie une requête d’origine de robots si ces conditions sont remplies : la requête provient de l’une des deux adresses IP, l’en-tête `referer` commence par `www.adobe.com` et l’en-tête `sec-ch-ua-mobile` identifie la requête comme provenant d’un navigateur de bureau.

![Règle de détection des robots configurée avec plusieurs conditions d’IP, de référent et d’agent utilisateur.](assets/bot-detection/bot-detection-multiple.png)
