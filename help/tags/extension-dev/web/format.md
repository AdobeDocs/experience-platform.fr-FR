...
Lorsque la bibliothèque du runtime de balise s’exécute, les modules sont immédiatement &quot;installés&quot; et leurs exportations sont mises en cache. En supposant le module suivant :
...
`runs on startup` sera consigné immédiatement, tandis que  `runs when necessary` ne sera consigné qu’une fois la fonction exportée appelée par le moteur de balises. Bien que cela puisse être inutile dans le cadre de votre module particulier, vous pouvez en profiter en effectuant toute configuration nécessaire avant d’exporter la fonction.
