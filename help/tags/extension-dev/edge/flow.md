...
Comme le montre le diagramme de haut niveau suivant, la vue du type d’action de l’extension sera affichée dans un iframe dans l’application intégrée à Adobe Experience Platform. La vue est ensuite utilisée pour modifier les paramètres qui sont ensuite enregistrés dans Platform. Lorsque la bibliothèque d’exécution de balises est créée, le module Bibliothèque de type action de l’extension ainsi que les paramètres définis par l’utilisateur sont inclus dans la bibliothèque d’exécution qui est déployée sur le nœud Edge. Les paramètres définis par l’utilisateur à partir de Platform sont injectés dans le module Bibliothèque au moment de l’exécution.
