...
* Les conditions et les exceptions ont été combinées dans le créateur de règles, car l’ordre a son importance. Cela reflète mieux la manière dont elles sont traitées dans le système et au moment de l’exécution. Pour créer une exception, cliquez sur le bouton **[!UICONTROL Add]** sous [!UICONTROL Conditions] et choisissez **[!UICONTROL Exception]** dans le sélecteur [!UICONTROL Logic Type].
...
Les codes intégrés sont passés de la page Détails des environnements à un modal d’instructions d’installation. Ce modal s’affiche automatiquement après la création d’un environnement et est accessible depuis la vue Liste des environnements.
...
Modification du comportement du bouton [!UICONTROL Add All Changes] sur la page [!UICONTROL Edit Library]. Auparavant, une ressource n’était considérée comme « modifiée » que lorsque vous l’aviez ajoutée à une bibliothèque et enregistrée. Désormais, une ressource est considérée comme « modifiée » jusqu’à ce qu’elle ait été publiée dans l’environnement de production.
...
#### Les pages d’environnement affichent par défaut les codes intégrés asynchrones.

Les pages d’environnement affichent désormais les codes intégrés asynchrones comme valeur par défaut. Basculer entre la synchronisation et l’asynchronisation fonctionne exactement comme auparavant.
...
#### Liaison du code intégré de DTM aux balises

Lorsque vous liez votre code intégré [!DNL DTM] aux balises, vous pouvez conserver votre code intégré de production [!DNL DTM] sur une page, mais y placer des fichiers de balises plutôt que des fichiers [!DNL DTM].
...
* Activation/désactivation asynchrone dans les environnements

  Lors de la récupération du code intégré d’un environnement, vous pouvez désormais actionner un interrupteur à bascule pour obtenir le code intégré si vous souhaitez que la bibliothèque se charge de manière asynchrone.
...
* Il existe désormais des entrées de journal d’audit pour la suppression de propriétés.
