...
description: Notes de mise à jour 2019 pour les balises dans Adobe Experience Platform.
...
* Ajustements visuels des contrôles de code dans l’éditeur de code : Nous avons réalisé quelques ajustements visuels pour adoucir l’aspect de la validation du code que nous avons publiée en août dernier.
...
Invalidations multiples du cache : Si vous utilisez l’hôte *Géré par Adobe* pour vos environnements, les balises effectuent désormais plusieurs invalidations du cache à chaque publication. Auparavant, une invalidation était effectuée pour chaque version immédiatement après le chargement de la version. Cependant, il existe un délai entre le moment où le fichier est chargé sur un serveur d’origine et celui où il est disponible sur tous les serveurs d’origine. Par conséquent, une condition de concurrence a parfois provoqué la mise en cache de la version obsolète de la bibliothèque sur le serveur Edge pendant 6 heures au maximum. Chaque version est maintenant purgée aux intervalles suivants :
...
* Correction d’un bogue qui vous empêchait de redévelopper le code personnalisé dans la vue de comparaison.

## 18 juin 2019

### Mises à jour

* Les bibliothèques qui utilisent l’hôte [!DNL Managed by Adobe] et un environnement non archivé font désormais référence à tous les sous-fichiers directement grâce à `https://` au lieu d’hériter du protocole de page sous-jacent. Vous pouvez ainsi utiliser des balises dans des scénarios HTML incorporés (environnements mobiles et de set-top box en particulier) sans auto-hébergement.
...
L’API [!DNL Reactor] qui alimente les balises atteint officiellement le statut 1.0 aujourd’hui. Cela s’accompagne d’un engagement à maintenir la compatibilité ascendante au sein de cette version majeure (la série 1.x). Pour commencer à utiliser l’API [!DNL Reactor], consultez le [guide de l’API Reactor](../api/overview.md).
...
* L’utilisation de l’interface utilisateur de collecte de données n’est plus prise en charge dans IE 11. Vous recevrez un avertissement si vous vous connectez à IE 11.
