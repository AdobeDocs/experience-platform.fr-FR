...
* **Arrêt des modules d’extension :** les développeurs d’extensions peuvent désormais arrêter leurs modules. Un module d’extension arrêté ne peut plus être installé à partir du catalogue des balises. Les installations existantes continuent de fonctionner comme prévu. Pour plus d’informations, reportez-vous à la section [Arrêt d’un package d’extension](https://developer.adobelaunch.com/api/reference/1.0/extension_packages/discontinue/) dans la documentation pour les développeurs.
