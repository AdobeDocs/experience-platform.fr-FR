...
Ce guide fournit un aperçu général de la configuration des secrets d’une propriété de transfert d’événement () dans l’interface utilisateur d’Experience Platform ou l’interface utilisateur de collecte de données.
...
Pour créer un secret, sélectionnez **[!UICONTROL Transfert d’événement]** dans le volet de navigation de gauche, puis ouvrez la propriété de transfert d’événement sous laquelle vous souhaitez ajouter le secret. Sélectionnez ensuite **[!UICONTROL Secrets]** dans le volet de navigation de gauche, puis **[!UICONTROL Créer un secret]**.
...
>[!NOTE]
>
>Si vous souhaitez toujours créer et enregistrer le secret avant de l’ajouter à un environnement, désactivez le bouton (bascule) **[!UICONTROL Associer le secret à des environnements]** avant de renseigner le reste des informations. Notez que vous devrez affecter le secret à un environnement par la suite si vous souhaitez lʼutiliser.
>
>![Désactivation de lʼenvironnement](../../images/ui/event-forwarding/secrets/env-disabled.png)

Sous **[!UICONTROL Environnement cible]**, utilisez le menu déroulant pour sélectionner l’environnement auquel vous souhaitez attribuer le secret. Sous **[!UICONTROL Nom du secret]**, attribuez un nom au secret dans le contexte de l’environnement. Chaque secret au sein de la propriété de transfert dʼévénement doit posséder un nom unique.
...
Pour chaque environnement ajouté, vous devez attribuer un nouveau nom unique pour le secret associé. Si vous avez épuisé tous les environnements disponibles, le bouton **[!UICONTROL Ajouter un environnement]** ne sera pas disponible.
...
Sous **[!UICONTROL Options d’identification]**, vous pouvez fournir d’autres options d’identification telles que `scope` et `audience` sous la forme de paires clé-valeur. Pour ajouter d’autres paires clé-valeur, sélectionnez **[!UICONTROL Ajouter une autre]**.
...
![L’onglet [!UICONTROL Créer un secret] avec le secret JWT OAuth 2 mis en surbrillance dans le menu déroulant [!UICONTROL Type].](../../images/ui/event-forwarding/secrets/oauth-jwt-secret.png)
...
Sous **[!UICONTROL Options d’identification]**, vous pouvez fournir d’autres options d’identification telles que `jwt_param` sous la forme de paires clé-valeur. Pour ajouter d’autres paires clé-valeur, sélectionnez **[!UICONTROL Ajouter une autre]**.
...
Pour créer un secret OAuth 2 Google, sélectionnez **[!UICONTROL Google OAuth 2]** dans la liste déroulante **[!UICONTROL Type]**. Sous **[!UICONTROL Portées]**, sélectionnez les API Google auxquelles vous souhaitez accorder l’accès à l’aide de ce secret. Les produits suivants sont actuellement pris en charge :
...
![Onglet [!UICONTROL Présentation des propriétés] mettant en surbrillance la fenêtre contextuelle de réautorisation secrète.](../../images/ui/event-forwarding/secrets/linkedin-reauthorization-popup.png)
...
Après avoir créé des secrets pour une propriété, vous pouvez les trouver dans l’espace de travail **[!UICONTROL Secrets]**. Pour modifier les détails d’un secret existant, sélectionnez son nom dans la liste.
