...
|---|---|---|
| 1.0.52 | 27 Avril 2026 | <ul><li>Ajout du suivi pour les ressources où CSS charge les images en arrière-plan d’un élément dans le DOM.</li><li>Ajout d’un tableau de `permanentlyBlockedURLs` codé en dur contenant [maps.googleapis.com](https://maps.googleapis.com) et [mapsresources-pa.googleapis.com](https://mapsresources-pa.googleapis.com). Ces URL sont toujours bloquées par défaut dans la bibliothèque Content Analytics.</li><li>Ajout de champs `idSource` et `channel` aux requêtes de collecte de données XDM.</li></ul> |
