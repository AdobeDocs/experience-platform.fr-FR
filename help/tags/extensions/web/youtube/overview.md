...
Chaque propriété de balise dans Adobe Experience Platform requiert que les extensions suivantes soient installées et configurées à partir de l’écran Extensions :

* Adobe Analytics
* Service d’identification des visiteurs Experience Cloud
* Extension Core

Utilisez l’extrait de code [&quot;Incorporer un lecteur à l’aide d’une balise \&lt;iframe\>&quot;](https://developers.google.com/youtube/player_parameters#Manual_IFrame_Embeds) de la documentation destinée aux développeurs Google dans le code HTML de chaque page Web sur laquelle un lecteur vidéo doit être rendu.

Cette extension, version 2.0.1, prend en charge l’incorporation d’une ou de plusieurs vidéos YouTube sur une seule page web en insérant un attribut `id` avec une valeur unique dans la balise de script iframe, et en ajoutant `enablejsapi=1` et `rel=0` à la fin de la valeur de l’attribut `src`, le cas échéant. Par exemple :

`<iframe id="player1" width="560" height="315" src="https://www.youtube.com/embed/xpatB77BzYE?enablejsapi=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`

Cette extension est également conçue pour rechercher de manière dynamique une valeur d’attribut d’identifiant unique, telle que `player1`, que les paramètres de chaîne de requête `enablejsapi` et `rel` existent ou que leurs valeurs attendues soient correctes. Par conséquent, la balise de script YouTube peut être ajoutée à une page web avec ou sans l’attribut `id` et indépendamment de l’inclusion ou non des paramètres de la chaîne de requête `enablejsapi` et `rel`.
...
Par conséquent, il y a un léger scintillement après le chargement de la page. Ce comportement est attendu.

## Éléments de données

L’extension contient six éléments de données. Aucun ne nécessite d’être configuré.

* **Position du curseur de lecture :** enregistre l’emplacement, en secondes, de la position du curseur de lecture sur la chronologie de la vidéo, lorsqu’elle est appelée dans une règle de balise.
...
* **Suivi des points de repère personnalisés :** se déclenche lorsque la vidéo atteint le pourcentage de vidéo indiqué. Par exemple, si une vidéo dure 60 secondes et que le repère spécifié est de 50 %, l’événement se déclenche lorsque la position du curseur de lecture est égale à 30 secondes. Le suivi des points de repère s’applique à la lecture initiale et à la relecture. Notez que si l’utilisateur effectue une recherche sur un point de repère, l’événement ne se déclenche pas. Les événements de point de repère ne se déclenchent que lorsque le curseur de lecture traverse l’emplacement du point de repère calculé sur la chronologie et que le lecteur vidéo est en cours de lecture.
...
* **Actions :** Utilisez l’action **Extension Analytics** pour &quot;Définir des variables&quot;, pour mapper :
