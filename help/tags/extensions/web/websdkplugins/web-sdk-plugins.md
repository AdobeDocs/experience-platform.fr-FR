...
>[!NOTE]
>
>Adobe Experience Platform Launch devient une suite de technologies destinées à la collecte de données dans Experience Platform. Plusieurs modifications terminologiques ont par conséquent été apportées à la documentation du produit. Reportez-vous au [document](../../../launch-term-updates.md) suivant pour consulter une référence consolidée des modifications terminologiques.

>[!IMPORTANT]
>
>L’extension est destinée à être utilisée avec la fonction [!DNL Adobe Experience Platform Web SDK] extension . Pour afficher des informations sur la version qui doit être utilisée avec AppMeasurement, consultez ce lien.

Utilisez cette référence pour plus d’informations sur la configuration de l’extension des modules externes du SDK Web et sur les options disponibles lors de l’utilisation de cette extension afin d’augmenter le [!DNL Adobe Experience Platform Web SDK] Extension .
...
>[!IMPORTANT]
>
>L’extension de modules externes SDK Web courants est destinée à augmenter le [!DNL Adobe Experience Platform Web SDK] mais il n’est pas nécessaire de l’installer pour que l’extension fonctionne comme prévu.

Aucune configuration supplémentaire n’est requise au niveau de l’extension.

## Ajouter des modules externes à l’extension [!DNL Adobe Experience Platform Web SDK]

Contrairement à la contrepartie AppMeasurement, aucune configuration n’est nécessaire pour initialiser ou ajouter un plug-in à votre bibliothèque en dehors de l’utilisation des éléments de données natifs fournis. Voir ci-dessous pour plus d’informations sur chaque élément de données.

## Éléments de données de l’extension de modules SDK Web courants

L’extension de modules externes SDK Web courants fournit les éléments de données suivants :
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données dans Adobe Experience Platform pour configurer l’ [Module externe getAndPersistValue](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getandpersistvalue.html). Le `getAndPersistValue` l’élément de données vous permet de stocker une valeur dans un cookie qui peut être récupérée ultérieurement au cours d’une visite. Il joue un rôle similaire à celui de la fonction [!UICONTROL Durée du stockage] dans Adobe Experience Platform Launch.
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données dans Adobe Experience Platform pour configurer l’ [Module externe getGeoCoordinates](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getgeocoordinates.html). Le `getGeoCoordinates` l’élément de données vous permet de capturer la latitude et la longitude des appareils des visiteurs.
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données pour configurer le [Module externe getNewRepeat](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getnewrepeat.html). Le `getNewRepeat` l’élément de données vous permet de déterminer si un visiteur du site est un nouveau visiteur ou un visiteur régulier pendant un nombre de jours donné.
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données pour configurer le [Module externe getPageName](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpagename.html). Le `getPageName` l’élément de données crée une version formatée facile à lire et conviviale de l’URL actuelle.
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données pour configurer le [Module externe getPreviousValue](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getpreviousvalue.html). Le `getPreviousValue` l’élément de données vous permet de définir une variable sur une valeur définie pour un accès précédent.
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données pour configurer le [Module externe getTimeSinceLastVisit](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/gettimesincelastvisit.html). Le `getTimeSinceLastVisit` l’élément de données vous permet de suivre le temps nécessaire à un visiteur pour revenir sur votre site après sa dernière visite.
...
Permet aux utilisateurs d’utiliser l’interface utilisateur native de la collecte de données pour configurer le [Module externe getValOnce](https://experienceleague.adobe.com/docs/analytics/implementation/vars/plugins/getvalonce.html). Le `getValOnce` l’élément de données empêche qu’une variable soit définie plusieurs fois sur la même valeur.
