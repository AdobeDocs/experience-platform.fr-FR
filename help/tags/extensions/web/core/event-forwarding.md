...
# Présentation de l’extension de transfert d’événement principal
...
>[!NOTE]
>
>L’appel `interact` du client a `events`, mais pour le transfert d’événement, vous avez besoin de `event`. Cela est dû au fait que le transfert d’événement inspecte chaque événement individuellement, et non en tant que lot de plusieurs événements, comme indiqué sur le client.
