...
Le champ de schéma cible contient le chemin d’un champ XDM défini dans le schéma du jeu de données. Pour les champs définis plus en détail dans la hiérarchie du schéma, vous pouvez utiliser le point comme séparateur entre les parties du chemin (par exemple, timeSeriesEvents.eventType).
...
La section de mappage d’identité peut contenir plusieurs lignes. Chaque ligne peut définir un certain type d’identité. Vous pouvez définir les attributs suivants pour une identité : type, état authentifié, Principal et valeur.
...
Si un schéma comporte un champ `xdm:identityMap` et qu’un autre champ est marqué comme champ d’identité Principal, la colonne Principale de la section de mappage d’identité ne sera pas visible.

![](../../../images/adobe-experience-platform-send-beacon-identity-section-not-primary.png)

### Champs obligatoires

Certains schémas auront des champs obligatoires de niveau supérieur. Les plus fréquents sont `timestamp` et `_id`. Si ces champs ne sont pas définis, l’envoi de la balise échouera. Vous pouvez les définir dans la section de mappage de schéma.
