...
Le contrôle d’accès de Adobe Experience Platform est fourni via l’**[!UICONTROL Autorisations]** dans [Adobe Experience Cloud](https://experience.adobe.com/). Cette fonctionnalité exploite les rôles et les politiques, liant les utilisateurs et utilisatrices à des autorisations et des sandbox.
...
- Lors de la création ou de la modification d’un rôle, l’administrateur ajoute des utilisateurs au rôle à l’aide de l’onglet **[!UICONTROL utilisateurs]** et accorde des autorisations à ces utilisateurs (telles que « [!UICONTROL Lire des jeux de données] » ou « [!UICONTROL Gérer des schémas] ») en modifiant les autorisations du rôle. De même, l’administrateur ou administratrice peut attribuer des accès aux sandbox à l’aide de la même option de modification.
- Lorsque les utilisateurs et utilisatrices se connectent à l’interface utilisateur Experience Platform, leur accès aux fonctionnalités d’Experience Platform est géré par les autorisations qui leur ont été accordées à l’étape précédente. Par exemple, si un utilisateur ou une utilisatrice ne dispose pas de l’autorisation [!UICONTROL Afficher les jeux de données], l’onglet **[!UICONTROL Jeux de données]** du menu latéral n’est pas visible pour cet utilisateur ou cette utilisatrice.
...
[!UICONTROL Autorisations] fournit un emplacement central pour la gestion de l’accès Experience Platform pour votre organisation. Grâce aux [!UICONTROL autorisations], vous pouvez accorder des autorisations d’accès à des groupes d’utilisateurs pour diverses fonctionnalités d’Experience Platform, telles que [!UICONTROL Gérer les jeux de données], [!UICONTROL Afficher les jeux de données] ou [!UICONTROL Gérer les profils].
...
| [!DNL Data Governance] | [!UICONTROL Gérer les libellés d’utilisation] | Accès à la lecture, la création et la suppression des libellés dʼutilisation des données. |
| [!DNL Data Governance] | [!UICONTROL Gestion des politiques d’utilisation des données] | Accès à la lecture, la création, la modification et la suppression des politiques dʼutilisation des données. |
| [!DNL Data Governance] | [!UICONTROL Afficher les politiques d’utilisation des données] | Accès en lecture seule pour les politiques dʼutilisation des données appartenant à votre organisation. |
...
| [!DNL Data Modeling] | [!UICONTROL Afficher les schémas] | Accès en lecture seule aux schémas et aux ressources associées. |
| [!DNL Data Modeling] | [!UICONTROL Gérer les relations] | Accès à la lecture, la création, la modification et la suppression des relations de schéma. |
| [!DNL Data Modeling] | [!UICONTROL Gestion des métadonnées d’identité] | Accès à la lecture, la création, la modification et la suppression des métadonnées dʼidentité pour les schémas. |
...
| [!DNL Data Science Workspace] | [!UICONTROL Gestion du Workspace de science des données] | Accès à la lecture, la création, la modification et la suppression dans [!DNL Data Science Workspace]. |
...
| [!DNL Destinations] | [!UICONTROL Afficher les destinations] | Accès en lecture seule pour afficher les destinations disponibles dans l’onglet **[!UICONTROL Catalogue]** et les destinations authentifiées dans l’onglet **[!UICONTROL Parcourir]**. |
...
| [!DNL Destinations] | [!UICONTROL Activer les destinations] | Capacité à activer les données vers les destinations actives qui ont été créées. Cette autorisation nécessite également d’accorder les autorisations [!UICONTROL Afficher les destinations] ou [!UICONTROL Gérer les destinations] à l’utilisateur ou utilisatrice qui activera les destinations. |
| [!DNL Destinations] | [!UICONTROL Activer un segment sans mappage] | La possibilité d’activer des audiences vers des destinations existantes, sans afficher l’[étape de mappage](../destinations/ui/activate-batch-profile-destinations.md#mapping). Les utilisateurs et utilisatrices peuvent ajouter et supprimer des audiences dans les workflows d’activation, mais ne peuvent pas ajouter ni supprimer des identités ou des attributs mappés. Cette autorisation nécessite également d’accorder l’autorisation [!UICONTROL Afficher les destinations] à l’utilisateur ou utilisatrice qui activera les données vers les destinations. |
...
| [!DNL Identity Management] | [!UICONTROL Affichage des espaces de noms d’identité] | Accès en lecture seule aux espaces de noms d’identité. |
| [!DNL Identity Management] | [!UICONTROL Afficher le graphique d’identités] | Accès en lecture seule aux graphiques d’identité. |
...
| [!DNL Profile Management] | [!UICONTROL Gérer les profils] | Accès à la lecture, la création, la modification et la suppression des jeux de données utilisés pour les profils de clients. Accès en lecture seule aux profils disponibles. |
| [!DNL Profile Management] | [!UICONTROL Affichage des profils] | Accès en lecture seule aux profils disponibles. |
| [!DNL Profile Management] | [!UICONTROL Gérer les segments] | Accès à la lecture, la création, la modification et la suppression des audiences. |
...
| [!DNL Profile Management] | [!UICONTROL Évaluation d’un segment vers une audience] | Possibilité de générer des profils pour une audience en évaluant une définition de segment. |
