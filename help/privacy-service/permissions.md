...
>[!NOTE]
>
>Toutes les autorisations Privacy Service et [!UICONTROL Opt-out/refus de la vente] sont distinctes et séparées les unes des autres, sans chevauchement fonctionnel. Cela est possible, car l’API de Privacy Service est considérée comme idempotent.
...
Après avoir sélectionné un profil de produit, vous pouvez utiliser l’onglet **[!UICONTROL Autorisations]** pour commencer à [modifier les autorisations](#edit-permissions) pour le profil, ou sélectionner l’onglet **[!UICONTROL Utilisateurs]** pour commencer à [affecter des utilisateurs](#assign-users) au profil.
...
![La section [!UICONTROL Désinscription de la vente] sous autorisations.](./images/permissions/switch-category.png)
...
Après vous être connecté à votre projet, sélectionnez l’intégration **[!UICONTROL API]** dans la barre latérale gauche.
...
La boîte de dialogue [!UICONTROL Configurer l’API] affiche les profils de produit disponibles qui existent actuellement dans le service. Ils sont en corrélation avec les profils de produit créés dans l’Admin Console. Dans la liste des profils de produit disponibles, cochez la case correspondant au nouveau profil de produit que vous avez créé pour le compte technique dans l’Admin Console. Ce compte technique est ainsi automatiquement associé aux autorisations du profil de produit sélectionné. Sélectionnez **[!UICONTROL Enregistrer l’API configurée]** pour confirmer vos paramètres.
