...
1. **Ai-je appliqué les libellés corrects à mes données ?**
   * Les données doivent être correctement étiquetées pour le service afin de déterminer les champs à accéder ou à supprimer lors des tâches de confidentialité. Voir la section sur [étiquetage des données](#label) pour plus d’informations.
...
Pour plus d’informations sur les différentes réglementations légales en matière de confidentialité que Privacy Service gère les demandes pour , y compris les termes clés et les réponses aux questions fréquentes, reportez-vous à la section [documentation sur les réglementations de confidentialité](./regulations/overview.md).
...
Selon le [!DNL Experience Cloud] les applications utilisées par votre système CRM, le type et le nombre de valeurs d’identité que vous devez fournir pour chaque client varient. Certaines applications utilisent leurs propres valeurs d’ID client internes (comme les Adobe Target ID), tandis que d’autres solutions s’appuient sur des identifiants globaux provenant d’Adobe. [!DNL Experience Cloud Identity Service] (ECID) qui effectue le suivi de l’activité des clients sur l’ensemble des [!DNL Experience Cloud] applications. En outre, des informations personnelles génériques telles qu’une adresse e-mail ou un numéro de téléphone peuvent également servir de données d’identité valides.

Lire le document sur [données d’identité pour les demandes d’accès à des informations personnelles](./identity-data.md) pour obtenir des informations plus détaillées sur les types d’informations d’identité acceptés par Privacy Service. Ce document fournit également des conseils sur la manière d’appliquer les technologies d’Adobe pour récupérer efficacement les informations d’identité appropriées de vos clients lorsqu’ils interagissent avec votre site web, et envoyer ces données à Privacy Service dans les demandes d’API.
...
>
>Lorsque vous suivez les guides de l’API ou de l’interface utilisateur, reportez-vous au document sur [Applications Privacy Service et Experience Cloud](./experience-cloud-apps.md) pour obtenir de la documentation supplémentaire sur la manière de formater les demandes d’accès à des informations personnelles pour votre [!DNL Experience Cloud] applications.
>
...
Vous pouvez créer et surveiller des tâches de confidentialité à l’aide d’une interface graphique avec l’interface utilisateur de Privacy Service. L’interface utilisateur comprend un **[!UICONTROL Rapport d’état]** qui fournit une représentation visuelle de l’état de toutes les requêtes actives. Vous pouvez également créer des requêtes avec le **[!UICONTROL Créateur de requêtes]** ou en chargeant des fichiers JSON. Pour plus d’informations sur l’utilisation de l’interface utilisateur, consultez le [guide d’utilisation de Privacy Service](ui/overview.md).
