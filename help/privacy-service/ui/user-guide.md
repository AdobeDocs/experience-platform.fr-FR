...
Vous pouvez filtrer la liste en saisissant des mots-clés dans la barre de recherche située sous le [!UICONTROL Requêtes de tâche] titre. La liste est automatiquement filtrée au fur et à mesure de votre saisie, et affiche les requêtes qui contiennent des valeurs correspondant aux termes recherchés. Le champ de recherche effectue une recherche &quot;rapide&quot; qui correspond aux ID des tâches de confidentialité aux tâches actuellement rendues/chargées dans l’interface utilisateur. Il ne s’agit pas d’une recherche exhaustive de toutes les tâches envoyées. Il s’agit plutôt d’un filtre appliqué aux résultats chargés. Utilisez l’API du Privacy Service pour [renvoyer des tâches en fonction d’un règlement spécifique, de périodes ou d’une seule tâche ;](../api/privacy-jobs.md#list).
...
Vous pouvez également utiliser le bouton de recherche pour effectuer une requête de tâche de confidentialité qui s’étend sur une période donnée. Cette action renvoie toutes les tâches de confidentialité envoyées par votre organisation pendant la période donnée. Sélectionnez la variable **[!UICONTROL Demandé le]** menu déroulant pour sélectionner une date de début et une date de fin pour la requête. Les options disponibles incluent [!UICONTROL Aujourd&#39;hui], [!UICONTROL 7 derniers jours], [!UICONTROL 2 dernières semaines], [!UICONTROL 30 derniers jours], ou [!UICONTROL Personnalisé]. Lorsqu’elle est utilisée avec la fonction [!UICONTROL Demandé le] , la fonction de recherche affiche uniquement les requêtes de tâche qui ont été envoyées entre les périodes de votre choix.

![La section Requête de tâche avec le champ de recherche, le menu déroulant Demandé le et le bouton Rechercher sont mis en surbrillance.](../images/user-guide/requested-on-dropdown-menu.png)
