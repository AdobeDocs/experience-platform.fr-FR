...
>[!NOTE]
>
>En règle générale, le [!DNL Privacy JS Library] ne doit être installé que sur les pages relatives à la confidentialité. Il n’est pas nécessaire de l’installer sur toutes les pages d’un site web ou d’un domaine.

## Fonctions

Le [!DNL Privacy JS Library] fournit plusieurs fonctions de gestion des identités dans [!DNL Privacy Service]. Ces fonctions ne peuvent être utilisées que pour gérer les identités stockées dans le navigateur pour un visiteur spécifique. Ils ne peuvent pas être utilisés pour envoyer directement des informations au [!DNL Experience Cloud Central Service].

Le tableau suivant décrit les différentes fonctions proposées par la bibliothèque :

| Fonction | Description |
| --- | --- |
| `retrieveIdentities` | Renvoie un tableau d’identités correspondantes (`validIds`) qui ont été extraites de [!DNL Privacy Service], ainsi qu’un tableau d’identités introuvables (`failedIds`). |
...
Toutes les applications qui utilisent le [!DNL Privacy JS Library] doivent instancier un nouvel objet `AdobePrivacy`, qui doit être configuré sur une solution Adobe spécifique. Par exemple, une instanciation pour Adobe Analytics pourrait se présenter comme suit :
...
Les exemples de code suivants montrent comment utiliser le [!DNL Privacy JS Library] pour plusieurs scénarios courants, à condition que vous n’utilisiez pas de balises.
...
| `failedIDs` | Un objet JSON contenant tous les identifiants qui n’ont pas été récupérés depuis [!DNL Privacy Service] ou qui sont introuvables. |
...
Cette section contient des informations supplémentaires sur l’utilisation de l’[!DNL Privacy JS Library] .
...
| --- | --- |
| `key` | Identifiant unique qui identifie l’utilisateur ou le titulaire de données. Cette propriété est conçue pour être utilisée à des fins de suivi interne et n’est pas utilisée par Adobe. |

**Adobe Analytics**

| Paramètre | Description |
| --- | --- |
| `cookieDomainPeriods` | Nombre de périodes dans un domaine utilisé pour le suivi des cookies (par défaut `2`, par exemple `.domain.com`). Ne le définissez pas ici, sauf si spécifié dans votre balise web JavaScript. |
| `dataCenter` | Le centre de données de collecte de données Adobe. Il ne doit être inclus que s’il est spécifié dans votre balise web JavaScript. Les valeurs potentielles sont les suivantes : <ul><li>`d1` : centre de données de San Jose</li><li>`d2` : centre de données de Dallas</li></ul> |
