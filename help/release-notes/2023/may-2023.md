...
| Fonctionnalité | Description |
| --- | --- |
| Prise en charge des identifiants de partenaire dans les applications Adobe Experience Cloud [!BADGE bêta]{type=Informative} | Les identifiants de partenaire sont désormais disponibles dans le service d’identités. Les identifiants de partenaire sont des identifiants utilisés par les partenaires de données pour représenter des personnes. Dans Real-time Customer Data Platform, les identifiants de partenaire sont utilisés principalement pour l’activation étendue des audiences et l’enrichissement des données. Les identifiants de partenaire ne sont pas stockés dans le graphique d’identité. Pour plus d’informations, consultez la documentation sur les [types d’identité](../../identity-service/namespaces.md#identity-types). |

Pour en savoir plus sur Identity Service, consultez la [vue d’ensemble d’Identity Service](../../identity-service/home.md).
...
| Fonctionnalité | Description |
| --- | --- |
| Calculer des statistiques au niveau des colonnes sur les jeux de données ADLS | La variable `ANALYZE TABLE` a été étendue avec la fonction `COMPUTE STATISTICS` Commande SQL. Vous pouvez désormais calculer des statistiques pour un sous-ensemble d’un jeu de données ADLS ou pour certaines colonnes de ce jeu de données. Pour plus d’informations, veuillez consulter le [guide de calcul des statistiques des jeux de données](../../query-service/essential-concepts/dataset-statistics.md). |
