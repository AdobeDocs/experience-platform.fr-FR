...
| [Extension de balise de la couche de données Google](../../tags/extensions/web/google-data-layer/overview.md) | L’extension de la couche de données Google vous permet d’utiliser une couche de données Google lorsque vous implémentez des balises. |
...
| [Extension de transfert d’événement Mailchimp](../../tags/extensions/web/mailchimp/overview.md) | L’extension de transfert d’événement Mailchimp envoie des événements à l’API marketing Mailchimp qui peut déclencher des e-mails pour les campagnes marketing, les parcours ou les transactions Mailchimp. |
...
| Schéma global | (Plusieurs) | De nouveaux schémas globaux sont disponibles pour les mesures de destinations pour Real-Time CDP Insights. Voir la [requête de stratégie pull](https://github.com/adobe/xdm/pull/1560) pour plus d’informations. |
