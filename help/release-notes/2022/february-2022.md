...
| Fonctionnalité | Description |
| ----------- | ----------- |
| (Version bêta) Prise en charge des Destinations SDK pour les destinations basées sur des fichiers | [Prise en charge des Destinations SDK pour les destinations basées sur des fichiers](../../destinations/destination-sdk/functionality/destination-server/server-specs.md) est actuellement en version bêta privée et n’est disponible que pour un certain nombre de partenaires et de clients. Les fonctionnalités et la documentation associée peuvent faire l’objet de modifications avant la mise à disposition d’une version générale.<br><br>Contactez votre représentant de compte d’Adobe pour savoir comment accéder à la fonctionnalité. Les représentants du compte interne de l’Adobe doivent contacter les équipes produit et ingénieur des destinations Experience Platform pour discuter des cas d’utilisation pris en charge. <br><br> Lors de la phase bêta de la prise en charge des Destinations SDK pour les destinations basées sur des fichiers, les partenaires bêta et les clients peuvent utiliser le [Destination SDK Experience Platform](../../destinations/destination-sdk/overview.md) pour créer des destinations privées afin de bénéficier des fonctionnalités suivantes : <ul><li>Créez une destination basée sur des fichiers (par lots) via Amazon S3, les serveurs SFTP, Azure Blob, Azure Data Lake Storage, Data Landing Zone de stockage.</li><li>Configurez et définissez les options de planification et de fréquence d’exportation des fichiers par défaut.</li><li>Configurez et définissez des options pour formater vos fichiers CSV exportés (délimiteurs, caractères d’échappement et autres options).</li><li>Possibilité de définir et de modifier des en-têtes de fichier personnalisés.</li><li>Capacité à recevoir des notifications d’événement sur l’exportation de fichiers et de segments.</li><li>Possibilité d’exporter des types de fichiers supplémentaires tels que CSV, TSV, JSON, Parquet.</li></ul>  <br>Pour commencer à utiliser la nouvelle fonctionnalité, lisez [(Version bêta) Utilisez la Destination SDK pour configurer une destination basée sur des fichiers](../../destinations/destination-sdk/guides/configure-file-based-destination-instructions.md). <br><br> La fonctionnalité de création privée ou de production *diffusion en continu* les destinations à l’aide de Destination SDK sont déjà disponibles pour tous les clients et partenaires Experience Platform. Lisez le guide sur la façon de [utiliser la Destination SDK pour configurer une destination de diffusion en continu ;](../../destinations/destination-sdk/guides/configure-destination-instructions.md) pour plus d’informations. |
