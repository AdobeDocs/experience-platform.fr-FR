...
| --- | --- | --- |
| Ressources ajoutées | <ul><li> Politique de contrôle d’accès </li><li> Rôle </li><li> Journaux d’audit </li><li> Ordre de travail </li><li> Espace de noms d’identité </li><li> Graphique d’identité </li><li> Requête </li><li> Jeu de données </li><li> Flux de données source </li></ul> | Les ressources du journal d’audit sont automatiquement enregistrées lorsque l’activité se produit. Si la fonctionnalité est activée, vous ne devez pas activer manuellement la collecte des journaux. |
...
| --- | --- | --- |
| Comportement | [[!UICONTROL Time-series Schema]](https://github.com/surbhi114/xdm/blob/master/components/behaviors/time-series.schema.json) | Mise à jour de `eventType` pour inclure plusieurs nouveaux types d’événements liés aux médias et un cas d’utilisation entrant de canal web pour Adobe Journey Optimizer. |
...
| Groupe de champs | [[!UICONTROL Record Status]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/record-status.schema.json) | Mise à jour du statut du groupe de champs de `experimental` à `stable`. |
| Groupe de champs | (Plusieurs) | Plusieurs groupes de champs B2B ont été mis à jour afin que certains champs d’identifiant soient abandonnés au profit de champs de type clé utilisant le type de données [[!UICONTROL B2B Source]](../../xdm/data-types/b2b-source.md). Les champs d’identifiant précédents seront abandonnés lors d’une prochaine mise à jour. Consultez la [requête de stratégie pull](https://github.com/adobe/xdm/pull/1533/files#diff-720c0bb1d1cbaf622f5656c2a4b62d35830c75f6563794da72a280a6a520fbc1) pour obtenir la liste complète des modifications apportées aux groupes de champs concernés. |
| Type de données | [[!UICONTROL Browser details]](https://github.com/liljenback/xdm/blob/master/components/datatypes/browserdetails.schema.json) | Ajout d’un nouveau champ `xdm:userAgentClientHints` qui capture les informations contextuelles sur l’agent utilisateur interagissant avec le navigateur. |
| Type de données | [[!UICONTROL Media information]](https://github.com/lidiaist/xdm/blob/master/docs/reference/datatypes/media.schema.json) | Ajout d’un champ `xdm:playhead` pour capturer l’heure du curseur de lecture d’un contenu multimédia. Correction de la validation du modèle de `xdm:videoSegment`. |
| Type de données | [[!UICONTROL Rating]](https://github.com/lidiaist/xdm/blob/master/components/datatypes/external/iptc/rating.schema.json) | `iptc4xmpExt:RatingSourceLink` n’est plus un champ obligatoire. |
...
| Fonctionnalité | Description |
| --- | --- |
| Intégration du journal d’audit du service de requête | L’intégration du journal d’audit du service de requête fournit des enregistrements des actions de l’utilisateur ou de l’utilisatrice liées aux requêtes dans le but de résoudre les problèmes ou de respecter les politiques de gestion des données d’entreprise et les exigences réglementaires. Consultez la [documentation sur l’intégration du journal d’audit](../../query-service/data-governance/audit-log-guide.md) pour obtenir des informations complètes. |
| Création de la commande SQL ALTER TABLE | Utilisez SQL pour définir les identités principales dans un jeu de données ad hoc. Le service de requête vous permet de marquer les colonnes de jeux de données comme identités principales ou secondaires directement via SQL à l’aide de la commande `ALTER TABLE`. |

{style="table-layout:auto"}

Pour plus d’informations sur les fonctionnalités du service de requête, consultez la section [Présentation du service de requête](../../query-service/home.md)
