...
description: Dernières notes de mise à jour pour Experience Platform
...
| Intégration Adobe Audience Manager | Tout segment qui est partagé avec [!DNL Audience Manager] à partir de [!DNL Platform] hérite des libellés d’utilisation des données appliqués comme [!DNL Data Export Controls], et vice versa. Consultez la [!DNL Audience Manager] documentation pour connaître les [mappages spécifiques entre les libellés d’utilisation et les contrôles des exportations de données](https://experienceleague.adobe.com/docs/audience-manager/user-guide/implementation-integration-guides/integration-experience-platform/aam-aep-audience-sharing.html?lang=fr). |
| Libellés d’utilisation des données personnalisés | Vous pouvez désormais créer des libellés d’utilisation des données personnalisés à l’aide de l’API Policy Service ou dans l’interface utilisateur. Pour plus d’informations, consultez la [présentation des étiquettes](../../data-governance/labels/overview.md) . |
...
| Fonctionnalité | Description |
| ------- | ----------- |
| Application des stratégies d’utilisation des données | Dans [!DNL Real-time Customer Data Platform], les violations de stratégie d’utilisation des données sont automatiquement survenues lorsqu’une action violante est tentée dans l’espace de travail [!UICONTROL Profil]. Pour plus d’informations sur l’application automatique des stratégies, voir les [notes de mise à jour relatives à la gouvernance des données](#governance) . |
