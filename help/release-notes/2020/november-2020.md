...
| ------- | ----------- |
| Autorisations | Dans la balise [!DNL Admin Console], l’onglet d’un profil de produit [!DNL Platform] vous permet de personnaliser les fonctionnalités [!DNL Platform] disponibles pour les utilisateurs associés à ce profil. Les catégories d’autorisations disponibles sont les suivantes : **[!UICONTROL Modélisation des données]**, **[!UICONTROL Gestion des données]**, **[!UICONTROL Gestion des profils]**, **[!UICONTROL Identity Management]**, **[!UICONTROL Surveillance des données]**, **[!UICONTROL Administration des environnements de test]**, **[!UICONTROL Destinations&lt;a 13/>,**[!UICONTROL  Data Ingestion ]**,**[!UICONTROL  Data Science Workspace ]**,**[!UICONTROL  Query Service ]**et**[!UICONTROL  Gouvernance des données ]**.]** |
...
| Moteur de décision d’offre | Le moteur de décision d’offre tire parti des données [!DNL Platform] et [!DNL Real-time Customer Profiles], ainsi que de la bibliothèque des offres, afin de sélectionner l’heure, les clients et les canaux auxquels les offres seront diffusées. |
...
| Fonction Mapper | [!DNL Data Prep] Le mappeur a désormais la possibilité de  **** ne pas copier un attribut de la source vers le XDM cible. |

Pour plus d’informations, reportez-vous à la [[!DNL Data Prep] présentation](../../data-prep/home.md).

## Data Science Workspace {#dsw}

Data Science Workspace utilise le machine learning et l’intelligence artificielle pour exploiter les informations contenues dans vos données. Intégré à Adobe Experience Platform, Data Science Workspace vous aide à obtenir des prévisions en utilisant votre contenu et des ressources de données de l’ensemble des solutions Adobe. L’utilisation de [!DNL JupyterLab] permet d’y parvenir. [!DNL JupyterLab] est une interface utilisateur web pour [[!DNL Project Jupyter]](https://jupyter.org/) et est étroitement intégré à Adobe Experience Platform. Il fournit un environnement de développement interactif permettant aux spécialistes des données de travailler avec des notebooks, du code et des données [!DNL Jupyter].

**Fonctionnalités clés**

| Fonctionnalité | Description |
| ------- | ----------- |
| [!DNL JupyterLab] Modèle Recipe Builder | Mise à jour de l’utilisation et des versions du notebook vers les exigences de recette. [!DNL Python] L’image de base de l’environnement d’exécution ML a été mise à jour afin d’utiliser exclusivement la version  [!DNL Python] 3.6.7 et un  [!DNL Conda] environnement. |

Pour plus d’informations, consultez le document sur la [création d’une recette à l’aide des notebooks Jupyter](../../data-science-workspace/jupyterlab/create-a-recipe.md).
...
| Microsoft Bing | La destination Microsoft Bing vous aide à exécuter le reciblage et l’audience des campagnes numériques ciblées sur Microsoft Display Advertising. |
...
Pour plus d’informations, consultez la section sur l’ [ajout de champs d’identité à un jeu de données](../../intelligent-services/data-preparation.md#add-identity-fields-to-the-dataset) dans le guide de préparation des données d’Intelligent Services .
...
Pour plus d’informations, consultez la [présentation Attribution AI](../../intelligent-services/attribution-ai/overview.md).
...
Pour plus d’informations sur Real-time Customer Profile, y compris des tutoriels et des bonnes pratiques concernant l’utilisation des données [!DNL Profile], consultez la [présentation de Real-time Customer Profile](../../profile/home.md).
