...
description: Notes de mise à jour de juin 2019 pour Adobe Experience Platform.
...
| Exploration des données | Accès en toute transparence aux données XDM ingérées dans [!DNL Platform] intégré à [!DNL Platform Data Access] SDK. |
...
| [!DNL Decisioning] API REST | Le [!DNL Decisioning Service] peut être appelé à l’aide d’une API REST simple pour obtenir la meilleure offre disponible pour une personne donnée. Une API de mesures peut être utilisée pour vérifier les valeurs de limitation et les propositions d’offre en temps réel. |
| Diffusion en continu des événements de décision dans [!DNL Data Lake] et [!DNL Query Service] | Le [!DNL Decisioning Service] crée automatiquement des jeux de données pour diffuser automatiquement tous les événements de décision XDM dans la variable [!DNL Data Lake]. Les jeux de données sont alors disponibles pour la création d’analyses et de rapports à l’aide de [!DNL Query Service]. |
...
Pour plus d’informations sur [!DNL Query Service], reportez-vous à la section [documentation du produit](../../query-service/home.md).
