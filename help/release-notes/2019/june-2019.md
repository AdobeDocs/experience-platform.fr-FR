...
| Première expérience client | Inclut des exemples prêts à l’emploi pour diverses structures et langues d’apprentissage automatique telles que [!DNL Python], R, PySpark et Scala [!DNL Spark]. |
| Notebooks | Environnement personnalisé pour les spécialistes des données/ingénieurs en données, optimisé par [!DNL Jupyter Notebooks] pour leur permettre de préparer des données, d’extraire des fonctionnalités et de développer des modèles ML avec une liste organisée de bibliothèques et de structures d’apprentissage automatique populaires. |
...
| Visualisation des données | Possibilité d’exécuter des requêtes SQL dans [!DNL Jupyter Notebooks] pour accélérer la préparation des données et l’ingénierie des fonctionnalités. |
...
| Notation par lots | Enrichissez [!DNL Real-Time Customer Profile] avec des insights d’apprentissage automatique ou écrivez-les en tant que jeux de données à nouveau sur [!DNL Platform] |
...
| Diffusion en continu des événements de décision dans [!DNL Data Lake] et [!DNL Query Service] | [!DNL Decisioning Service] crée automatiquement des jeux de données pour diffuser automatiquement tous les événements de décision XDM dans le [!DNL Data Lake]. Les jeux de données sont ensuite disponibles pour l’analyse et la création de rapports à l’aide de [!DNL Query Service]. |
...
| Connectivité de l’outil de veille stratégique | Utilisez les pilotes PostgreSQL (Postgres) trouvés dans les outils de BI courants pour vous connecter à [!DNL Query Service] afin de créer des rapports et des visualisations. Les outils pris en charge comprennent : [!DNL Tableau], [!DNL Power BI] et [!DNL Looker]. Recherchez les informations d’authentification dans l’onglet dédié. |
| Connectivité de l’outil de gestion de base de données | Connectez [!DNL Aqua Data Studio] ou [!DNL DB Visualizer] à [!DNL Query Service] pour accéder aux fonctionnalités d’exploration des données et de création de jeux de données. [!DNL Query Service] prend également en charge la connectivité depuis R Studio. Recherchez les informations d’authentification dans l’onglet dédié. |
| Outil de requête de ligne de commande | Connectez PSQL pour pouvoir exécuter des requêtes à partir de la ligne de commande. |
| Journal des requêtes | Conserve un historique des requêtes exécutées par [!DNL Query Service] et vous permet de rechercher des requêtes SQL antérieures pour modification, exécution ou création d’un jeu de données à partir des résultats. |
| API de planification de requête | Planifiez des requêtes pour une exécution récurrente via cette API. |

**Problèmes connus**

* [!DNL Query Editor] présente un échantillon de 100 lignes de résultats pour vos requêtes. Pour conserver le jeu de résultats complet, utilisez les fonctionnalités de création de jeux de données du journal de requêtes.
...
* Impossible d’étendre un groupe de champs fourni par [!DNL Platform] en ajoutant un champ.
