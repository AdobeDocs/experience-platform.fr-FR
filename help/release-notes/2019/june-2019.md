...
Adobe Experience Platform [!DNL Data Science Workspace] est un service entièrement géré au sein de [!DNL Experience Platform] qui permet aux spécialistes des données de générer en toute transparence des informations à partir de données et de contenu entre les solutions d&#39;Adobe et les systèmes tiers en construisant et en mettant en oeuvre des modèles d&#39;apprentissage automatique. [!DNL Data Science Workspace] est étroitement intégré à et alimente le cycle de vie de la science des données de bout en bout, y compris l’exploration et la préparation des données XDM, puis le développement et la mise en œuvre de modèles pour enrichir automatiquement avec des insights d’apprentissage automatique.[!DNL Platform][!DNL Real-time Customer Profile]

**Fonctionnalités clés**

| Fonctionnalité | Description |
| -----------| ---------- |
| Mise en service et isolation du calcul | Fournir les ressources informatiques dédiées nécessaires pour permettre aux chercheurs de données d&#39;exécuter du code non approuvé dans [!DNL Experience Platform] de manière sécurisée. |
| Première expérience client | Comprend des exemples prêts à l&#39;emploi pour divers cadres et langages d&#39;apprentissage automatique tels que [!DNL Python], R, PySpark et Scala [!DNL Spark]. |
| Notebooks | Environnement personnalisé pour les spécialistes des données et les ingénieurs de données alimenté par [!DNL Jupyter Notebooks] pour leur permettre de préparer des données, d&#39;extraire des fonctionnalités et de développer des modèles ML avec une liste de bibliothèques et des structures d&#39;apprentissage automatique populaires. |
| Exploration des données | Accès transparent aux données XDM assimilées à [!DNL Platform] intégrées au SDK [!DNL Platform Data Access]. |
...
| Évaluation de modèles | Evaluer et optimiser les modèles de régression et de classification dans [!DNL Python], PySpark, R et Scala. |
...
| Notation par lots | Enrichissez [!DNL Real-time Customer Profile] les informations d&#39;apprentissage automatique ou écrivez-les en tant que jeux de données dans [!DNL Platform] |
| Planification | Intégré à [!DNL Platform] Orchestration Service pour automatiser la formation, la notation et les pipelines de modèles avec des calendriers définis par les utilisateurs via les API. |
...
| [!DNL Decisioning] API REST | [!DNL Decisioning Service] peut être appelé à l&#39;aide d&#39;une API REST simple pour obtenir la meilleure Offre suivante pour une personne donnée. Une API de mesures peut être utilisée pour vérifier les valeurs de limitation et les propositions d’offre en temps réel. |
| Diffusion en flux continu des Événements de décision en [!DNL Data Lake] et [!DNL Query Service] | [!DNL Decisioning Service] crée automatiquement des jeux de données pour diffuser automatiquement tous les Événements de décision XDM dans [!DNL Data Lake]. Les jeux de données sont alors disponibles pour la création d’analyses et de rapports à l’aide de [!DNL Query Service]. |
...
[!DNL Query Service] permet d’utiliser des commandes SQL standard pour rechercher des données dans Adobe Experience Platform afin de prendre en charge de nombreux cas d’utilisation d’analyse et de gestion des données. Il s&#39;agit d&#39;un outil sans serveur qui vous permet de joindre n&#39;importe quel jeu de données dans [!DNL Data Lake] et de capturer les résultats de la requête sous la forme d&#39;un nouveau jeu de données à utiliser dans le rapports, [!DNL Data Science Workspace] ou pour l&#39;assimilation dans [!DNL Profile Service].

Vous pouvez utiliser [!DNL Query Service] pour construire des écosystèmes d&#39;analyse de données, en créant une image des consommateurs dans leurs différents canaux d&#39;interaction. Ces canaux peuvent prendre les formes suivantes :
...
| Création de jeux de données | Créez des jeux de données sur [!DNL Experience Platform] au moyen de la syntaxe SQL standard. |
...
| Connectivité de l’outil de veille stratégique | Utilisez les pilotes PostgreSQL (Postgres) des outils de BI courants pour vous connecter à [!DNL Query Service] afin de créer des rapports et des visualisations. Les outils pris en charge sont les suivants : [!DNL Tableau], [!DNL Power BI] et [!DNL Looker]. Recherchez les informations d’authentification dans l’onglet dédié. |
| Connectivité de l’outil de gestion de base de données | Connectez [!DNL Aqua Data Studio] ou [!DNL DB Visualizer] à [!DNL Query Service] pour la fonctionnalité d&#39;exploration des données et de création de jeux de données. [!DNL Query Service] prend également en charge la connectivité à partir de R Studio. Recherchez les informations d’authentification dans l’onglet dédié. |
...
Pour plus d&#39;informations sur [!DNL Query Service], consultez la [documentation du produit](../../query-service/home.md).
...
* Correction d’un bogue en raison duquel [!DNL Schema Registry] donnait des réponses aléatoires pour les appels de schéma d’union par profil.
...
* Impossible d&#39;étendre un groupe de champs [!DNL Platform] fourni en ajoutant un champ.
