...
| Exploration des données | Accès en toute transparence aux données XDM ingérées dans [!DNL Platform] intégrées au SDK [!DNL Platform Data Access]. |
...
| Notation par lots | Enrichissez [!DNL Real-time Customer Profile] avec des insights d’apprentissage automatique ou écrivez-les en tant que jeux de données dans [!DNL Platform] |
| Planification | Intégration avec [!DNL Platform] Orchestration Service afin d’automatiser les pipelines de formation, de notation et de fonctionnalités des modèles avec des plannings définis par l’utilisateur via les API. |
...
| [!DNL Decisioning] API REST | [!DNL Decisioning Service] peut être appelé à l’aide d’une API REST simple pour obtenir la meilleure offre disponible pour une personne donnée. Une API de mesures peut être utilisée pour vérifier les valeurs de limitation et les propositions d’offre en temps réel. |
| Diffusion en continu des événements de décision dans [!DNL Data Lake] et [!DNL Query Service] | [!DNL Decisioning Service] crée automatiquement des jeux de données pour diffuser automatiquement tous les événements de décision XDM dans le [!DNL Data Lake]. Les jeux de données sont alors disponibles pour la création d’analyses et de rapports à l’aide de [!DNL Query Service]. |
...
[!DNL Query Service] permet d’utiliser des commandes SQL standard pour rechercher des données dans Adobe Experience Platform afin de prendre en charge de nombreux cas d’utilisation d’analyse et de gestion des données. Il s’agit d’un outil sans serveur qui vous permet de joindre n’importe quel jeu de données dans [!DNL Data Lake] et de capturer les résultats de la requête sous la forme d’un nouveau jeu de données à utiliser dans les rapports, [!DNL Data Science Workspace], ou pour une ingestion dans [!DNL Profile Service].

Vous pouvez utiliser [!DNL Query Service] pour créer des écosystèmes d’analyse des données, en créant une image des consommateurs sur leurs différents canaux d’interaction. Ces canaux peuvent prendre les formes suivantes :
...
| Journal des requêtes | Conserve l’historique des requêtes exécutées par [!DNL Query Service] et vous permet de rechercher des requêtes SQL antérieures pour modification, exécution ou création d’un jeu de données à partir des résultats. |
