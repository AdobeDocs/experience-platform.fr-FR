...
Les connecteurs Adobe Experience Platform vous aident à ingérer facilement des données provenant de plusieurs sources, ce qui vous permet de structurer, d’étiqueter et d’améliorer vos données à l’aide des services [!DNL Experience Platform]. Les connecteurs [!DNL Platform] vous permettent de vous authentifier auprès des services de gestion de la relation client et des systèmes de stockage basés sur le cloud, de définir des heures d’ingestion et de gérer le débit d’ingestion des données.

| Fonctionnalité | Description |
| -----------| ---------- |
| Connecteurs de stockage basés sur le cloud | Ingérez des données de [!DNL Microsoft Azure Blob] et de l’espace de stockage dans le cloud [!DNL Amazon S3] vers Adobe Experience Platform |
| [!DNL Microsoft Dynamics CRM] &amp; [!DNL Salesforce CRM] | Découvrez toutes les données disponibles dans le CRM [!DNL Microsoft Dynamics] ou [!DNL Salesforce]. Ingérez n&#39;importe quel objet, standard ou personnalisé, à partir de [!DNL Microsoft Dynamics] ou de [!DNL Salesforce] CRM. Les données incrémentielles peuvent être ingérées toutes les heures ou de façon quotidienne. | |
...
| Fonctionnalité | Description |
| -----------| ---------- |
| Adobe Analytics | Les données des suites de rapports d’Adobe Analytics sont disponibles dans [!DNL Platform]. |
| Adobe Audience Manager | Les journaux de collecte de données, les données intégrées, les données de segments et de caractéristiques de Adobe Audience Manager sont disponibles dans [!DNL Platform]. |
| [!DNL Advertising Cloud] | DSP journaux de [!DNL Advertising Cloud] sont disponibles dans [!DNL Platform]. |
| Adobe Campaign Standard | Les données de profil et d’événement de Adobe Campaign Standard sont disponibles dans [!DNL Platform]. |
...
Le [!DNL Catalog] est le système d’enregistrement pour l’emplacement et la parenté des données au sein d’Adobe Experience Platform. [!DNL Catalog] contient les métadonnées et les descriptions des fichiers et répertoires dans [!DNL Experience Platform], agissant comme un magasin de métadonnées où vous pouvez trouver des informations sur toutes les données stockées dans [!DNL Platform].

| Fonctionnalité | Description |
| -----------| ---------- |
| [!DNL Catalog Service] API | [!DNL Catalog] fournit une API RESTful à partir de laquelle les utilisateurs peuvent créer et gérer plusieurs types d’objets, y compris des comptes, des lots, des connexions, des connecteurs, des jeux de données, des balises et des transformations liés aux données [!DNL Experience Platform]. |
...
| Fonctionnalité | Description |
| -----------| ---------- |
| [!DNL Schema Registry] | [!DNL Schema Registry] fournit une interface utilisateur ([!DNL Schema Editor]) et une API RESTful à partir de laquelle toutes les ressources liées aux schémas peuvent être visualisées et gérées. L’API [!DNL Schema Editor] et [!DNL Schema Registry] peut également être utilisée pour créer et gérer de nouveaux schémas et ressources propres à chaque organisation. |
| [!DNL Schema Library] | [!DNL Schema Library] contient toutes les ressources [!DNL Schema Registry], y compris celles définies par une organisation individuelle et celles mises à la disposition de tous les utilisateurs par Adobe, [!DNL Experience Platform] partenaires et fournisseurs dont les applications sont intégrées à [!DNL Experience Platform]. |
...
[!DNL Profile] est un magasin d’entités de recherche générique qui fusionne les données de diverses ressources de données d’entreprise et permet d’accéder à ces données sous la forme de profils consommateurs en temps réel et d’événements de série temporelle associés, appelés la vue d’union. La vue unifiée permet d’informer et d’autonomiser les actions sur les canaux, [!DNL Platform] et d’Adobe les intégrations de solutions. De plus, [!DNL Profile] est utilisé pour alimenter l’apprentissage automatique et Sensei. Les API [!DNL Profile] peuvent également être utilisées directement pour enrichir les fonctionnalités des solutions tierces, des systèmes CRM et des solutions propriétaires.
...
| Recherche de violations de politiques | Déterminez si les stratégies d’utilisation définies sont violées lors de l’exécution d’actions marketing sur des données [!DNL Platform]. |
