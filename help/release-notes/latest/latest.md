...
| Alertes contextuelles pour les flux de données de sources | Vous pouvez maintenant vous abonner pour recevoir des messages d’alerte concernant l’état de vos flux de données pendant le processus d’ingestion. For more information, see the guide on [subscribing to sources alerts in the UI](../../sources/tutorials/ui/alerts.md). |
...
| Améliorations de la source [!DNL Event Hubs] | Le [!DNL Event Hubs] source prend désormais en charge le type d’authentification de clé SAS non racine pour se connecter et créer une connexion source. For more information, see the [[!DNL Event Hubs] overview](../../sources/connectors/cloud-storage/eventhub.md). |
| Améliorations de la source [!DNL SFTP] | Le [!DNL SFTP] source vous permet désormais d’établir un nombre défini de connexions simultanées maximales qu’un flux de données peut utiliser pour se connecter au serveur SFTP. For more information, see the [[!DNL SFTP] overview](../../sources/connectors/cloud-storage/sftp.md). |
