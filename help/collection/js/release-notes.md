...
- Les [remplacements de flux de données](/help/datastreams/overrides.md) sont désormais pris en charge au démarrage des sessions multimédia.
...
- Correction d’une erreur qui se produisait lorsque les éléments de contenu par défaut étaient rendus via [`applyPropositions`](commands/applypropositions.md).
- Correction d’une erreur CSS dans les actions de déplacement et de redimensionnement d’Adobe Target.
- Suppression de la clé `machineLearning` des réponses [`sendEvent`](commands/sendevent/overview.md).
...
- Ajout de la prise en charge de [Streaming Media Collection](commands/configure/streamingmedia.md).
...
- Ajout de la prise en charge des [événements haut et bas de page](../use-cases/personalization/top-bottom-page-events.md).
...
- Ajout de la prise en charge des [remplacements par commande de l’identifiant de train de données](/help/datastreams/overrides.md).
...
- Ajout de la prise en charge du rappel de clic sur les liens [`onBeforeLinkClickSend`](commands/configure/onbeforelinkclicksend.md).
...
- Correction d’un problème en raison duquel les propriétés de l’[indicateur client de l’agent utilisateur](../use-cases/client-hints.md) pouvaient être définies sur un type incorrect.
...
- Le SDK peut désormais être installé en tant que package [NPM](install/npm.md).
- Ajout de la prise en charge d’une option `out` lors de la [configuration du consentement par défaut](commands/configure/defaultconsent.md), qui ignore tous les événements jusqu’à ce que le consentement soit reçu (l’option `pending` existante met les événements en file d’attente et les envoie une fois le consentement reçu).
- Le rappel [`onBeforeEventSend`](commands/configure/onbeforeeventsend.md) peut désormais être utilisé pour empêcher l’envoi d’un événement.
...
- La commande [`getIdentity`](commands/getidentity.md) renvoie désormais l’identifiant de zone géographique Edge ainsi que l’identité.
- Amélioration et gestion plus efficace des avertissements et des erreurs reçus du serveur.
- Ajout de la prise en charge de la norme de consentement 2.0 d’Adobe pour la commande [`setConsent`](commands/setconsent.md).
