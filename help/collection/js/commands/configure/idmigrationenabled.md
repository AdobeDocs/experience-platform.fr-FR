...
La propriété `idMigrationEnabled` permet au SDK Web de lire les cookies AMCV définis par le service d’identification des visiteurs (ECID) Adobe précédent. Si votre entreprise met à niveau votre implémentation vers Web SDK, ce paramètre permet une transition plus fluide vers Experience Platform Identity Service actuel. Ce paramètre est utile pour éviter une forte augmentation du nombre de visiteurs uniques lors de la mise à niveau vers le SDK web.
...
Le service d’identification des visiteurs stocke l’Experience Cloud ID (ECID) dans un cookie appelé `AMCV_<ORG_ID>`. Lorsque `idMigrationEnabled` est `true` (valeur par défaut), le SDK Web lit automatiquement l’ECID à partir du cookie AMCV et l’utilise comme identité du visiteur lors de la première requête Edge Network.

1. Un visiteur arrive sur une page qui utilise désormais le SDK web plutôt que le service d’identification des visiteurs.
...
Pour que la migration fonctionne, le Web SDK doit être configuré avec le même [`orgId`](/help/collection/js/commands/configure/orgid.md) que le service d’identification des visiteurs utilisé et doit être déployé sur le même domaine (ou un sous-domaine du même domaine parent) où le cookie AMCV a été défini.

## Scénarios de migration pris en charge

La migration des identifiants prend en charge les schémas de transition suivants :

* **Certaines pages utilisent toujours le service d’identification des visiteurs tandis que d’autres pages utilisent Web SDK** : le SDK lit les cookies AMCV existants et écrit un nouveau cookie avec l’ECID existant. Il écrit également des cookies AMCV afin que les pages qui utilisent toujours le service d’identification des visiteurs continuent à reconnaître le même visiteur.
* **Web SDK et le service d’identification des visiteurs sont tous deux présents sur la même page** : si le cookie AMCV n’est pas défini, le SDK recherche le service d’identification des visiteurs sur la page et l’utilise pour obtenir l’ECID.
...
Si la mise en œuvre du service d’identification des visiteurs a utilisé des identifiants tiers (cookie `demdex.net`), le SDK web peut également les migrer. Configurez les [`thirdPartyCookiesEnabled`](/help/collection/js/commands/configure/thirdpartycookiesenabled.md) à `true` dans votre configuration de Web SDK. Le SDK Web lit le cookie Demdex existant et inclut l’identité tierce dans ses requêtes Edge Network, en suivant le même modèle de migration que le cookie AMCV.

## Validation {#validation}

Pour vérifier que la migration d’identité fonctionne correctement :

1. Ouvrez une page qui utilisait auparavant le service d’identification des visiteurs (qui utilise désormais le SDK Web) dans un navigateur qui possède déjà un cookie `AMCV_`.
