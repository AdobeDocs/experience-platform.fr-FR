...
>[!IMPORTANT]
>
>Les identifiants d’appareils propriétaires ne sont pas compatibles avec la fonctionnalité [cookies tiers](/help/tags/extensions/client/web-sdk/configure/identity.md) de Web SDK. Vous pouvez utiliser des identifiants d’appareil propriétaires ou des cookies tiers, mais pas les deux simultanément.

## Conditions préalables {#prerequisites}

Avant de commencer, assurez-vous de connaître le fonctionnement des données d’identité dans le SDK web, y compris les ECID et les `identityMap`. Pour plus d’informations, consultez la présentation des [données d’identité dans le SDK Web](id-overview.md).
...
Prenons l’exemple suivant : vous souhaitez implémenter Web SDK sur les `example.com` de votre site web. Le SDK Web envoie des données à Edge Network vers le domaine `edge.adobedc.net`.

| Sans [!DNL CNAME] | Avec [!DNL CNAME] |
|---------|----------|
| <ul><li>Le `example.com` de votre site web utilise le `edge.adobedc.net` de domaine Web SDK pour envoyer des données à Edge Network.</li><li>Les cookies définis par `edge.adobedc.net` sont considérés comme des cookies tiers, puisqu’ils ne proviennent pas de votre domaine `example.com`. Selon les navigateurs de vos utilisateurs, les cookies tiers peuvent être bloqués et vos données n’atteignent pas Edge Network.</li></ul> | <ul><li>Vous créez un sous-domaine dans lequel vous déployez Web SDK, tel que `metrics.example.com`.</li><li>Vous définissez un enregistrement [!DNL CNAME] dans votre système DNS de sorte que `metrics.example.com` pointe vers `edge.adobedc.net`.</li><li>Lorsque votre site web définit des cookies par l’intermédiaire de `metrics.example.com`, ils semblent provenir du navigateur `example.com` (tiers) au lieu de `edge.adobedc.net` (tiers). Cela rend le blocage du cookie d’ID propriétaire moins probable, ce qui garantit une collecte de données plus précise.</li></ul> |
...
Consultez la [documentation sur la configuration des trains de données](/help/datastreams/configure.md#advanced-options) pour savoir comment configurer votre train de données.
